...
function parseSlot(slot: string, groupCount: number, groupSize?: number): { groupIndex: number; slotIndex: number } | null {
  ...
  if (
    ...
    slotIndex < 1 ||
    (groupSize != null && slotIndex > groupSize)
  ) {
    return null;
  }
...
function assignTeamsToGroups<T extends { id: bigint; seedNumber: number | null }>(
  ...
  if (seedAssignments.length > 0) {
    ...
    for (const assignment of seedAssignments) {
      ...
      const parsed = parseSlot(assignment.slot, groupCount, groupSize);
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  let teams = await prisma.tournamentTeam.findMany({
    where: {
      tournamentId,
      status: "approved",
      category: rule.code,
  ...
  let shouldNormalizeSingleDivisionCategory = false;
  if (teams.length === 0) {
    const ruleCount = await prisma.tournamentDivisionRule.count({ where: { tournamentId } });
    if (ruleCount === 1) {
      teams = await prisma.tournamentTeam.findMany({
        where: { tournamentId, status: "approved" },
        orderBy: [{ seedNumber: "asc" }, { createdAt: "asc" }],
        select: { id: true, seedNumber: true, team: { select: { name: true } } },
      });
      shouldNormalizeSingleDivisionCategory = teams.length > 0;
    }
  }
    ...
    assignments.map(({ team, groupName, seedNumber, groupOrder }) => {
      ...
      return prisma.tournamentTeam.update({
        ...
        data: {
          ...
          ...(shouldNormalizeSingleDivisionCategory && { category: rule.code }),
        },
      });
    }),
  );
