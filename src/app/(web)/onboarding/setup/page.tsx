...
/* ============================================================
 * /onboarding/setup — BDR v2 신규 가입 프로필 설정 위저드
 *
 * 이유(왜):
 *   Phase 9 P1-1 에서 새로 만든 공용 StepWizard 셸의 "첫 사용처"로,
 *   시안 OnboardingV2.jsx 6단계+완료 화면을 박제한다. 이 페이지가
 *   없으면 위저드 셸이 실제 어떻게 쓰이는지 보여줄 레퍼런스가 없어
 ...
 *   - 완료 화면에서 `/profile` 로 이동하는데, /profile 페이지가 자체적으로
 *     비로그인 카드를 띄우므로 onboarding 자체엔 redirect 가드 미설치.
 *   - "use client" 컴포넌트라 서버 redirect 도 어색하므로 의도적으로 비둠.
 *
...
export default function OnboardingSetupPage(): ReactElement {
