...
export default function CourtDetailLoading() {
  return (
    <div className="space-y-6 animate-pulse">
      {/* 히어로 이미지 스켈레톤 */}
      <div className="h-48 md:h-64 rounded-md bg-[var(--border)]" />

      {/* 코트 이름 + 주소 */}
      <div className="space-y-2">
        <div className="h-8 w-2/3 rounded-lg bg-[var(--border)]" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="h-40 rounded-md bg-[var(--border)]" />
        <div className="h-40 rounded-md bg-[var(--border)]" />
      </div>
      ...
      <div className="space-y-3">
        ...
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-20 rounded-md bg-[var(--border)]" />
        ))}
      </div>
    </div>
  );
