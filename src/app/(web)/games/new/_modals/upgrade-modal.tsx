...
export function UpgradeModal({ reason, onClose }: UpgradeModalProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm rounded-[20px] bg-white p-6 shadow-xl mx-4">
        <div className="mb-4 flex items-start justify-between">
          <div>
            <h2 className="text-lg font-bold text-[#111827]">{info.title}</h2>
            <p className="mt-1 text-sm text-[#6B7280]">{info.description}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="ml-3 flex-shrink-0 text-xl leading-none text-[#6B7280] hover:text-[#111827]"
          >
            ✕
          </button>
        </div>
