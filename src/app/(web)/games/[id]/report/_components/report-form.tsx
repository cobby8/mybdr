...
// 경기 후 리포트 폼 (client) — 시안: Dev/design/BDR v2 (1)/screens/GameReport.jsx (198줄)
// 이유:
//   - server wrapper(page.tsx)가 인증/게임/참가자 fetch 처리 → 이 컴포넌트는 form 로직만 담당.
//   - props.players는 실제 game 참가자 (호스트 + 승인된 신청자). PLACEHOLDER_PLAYERS 제거.
//   - 시안의 team/num/color는 DB에 없는 시각 메타 → 인덱스 기반으로 자동 분배 (홀=A팀 빨강, 짝=B팀 파랑).
//   - 평가 페이로드는 player.id (string, BigInt 문자열) 그대로 전송 → 서버 UUID 검증 통과.
//   - 임시저장 LocalStorage 키도 string id 기반으로 변경 (number 캐스팅 제거).
...
// server에서 내려주는 player shape — id는 string(BigInt), 시각 메타는 client에서 derive
...
type DisplayPlayer = ReportPlayer & {
  displayName: string;
  team: "A" | "B";
  num: number;
  color: string;
...
// 시안 라인 12~19의 team/num/color 매핑을 인덱스 기반으로 자동 부여
// 이유: DB에 team/num 컬럼이 없으므로 시각만 박제 — 짝수 idx=A팀 빨강, 홀수=B팀 파랑
function toDisplayPlayer(p: ReportPlayer, idx: number): DisplayPlayer {
  const isTeamA = idx % 2 === 0;
  return {
    ...p,
    displayName: p.nickname?.trim() || p.name?.trim() || "익명",
    team: isTeamA ? "A" : "B",
    num: idx + 1, // 1부터
    color: isTeamA ? "#DC2626" : "#0F5FCC",
    pos: p.position?.trim() || "—",
  };
}

// 페이지 props — server wrapper에서 내려준다
export type GameReportFormProps = {
  gameId: string; // URL params.id (= games.uuid)
  game: {
    title: string | null;
...
export function GameReportForm({
  ...
  // 시각 메타 derive (props.players는 안정적이므로 매 렌더 계산 OK)
  // useMemo도 가능하지만 현 규모에서는 인라인 map이 더 단순
  const displayPlayers: DisplayPlayer[] = players.map(toDisplayPlayer);

  // 7개 state 시안 라인 4~10 박제 — id 타입만 number → string 으로 변경
  const [submitted, setSubmitted] = useState(false);
  // ratings: { [playerId(string)]: 1~5 }
  const [ratings, setRatings] = useState<Record<string, number>>({});
  // reports: { [playerId(string)]: ['noshow', 'foul', ...] }
  const [reports, setReports] = useState<Record<string, string[]>>({});
  // mvp: 추천한 선수 id (단일 선택, 토글) — string
  const [mvp, setMvp] = useState<string | null>(null);
  // noshows: 불참/노쇼 체크된 선수 id 배열 — string[]
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 이유: server wrapper에서 redirect 처리하지만, GET 호출 중 세션 만료 케이스 방어
        if (res.status === 401) {
          if (!cancelled) {
        ...
        if (rep) {
          ...
          // mvp_user_id는 서버상 string(BigInt). 이제 player.id도 string이라 직접 매칭 OK
          setMvp(rep.mvp_user_id ?? null);
          // ratings 배열 → state map 변환 (key=string)
          const ratingMap: Record<string, number> = {};
          const flagMap: Record<string, string[]> = {};
          const noshowList: string[] = [];
          for (const r of rep.ratings) {
            ratingMap[r.rated_user_id] = r.rating;
            flagMap[r.rated_user_id] = r.flags;
            if (r.is_noshow) noshowList.push(r.rated_user_id);
          }
  ...
  const toggleReport = (id: string, flag: string) => {
    setReports((prev) => {
      const cur = prev[id] || [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 페이로드 빌드 — 평점 0(미평가)은 제외. id는 이제 실제 BigInt 문자열이므로 그대로 전송.
      const ratingsBody = displayPlayers
        .filter(
          (p) =>
            (ratings[p.id] ?? 0) > 0 ||
  ...
  const dateLabel = game.date
    ...
    : "";
  const titleLabel = game.title?.trim() || "경기";
  const hostLabel = game.host_nickname?.trim()
    ? `호스트 ${game.host_nickname.trim()}`
    : "호스트로서 리포트 작성";
  ...
  if (submitted) {
    ...
    const reportCount = Object.keys(reports).filter((k) => (reports[k] ?? []).length > 0).length;
    const mvpName = mvp
      ? displayPlayers.find((p) => p.id === mvp)?.displayName ?? "—"
      : "—";
  ...
  return (
    <div className="page">
      ...
      {/* 페이지 헤더 (시안 라인 72~76) — 실 game 메타 사용 */}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="card" style={{ padding: "22px 24px" }}>
            ...
            {displayPlayers.length === 0 ? (
              // 참가자 0명 폴백 — 시안에는 없는 케이스지만 빈 배열일 때 빈 카드 회피
                    ...
                    >
                      ...
                      <div>
                        <div style={{ display: "flex", gap: 8, alignItems: "baseline" }}>
                          <div style={{ fontWeight: 800, fontSize: 14 }}>{p.displayName}</div>
          ...
          <div className="card" style={{ padding: "16px 18px" }}>
            ...
            >
              ...
              <b style={{ fontFamily: "var(--ff-mono)" }}>
                {Object.keys(ratings).length}/{displayPlayers.length}
              </b>
            </div>
            <div
              style={{
