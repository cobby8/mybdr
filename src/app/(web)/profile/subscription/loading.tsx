...
export default function SubscriptionLoading() {
  return (
    <div className="space-y-6 animate-pulse">
      ...
      {/* 현재 구독 상태 카드 */}
      <div className="rounded border border-[var(--border)] bg-[var(--bg-card)] p-6 space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Array.from({ length: 2 }).map((_, i) => (
          <div
            key={i}
            className="rounded border border-[var(--border)] bg-[var(--bg-card)] p-6 space-y-3"
          >
            <div className="h-6 w-20 rounded bg-[var(--border)]" />
            <div className="h-8 w-28 rounded bg-[var(--border)]" />
            <div className="space-y-2">
              {Array.from({ length: 4 }).map((_, j) => (
                <div key={j} className="h-4 w-full rounded bg-[var(--border)]" />
              ))}
            </div>
          </div>
        ))}
