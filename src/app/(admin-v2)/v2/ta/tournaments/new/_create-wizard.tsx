...
const ALLOWED_FORMATS = Object.keys(FORMAT_LABEL);
...
// ── 폼 타입 ───────────────────────────────────────────────
type Venue = { id: string; name: string; region: string; courtCount: number; naming: "num" | "alpha" };
type DateRow = { id: string; date: string; courtIds: string[] };
type DivisionRow = { id: string; label: string; cap: number | null; fee: number };
type GameRules = {
  quarterType: "4Q" | "HALF";
...
// 작은 보조 컴포넌트 — 정본 Field/GroupTitle/Stepper/SegSm 1:1
...
function GroupTitle({ children, flush }: { children: React.ReactNode; flush?: boolean }) {
  return <div className={"ct-group-title" + (flush ? " ct-group-title--flush" : "")}>{children}</div>;
}
function Stepper({ value, unit, min = 1, max = 8, onChange }: { value: number; unit?: string; min?: number; max?: number; onChange: (n: number) => void }) {
  return (
...
// ── 장소 직접 추가 입력(정본 VenueSearch 의 mock 체육관 DB 제외·실 입력만) ──
function VenueAdd({ onAdd }: { onAdd: (name: string) => void }) {
...
// ── 일정 추가(date 입력 — 정본 CalendarModal 대체) ──
function DateAdd({ onAdd }: { onAdd: (date: string) => void }) {
