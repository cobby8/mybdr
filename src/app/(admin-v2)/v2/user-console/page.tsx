// ============================================================
// (admin-v2)/v2/user-console/page.tsx — R2-A 유저 콘솔 (BO-1 핵심)
//   탭 3(사용자/팀/단체) 리스트 + 행 드릴다운 상세. 정본 bo-pages ConsolePage +
//   bo-detail-2 1:1. ⚠ 백엔드 0변경 — 리스트/상세 READ 는 서버 컴포넌트 Prisma
//   단일 매핑(snake→표시값 1곳=snake 함정 차단). 단체 인증 mutation 은 실 REST
//   (adminFetch) · 회원 정지 mutation 은 실 server action(updateUserStatusAction) 재사용.
// ============================================================
...
export default async function AdminV2UserConsole() {
  const session = await getWebSession();
  const currentUserId = session?.sub ?? null;

  // 3개 리스트 병렬 조회(전부 READ) — 상세 데이터(소속팀/로스터/운영진)도 include 로 동시 적재
