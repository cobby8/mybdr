            ...
            {data.monthlyUsers.map((m) => {
              ...
              return (
                <div key={m.month} className="flex flex-1 flex-col items-center gap-2">
                  {/* 2026-05-12 — 강조 통계 빨강 → info(Navy) 톤다운 + 차트 막대도 info */}
                  <span className="text-xs font-semibold text-[var(--color-info)]">{m.count}</span>
                  <div className="flex w-full flex-col justify-end" style={{ height: "140px" }}>
                    <div
                      className="w-full rounded-t-[6px] bg-[var(--color-info)] transition-all"
