...
├── Codex-project-knowledge/     영구 — 9 파일 박제 룰
...
`Dev/design/Codex-project-knowledge/` (9 파일)
...
- `07-custom-instructions.md` — Custom Instructions 텍스트 (Codex.ai Project 용)
- `README.md` — 사용 가이드
...
**D. 카피 / 모바일**:
11. 글로벌 카피 — **시안 우선** (사용자 결정 2026-05-01). "서울 3x3 농구 커뮤니티" / "다음카페" 카피 ✅ 시안 보존. About 운영진 실명 박제 ❌ (보존). 자세한 사유: `Dev/design/Codex-project-knowledge/01-user-design-decisions.md` §6-1
...
```
[Step 1] Dev/design/Codex-project-knowledge/00-master-guide.md 읽기 (13 룰 인지)
...
Dev/design/          디자인 시안
.Codex/knowledge/   6파일 지식 베이스
.Codex/scratchpad.md 작업 현황 (100줄 이내)
...
- 관리자 화면에서 공개 프론트 표시를 제어할 수 있어야 한다. 새 표시값은 관리자 입력 → 저장 API → DB → 공개 API/페이지까지 한 흐름으로 연결한다.
- 미배선 기능은 숨기거나 저장/노출 불가 상태를 명시하고, 운영 저장 완료처럼 보이게 하지 않는다.
- 더미 데이터는 테스트/시안/스토리 전용 영역에만 허용하고 운영 경로에는 넣지 않는다.
...
이 프로젝트는 클로드 CLI에서 작업하고 있어. 지금 클로드 디자인을 통해 UI, UX 작업 중이고 이 부분에 대한 적용을 클로드 CLI에 적절한 계획과 프롬프트를 작성하는 역할을 여기서 할거야. 또 앱의 전체적 완성도 점검과 자동 검증 시스템에 대한 계획 및 프롬프트도 작성해서 제공해서 앱의 완성도를 높이는 역할을 하자.
