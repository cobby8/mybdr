...
> BDR v2.34 · 2026-06-18 · 코워크(Claude Code) 구현용 핸드오프 패키지
...
- **출석 토글** `.att-toggle`: `inline-flex; border:1px var(--border); border-radius:var(--radius-pill)`. 버튼 2개 [출석][미출석], `min-height:44px; padding:0 16px; font:700 13px`. active present → `background:var(--ok); color:#fff` / active absent → `background:var(--warn); color:#000`. 저장: 낙관적 업데이트(`PUT /attendance`), 0.5s 저장중 표시.
