    ...
    <AdminShell
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 10, marginBottom: 16 }}>
        ...
        <AdminChecklistCard
          ...
          onClick={() => {
            // v2.15 — 각 항목 → 보조 페이지 deep link (운영 라우트와 동일)
            if (it.key === 'basic' || it.key === 'apply_policy' || it.key === 'format' || it.key === 'recording') setRoute('adminTournamentEditWizard');
            else if (it.key === 'series') setRoute('adminTournamentEditWizard'); // series 는 wizard 의 시리즈 섹션
            else if (it.key === 'divisions') setRoute('adminTournamentDivisions');
            else if (it.key === 'site') setRoute('adminTournamentSite');
            else if (it.key === 'bracket') setRoute('adminTournamentBracket');
          }} />
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            <SubAction icon="groups" label="참가팀" desc="38팀" onClick={() => setRoute('adminTournamentTeams')} />
            <SubAction icon="manage_accounts" label="관리자" desc="3명" onClick={() => setRoute('adminTournamentAdmins')} />
            <SubAction icon="edit_note" label="기록원" desc="2명" onClick={() => setRoute('adminTournamentRecorders')} />
            <SubAction icon="sports_basketball" label="경기" desc="24경기" onClick={() => setRoute('adminTournamentMatches')} />
          </div>
        </div>
      </section>
    </AdminShell>);
