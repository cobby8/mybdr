...
//   - tournament_admin  — 콘텐츠 > 대회 관리 (header_only) > 대회 운영자 도구 만
...
},
{
  ...
  items: [
  {
    ...
    children: [
    { key: 'adminWizardTournament', label: '대회 운영자 도구', icon: 'manage_accounts', roles: ['super_admin', 'site_admin', 'tournament_admin'] }]
...
},
{
  ...
  { key: 'adminReports', label: '신고 검토', icon: 'report', roles: ['super_admin'] },
  ...
  for (const grp of ADMIN_NAV) {
    ...
    for (const it of grp.items) {
      if (it.roles && !it.roles.includes(role)) {
        // parent self-blocked: 아이템의 roles 에서 제외돼도 children 중 하나라도 접근 가능하면
        // header_only 로 라벨만 노출 (tournament_admin 의 "대회 관리" 경우)
        const visibleChildren = (it.children || []).filter((c) => !c.roles || c.roles.includes(role));
        if (visibleChildren.length > 0) {
          items.push({ ...it, header_only: true, children: visibleChildren });
        }
        continue;
      }
      const children = (it.children || []).filter((c) => !c.roles || c.roles.includes(role));
      ...
      <nav className="admin-aside__nav">
        ...
        <div key={grp.group_id} className="admin-aside__group">
          ...
          <React.Fragment key={it.key}>
                {it.header_only ?
            <div className="admin-aside__link" data-active="false" style={{ opacity: 0.55, cursor: 'default' }}>
        ...
        <div className="admin-mobile-drawer__body">
          ...
          <div key={grp.group_id} className="admin-aside__group">
            ...
            <React.Fragment key={it.key}>
                ...
                data-active={route === it.key ? 'true' : 'false'}
                onClick={() => {
                  if (!it.header_only) setRoute(it.key);
                  setOpen(false);
                }}>
