      ...
      <div className="app-nav__main">
        ...
        <div className="app-nav__right">
          ...
          <div style={{position:'relative'}} onClick={(e)=>e.stopPropagation()}>
            ...
            {moreOpen && (
              <div style={{position:'absolute', top:'calc(100% + 6px)', right:0, width:680, maxHeight:'70vh', overflowY:'auto', background:'var(--bg)', border:'1px solid var(--border)', boxShadow:'var(--sh-lift)', borderRadius:10, padding:'12px', zIndex:30, display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4px 18px'}}>
                {moreGroups.map(g => (
                  <div key={g.title} style={{breakInside:'avoid'}}>
                    <div style={{fontSize:10, fontWeight:800, letterSpacing:'0.08em', color:'var(--ink-mute)', textTransform:'uppercase', padding:'8px 10px 4px'}}>{g.title}</div>
                    {g.items.map(m => (
                      <button key={m.id+g.title} onClick={()=>go(m.id)} style={{display:'flex', alignItems:'center', gap:10, width:'100%', padding:'7px 10px', background: route === m.id ? 'var(--bg-alt)' : 'transparent', border:0, cursor:'pointer', fontSize:12.5, color:'var(--ink)', textAlign:'left', borderRadius:6, whiteSpace:'nowrap'}} onMouseEnter={(e)=>e.currentTarget.style.background='var(--bg-alt)'} onMouseLeave={(e)=>e.currentTarget.style.background = route === m.id ? 'var(--bg-alt)' : 'transparent'}>
                        <span style={{width:18, textAlign:'center', fontSize:13, flexShrink:0}}>{m.icon}</span>
                        <span style={{fontWeight: route === m.id ? 700 : 500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{m.label}</span>
                      </button>
                    ))}
                  </div>
                ))}
