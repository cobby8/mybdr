...
function Checkout({ setRoute }) {
  ...
  const pay = () => {
    ...
    setTimeout(() => {
      setProcessing(false);
      // 카드 결제는 전용 성공 페이지로, 다른 수단은 인라인 완료 UI (가상계좌 안내 등)
      if (method === 'card') {
        sessionStorage.setItem('mybdr.paySuccess', JSON.stringify({ title: ctx.title, code: ctx.code, total, method }));
        setRoute('pricingSuccess');
      } else if (method === 'transfer') {
        // 가상계좌는 인라인 완료 UI 유지
        setComplete(true);
      } else {
        // toss/kakao — 외부 결제창 시뮬: 90% 성공, 10% 실패
        if (Math.random() > 0.1) {
          sessionStorage.setItem('mybdr.paySuccess', JSON.stringify({ title: ctx.title, code: ctx.code, total, method }));
          setRoute('pricingSuccess');
        } else {
          sessionStorage.setItem('mybdr.payFail', JSON.stringify({ title: ctx.title, code: ctx.code, method, reason:'PAY_PROCESS_CANCELED' }));
          setRoute('pricingFail');
        }
      }
    }, 1400);
  };
