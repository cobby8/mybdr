    ...
    <div className="page">
      ...
      {mode === 'team' ? (
        <div className="board data-table">
          <div className="board__head data-table__head" style={{gridTemplateColumns:'64px 1fr 90px 90px 90px 90px'}}>
            <div>순위</div><div>팀</div><div>레이팅</div><div>승</div><div>패</div><div>승률</div>
          </div>
          {teams.map((t, i) => (
            <div key={t.id} className="board__row data-table__row" style={{gridTemplateColumns:'64px 1fr 90px 90px 90px 90px'}}>
              <div data-label="순위" className="num" style={{fontFamily:'var(--ff-display)', fontSize:16, fontWeight:900, color: i<3 ? 'var(--accent)' : 'var(--ink-dim)'}}>{i+1}</div>
              ...
              <div data-label="레이팅" style={{fontFamily:'var(--ff-mono)', fontWeight:700}}>{t.rating}</div>
              <div data-label="승" style={{color:'var(--ok)', fontFamily:'var(--ff-mono)', fontWeight:700}}>{t.wins}</div>
              <div data-label="패" style={{color:'var(--ink-mute)', fontFamily:'var(--ff-mono)'}}>{t.losses}</div>
              <div data-label="승률" style={{fontFamily:'var(--ff-mono)'}}>{Math.round(t.wins/(t.wins+t.losses)*100)}%</div>
            </div>
          ))}
        </div>
      ) : (
        <>
          ...
          <div className="board data-table">
            <div className="board__head data-table__head" style={{gridTemplateColumns:'56px 1fr 72px 72px 72px 72px 80px 64px'}}>
              <div>순위</div><div>선수</div><div>팀</div><div>PPG</div><div>APG</div><div>RPG</div><div>레이팅</div><div>변동</div>
            </div>
            {players.map((p, i) => (
              <div key={p.id} className="board__row data-table__row" style={{gridTemplateColumns:'56px 1fr 72px 72px 72px 72px 80px 64px'}}>
                <div data-label="순위" className="num" style={{fontFamily:'var(--ff-display)', fontSize:15, fontWeight:900, color: i<3 ? 'var(--accent)' : 'var(--ink-dim)'}}>{i+1}</div>
                <div data-primary="true" className="title">
                ...
                <div data-label="팀" style={{fontFamily:'var(--ff-mono)', fontSize:11, fontWeight:700}}>{p.team}</div>
                <div data-label="PPG" style={{fontFamily:'var(--ff-mono)', fontWeight: sort==='ppg' ? 800 : 500, color: sort==='ppg' ? 'var(--ink)' : 'var(--ink-mute)'}}>{p.ppg}</div>
                <div data-label="APG" style={{fontFamily:'var(--ff-mono)', fontWeight: sort==='apg' ? 800 : 500, color: sort==='apg' ? 'var(--ink)' : 'var(--ink-mute)'}}>{p.apg}</div>
                <div data-label="RPG" style={{fontFamily:'var(--ff-mono)', fontWeight: sort==='rpg' ? 800 : 500, color: sort==='rpg' ? 'var(--ink)' : 'var(--ink-mute)'}}>{p.rpg}</div>
                <div data-label="레이팅" style={{fontFamily:'var(--ff-mono)', fontWeight:800}}>{p.rating}</div>
                <div data-label="변동"><Trend v={p.trend}/></div>
              </div>
            ))}
          </div>
        </>
