  ...
  return (
    <div className="page">
      ...
      {shown.length === 0 ? (
        ...
        <div className="games-grid" style={{display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:12}}>
          {shown.map(g => {
            const pct = fillPct(g);
            ...
            const ringColor = full ? 'var(--ink-mute)' : hot ? 'var(--accent)' : kc;
            const remain = full ? (waitAvail ? g.waitlist : 0) : g.spots - g.applied;
            const md = g.date.match(/(\d{2})\.(\d{2})\s*\(/);
            const dow = (g.date.match(/\(([^)])\)/) || [])[1] || '';
            const start = (g.time.match(/(\d{1,2}:\d{2})/) || [])[1] || g.time;
            return (
              <div key={g.id} className="card games-card" style={{padding:'14px 16px', display:'grid', gridTemplateColumns:'56px 1fr auto', gap:14, alignItems:'center', cursor:'pointer'}} onClick={()=>setRoute('gameDetail')}>
                <div className="games-ring" style={{width:56, height:56, borderRadius:'50%', background:`conic-gradient(${ringColor} ${pct*3.6}deg, var(--bg-alt) 0)`, display:'grid', placeItems:'center'}} aria-hidden="true">
                  <div style={{width:42, height:42, borderRadius:'50%', background:'var(--bg-card)', display:'grid', placeItems:'center', textAlign:'center', lineHeight:1}}>
                    <div>
                      <div style={{fontFamily:'var(--ff-mono)', fontWeight:800, fontSize:15, color:ringColor}}>{remain}</div>
                      <div style={{fontSize:8, color:'var(--ink-dim)', marginTop:1, letterSpacing:'.02em'}}>{full ? (waitAvail ? '대기' : '마감') : '자리'}</div>
                    </div>
                  </div>
                </div>
                <div style={{minWidth:0}}>
                  ...
                  <div className="games-card__title" style={{fontWeight:700, fontSize:15, letterSpacing:'-0.005em', marginBottom:3, color:'var(--ink)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{g.title}</div>
                  <div className="games-card__meta" style={{fontSize:12, color:'var(--ink-mute)', overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical'}}>
                    {g.court} · {md ? `${md[1]}.${md[2]}` : g.date}({dow}) {start} · {g.level} · <span style={{color: isFree(g) ? 'var(--ok)' : 'var(--ink-soft)', fontWeight: isFree(g) ? 700 : 400}}>{g.fee}</span>
                  </div>
                </div>
                <button className={`btn btn--sm ${full ? '' : 'btn--primary'}`} onClick={(e)=>e.stopPropagation()} style={{whiteSpace:'nowrap'}}>{full ? (waitAvail ? '대기 신청' : '마감') : '신청'}</button>
              </div>
            );
          })}
        </div>
      ...
      <style>{`
        ...
        .games-chip[data-active="true"] {
          background: var(--cafe-blue); border-color: var(--cafe-blue); color: var(--ink-on-brand);
        }
        ...
        @media (max-width: 720px) {
          ...
          .games-grid { gap: 10px !important; }
          .games-card { gap: 12px !important; padding: 12px 14px !important; }
        }
      `}</style>
    </div>
  );
