...
## 섹션 1-A 🔥 (NEW · 2차 검증) — Phase 9 신규 박제 라우트 진입점 점검

`design_v2` 브랜치의 BDR v2 신규 시안 박제 7개 라우트 중, **실제 진입점을 갖지 못한** 라우트.

| 라우트 | 시안 출처 | 진입점 후보 수 | 직접 링크 위치 | 결론 |
|--------|----------|-------------|---------------|------|
| `/games/[id]/report` | GameReport.jsx | **0** | 어디에도 없음 | 🔥 단절 |
| `/games/[id]/guest-apply` | GuestApply.jsx | **0** | 어디에도 없음 | 🔥 단절 |
| `/tournaments/[id]/referee-request` | RefereeRequest.jsx | **0** | 어디에도 없음 | 🔥 단절 |
| `/match` | Match.jsx | **0** | 어디에도 없음 | 🔥 단절 |
| `/courts/submit` | CourtAdd.jsx | 1 (More 메뉴만) | `more-groups.ts:69` | ⚠️ More 1점 의존 |
| `/onboarding/setup` | OnboardingV2.jsx | 1 (More 메뉴만) | `more-groups.ts:100` "가입 설정" | ⚠️ More 1점 의존 + 회원가입 직후 자동 진입 누락 |
| `/series/new` | SeriesCreate.jsx | **0** (사용자 페이지) | `/tournament-admin/series/new` 가 별도로 존재(admin) — 사용자용 진입점 없음 | 🔥 단절 |

→ **신규 박제 7개 중 4개가 0건, 3개가 1건 미만**. 박제 작업이 "페이지 만들고 끝"으로 종료되어 진입점 연결이 후속 작업으로 미뤄진 상태.
...
## 섹션 1-B 🔥 (NEW · 2차 검증) — More 메뉴 가짜 링크

`src/components/bdr-v2/more-groups.ts` 의 항목 중, **레이블은 구체 기능을 가리키지만 href 는 부모 허브로만 점프**하는 케이스. 사용자가 항목을 누르면 "왜 다른 페이지로 가지?" 혼란 발생.

| More 메뉴 라벨 | 시안 의도 | 현재 href | 실제 기능 라우트 | 갭 |
|--------------|----------|----------|---------------|-----|
| 대진표 | bracket | `/tournaments` | `/tournaments/[id]/bracket` | 토너먼트 선택 단계 거쳐야 함 |
| 경기 결과 | gameResult | `/games/my-games` | `/games/[id]/result` (또는 동등) | 종료된 게임 직접 진입 X |
| 경기 신고·평가 | gameReport | `/games/my-games` | `/games/[id]/report` | 신규 박제 라우트 미연결 |
| 게스트 지원 신청 | guestApply | `/games` | `/games/[id]/guest-apply` | 신규 박제 라우트 미연결 |
| 대회 접수 | tournamentEnroll | `/tournaments` | `/tournaments/[id]/join` (등) | 토너먼트 선택 단계 |
| 팀 관리 | teamManage | `/teams` | `/teams/[id]/manage` | 본인 팀 찾기 단계 |
| 심판 배정 요청 | refereeRequest | `/tournaments` | `/tournaments/[id]/referee-request` | 신규 박제 라우트 미연결 |

→ More 메뉴 자체는 5그룹 IA로 잘 박제됐으나, **레이블 ↔ href 의미 일치율이 낮음**. 동적 라우트 특성상 ID 없이는 직접 못 가는 경우는 어쩔 수 없지만, 사용자에게 "왜 부모로 보내는지" 안내 또는 항목 자체를 제거하는 정리 필요.
...
| 작업 | 파일 | 변경 내용 |
|------|------|---------|
| `/games/[id]/report` 진입점 | 게임 상세 v2 컴포넌트 | 종료된 경기에 "경기 평가" CTA |
| `/games/[id]/guest-apply` 진입점 | 게임 상세 v2 컴포넌트 | "게스트 모집중" 배지 + CTA |
| `/tournaments/[id]/referee-request` 진입점 | 토너먼트 운영 영역 | 운영자/주최자만 보이는 CTA |
| `/match` 정책 결정 | `src/app/(web)/match/page.tsx` | (a) admin/preview 이동 / (b) More 추가 / (c) 폐기 — PM 결정 |

**0-B. More 메뉴 가짜 링크 정리** (섹션 1-B)
7개 항목 — A안(직접 라우트로) 또는 B안(More에서 제거)으로 통일.
