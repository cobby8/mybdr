        ...
        <div>
          ...
          <div className="card" style={{padding:'18px 22px', marginBottom:14}}>
            <div style={{display:'flex', alignItems:'baseline', justifyContent:'space-between', marginBottom:14}}>
              <h2 style={{margin:0, fontSize:15, fontWeight:700}}>리뷰 ({venue.reviewCount})</h2>
              <a onClick={requireAuth()} style={{fontSize:12, color:'var(--cafe-blue)', cursor:'pointer'}}>전체 보기 →</a>
            </div>
            <div style={{display:'grid', gap:14}}>
              {reviews.map((r, i) => (
                <div key={i} style={{paddingBottom:14, borderBottom: i<reviews.length-1 ? '1px solid var(--border)' : 'none'}}>
                  <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:4}}>
                    <b style={{fontSize:13}}>{r.name}</b>
                    <span style={{color:'#F59E0B', fontSize:13}}>{'★'.repeat(r.rating)}<span style={{color:'var(--border)'}}>{'★'.repeat(5-r.rating)}</span></span>
                  </div>
                  <p style={{margin:0, fontSize:13, color:'var(--ink-soft)', lineHeight:1.6}}>{r.body}</p>
                </div>
              ))}
            </div>
          </div>
