...
| main 배포 정합 | 분기 해소 | `07427de`로 main 변경 27커밋 dev 병합. `origin/main..origin/dev` 22 / `origin/dev..origin/main` 0 |
...
- 릴리스 전 남은 작업은 dev→main PR 생성/체크 확인
...
| 새 대회 생성폼 +8 | dev 반영 + `BDR-current` handoff 보존 | dev→main 릴리스 PR 판단 |
...
`dev == origin/dev`이고 v2.40 핵심 시안 갭과 main/dev 분기는 해소됨. 다음은 **dev→main 릴리스 PR 판단 → 일관성 QA/STAGE E/F/G 재개** 순서가 안전함.
