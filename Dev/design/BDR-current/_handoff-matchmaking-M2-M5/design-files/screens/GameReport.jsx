...
function GameReport({ setRoute }) {
  ...
  const [noshows, setNoshows] = React.useState([6]); // 호스트 출석 체크에서 미출석 처리된 참가자
  const [overall, setOverall] = React.useState(0);
  const [comment, setComment] = React.useState('');

  const players = [
    { id:1, name:'rdm_sniper', team:'A', pos:'G', num:7, color:'var(--bdr-red)', attended:true },
    { id:2, name:'rdm_forward', team:'A', pos:'F', num:23, color:'var(--bdr-red)', attended:true },
    { id:3, name:'rdm_pivot', team:'A', pos:'C', num:44, color:'var(--bdr-red)', attended:true },
    { id:4, name:'hoops_alex', team:'B', pos:'G', num:11, color:'var(--cafe-blue)', attended:true },
    { id:5, name:'threes_jk', team:'B', pos:'F', num:22, color:'var(--cafe-blue)', attended:true },
    { id:6, name:'paint_bk', team:'B', pos:'C', num:33, color:'var(--cafe-blue)', attended:false },
  ];
      ...
      <div style={{display:'grid', gridTemplateColumns:'minmax(0,1fr) 300px', gap:18, alignItems:'flex-start'}}>
        <div style={{display:'flex', flexDirection:'column', gap:16}}>
          ...
          <div className="card" style={{padding:'22px 24px'}}>
            ...
            <div style={{display:'flex', gap:6, marginBottom:14}}>
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={()=>setOverall(n)} style={{
                  ...
                  background: overall>=n?'var(--accent)':'var(--bg-alt)', color:overall>=n?'var(--ink-on-brand)':'var(--ink-dim)',
            ...
            <textarea className="input" rows={2} value={comment} onChange={e=>setComment(e.target.value)} placeholder="경기 후기를 남겨주세요" style={{resize:'vertical'}}/>
          </div>

          {/* Per-player */}
          <div className="card" style={{padding:'22px 24px'}}>
            <h2 style={{margin:'0 0 4px', fontSize:16, fontWeight:700}}>선수별 평가</h2>
            <p style={{margin:'0 0 12px', fontSize:12, color:'var(--ink-mute)'}}>매너 점수 · 문제 있었다면 신고 (익명)</p>
            <div style={{display:'flex', gap:14, marginBottom:14, fontSize:11, color:'var(--ink-dim)'}}>
              <span style={{display:'flex', alignItems:'center', gap:5}}><span style={{width:8, height:8, borderRadius:'50%', background:'var(--ok)'}}/>출석 {players.filter(p=>!noshows.includes(p.id)).length}</span>
              <span style={{display:'flex', alignItems:'center', gap:5}}><span style={{width:8, height:8, borderRadius:'50%', background:'var(--warn)'}}/>노쇼 {noshows.length}</span>
            </div>
            <div style={{display:'flex', flexDirection:'column', gap:0}}>
              {players.map((p, i) => {
                const isNoshow = noshows.includes(p.id);
                return (
                <div key={p.id} style={{padding:'16px 0', borderTop: i>0?'1px solid var(--border)':'none', opacity: isNoshow ? .68 : 1}}>
                  <div style={{display:'grid', gridTemplateColumns:'44px 1fr auto', gap:12, alignItems:'center', marginBottom:10}}>
                    <Avatar tag={`${p.team}${p.num}`} color={p.color} ink="var(--ink-on-brand)" size={44} radius={6}/>
                    <div>
                      <div style={{display:'flex', gap:8, alignItems:'center', flexWrap:'wrap'}}>
                        <div style={{fontWeight:800, fontSize:14}}>{p.name}</div>
                        <span className={`badge ${isNoshow ? 'badge--warn' : 'badge--ok'}`}>{isNoshow ? '노쇼' : '출석'}</span>
                        <span className="badge badge--soft">{p.pos}</span>
                        <span className="badge badge--ghost" style={{background: `color-mix(in oklab, ${p.color} 13%, transparent)`, color:p.color}}>팀 {p.team}</span>
                      </div>
                      <div style={{fontSize:11, color:'var(--ink-dim)', fontFamily:'var(--ff-mono)', marginTop:2}}>#{p.num} · 매너 4.8 · 과거 픽업 23회</div>
                    </div>
                    <button onClick={()=>setMvp(p.id===mvp?null:p.id)} disabled={isNoshow} className={`btn btn--sm ${mvp===p.id?'btn--primary':''}`}>
                    ...
                    <div style={{display:'flex', gap:2}}>
                      {[1,2,3,4,5].map(n => (
                        <button key={n} onClick={()=>setRating(p.id, n)} style={{
                          ...
                          color:ratings[p.id]>=n?'var(--ink-on-brand)':'var(--ink-dim)', border:0, cursor:'pointer', fontSize:14, fontWeight:900,
                        }}>★</button>
                      ))}
                    </div>
