...
> BDR v2.36 · 2026-06-19 · 코워크(Claude Code) 구현용 핸드오프 패키지
> 동봉: `DATA-BINDING.md`(데이터 계약) · `design-files/`(HTML 시안 원본)

## 변경 요약 (v2.35 → v2.36) — 경기 카드 콤팩트화 + PC/태블릿/모바일 반응형
시안 A(GamesList) 경기 카드를 **시안 5 (용량 강조)** 로 교체. 데이터/상태 로직 동일, 토큰만 사용.
- **카드 교체** — 세로형(≈250px) → 가로 콤팩트 카드(≈118px). 좌측 **잔여 자리 링**(conic-gradient: "N 자리" / 마감·대기) + 배지(kind·상태) + 제목(1줄 ellipsis) + 메타 한 줄(장소·날짜(요일) 시간·레벨·비용) + 우측 신청/대기 버튼.
- **반응형** — 그리드 `repeat(auto-fill, minmax(300px, 1fr))`: PC 3열(≥1200) / 태블릿 2열(721–900) / 모바일 1열(≤720, responsive.css 300px 분기). 메타는 모바일에서 2줄 clamp. 카드 자체 `56px 1fr auto`(링·본문·CTA), ≤720 gap/padding 축소.
- **토큰** — kind 배지 on-color `#fff` → `var(--ink-on-brand)`, 칩 active 동일. 링·프로그레스 색은 kind 토큰(`--cafe-blue`/`--bdr-red`/`--ok`)·임박 시 `--accent`. 4px 라운딩(`--radius-card`) 유지.

**보존** — 정렬/필터 칩/kind 탭/빈 상태/상태 배지 로직 모두 동일. 카드 클릭→gameDetail, CTA는 stopPropagation. AppNav frozen.

## 변경 이력 (v2.34 → v2.35) — 룰 위반 수정 리비전
...
- 카드 그리드: `grid; gridTemplateColumns: repeat(auto-fill, minmax(300px, 1fr)); gap:12px` (PC 3열 / 태블릿 2열 / 모바일 1열).
...
- **경기 카드 (시안 5 · 용량 강조)** `.card.games-card` (`gridTemplateColumns:56px 1fr auto`, padding 14/16, 카드 전체 클릭→gameDetail):
  - **잔여 자리 링** — 56px conic-gradient(`${pct*3.6}deg` 채움, 나머지 `--bg-alt`), 내부 42px `--bg-card` 홀에 mono 숫자(잔여=spots-applied / 마감 0 / 대기=waitlist) + 8px 라벨(자리/마감/대기). 링·숫자 색 = 마감 `--ink-mute` / 80%+ · closing `--accent` / else kind 색.
  - 본문: 배지 row(kind 채움색+상태 배지+area mono 우측), 제목(700 15px, 1줄 ellipsis), 메타(`court · MM.DD(요) 시작 · 레벨 · 비용`, 무료는 `--ok` 700, ≤720 2줄 `-webkit-line-clamp:2`).
  - 우측 CTA `.btn--sm`(모집중 `--primary` 신청 / 마감 대기가능 대기 신청 / 마감), `stopPropagation`. 진행률·정원은 링이 대체(푸터 진행바 제거).
