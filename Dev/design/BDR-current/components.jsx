...
// (AppNav 제거 — DualSideNav 로 대체: dual-sidenav.jsx + MyBDR.html App)
...
// ============================================================
// TournamentTabBar — 5 탭 (개요 / 일정 / 대진표 / 참가팀 / 규정)
// Phase F3 — TournamentDetail / Schedule / Teams 페이지 공통
// ============================================================
function TournamentTabBar({ value, onChange }) {
  const tabs = [
    ...
    <div style={{
      ...
      {tabs.map(t => (
        <button key={t.id} onClick={()=>onChange && onChange(t.id)} style={{
          padding:'12px 18px', minHeight:44, background:'transparent',
          border:'none', borderBottom: value===t.id ? '2px solid var(--accent)' : '2px solid transparent',
          color: value===t.id ? 'var(--ink)' : 'var(--ink-mute)',
          fontWeight: value===t.id ? 700 : 500, cursor:'pointer', fontSize:13,
...
Object.assign(window, { Icon, ThemeSwitch, NavBadge, MemberPendingBadge, PasswordInput, Modal, ForceActionModal, Onboarding, LevelBadge, Pager, Sidebar, Avatar, Poster, SettingsToggle, SettingsRow, OnboardingStepHeader, SegmentedControl, OnboardingNav, TournamentTabBar });