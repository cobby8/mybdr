...
### [2026-06-16] 새 계산 로직 만들기 전 "정본 엔진" 탐색 — 사용자 "이미 한 작업 있다" 신호 = 재사용 1순위
- **분류**: lesson/process (기존 자산 재사용 / 중복 구현 방지)
- **발견자**: pm + live-expert (records 출전시간 PBP 재계산 `90d67e7`)
- **계기**: records MIN 비현실값(초 단위+999) 발견 → 처음엔 "÷60 변환" 단순 수정 검토. 사용자가 **"출전시간 이슈로 계산을 명확히 한 작업이 이미 있었다"** 한마디 → 전수 조사 결과 `minutes-engine v3`(test 21/21, PBP 재계산)가 **정본**이었고 라이브 화면만 쓰고 records는 raw 컬럼 직접 read(이원화)였음. 새 알고리즘 0줄, 기존 엔진 공용 추출+재사용으로 해결.
- **재발 방지 룰**: (a) 계산/집계 버그 수정 전 **동일 도메인의 기존 정본 로직 grep**(엔진/서비스/유틸) — 특히 "정확도 정비 이력"(knowledge decisions/lessons)이 있으면 그게 정본. (b) 사용자의 "전에 ~한 작업 있다" = 재사용 대상 존재 강신호, 새로 만들기 전 반드시 탐색. (c) 같은 알고리즘이 화면마다 다른 source를 쓰면(이원화) 한쪽이 버그 — 정본으로 통일. (d) 재사용 시 알고리즘은 import, 진입점(전처리)만 공용 추출 → 추출 시 원본 호출처 **회귀 비트단위 검증**(maxDiff=0 확인).
- **참조횟수**: 0
...
- **계기**: 사용자 "1건 사고 (매치 124) = 시스템적 결함 의심" → 전수 audit 박제 필요. 4 source 분산 박제 시스템에서 단일 매치 fix 후 잔존 매치 검증 미흡 시 같은 패턴 반복.
- **표준 절차 (운영 DB SELECT-only 진단)**:
  1. **운영 DB 안전 가드 §"운영 영향 0" 부합** — SELECT만 / schema 변경 0 / 사용자 결재 없이 즉시 실행 가능
  2. **`scripts/_temp/{문제}-audit.ts` 박제** — Prisma client + `$queryRaw` SELECT만 / 본 사고 후 정리
  3. **분류 (bucket) 기반 출력** — 단순 "일치/불일치" 가 아닌 패턴별 분류 (A/B/C/D/E/X) → 영구 fix 우선순위 도출
  4. **토너먼트별 + 매치별 2단계 집계** — 토너먼트별 "100% 불일치" 발견 시 시스템 결함, "10% 불일치" 시 운영 case-by-case
  5. **결과 박제** = `Dev/{문제}-audit-{date}.md` (gitignored 아님 — 의사결정 추적용)
  6. **errors.md + decisions.md 동시 박제** — 실측 결과 + 영구 fix 우선순위
- **이번 사례 결과**:
  - 1건 사고 (매치 124) → 시스템 56% 불일치 발견 (125 매치 중 70건)
  - 5 토너먼트 중 3건이 100% 불일치 → 운영 데이터 정정 + 영구 fix 동시 필요
  - F1~F6 영구 fix 6건 우선순위 박제 → Sprint 1/2/3 결재 근거
- **재발 방지 룰**:
  - **사고 1건 발견 시 = 같은 패턴 시스템 검색 의무** (errors.md 의 "재발 N회" 부터 시작)
  - **운영 DB SELECT 가드 면제** = `.env DATABASE_URL` 운영 DB / SELECT만 / schema 변경 0 → 사용자 결재 없이 즉시 실행 가능 (CLAUDE.md §DB 정책 명시)
  - **읽기 전용 audit script 표준화** — `scripts/_temp/` 위치 + Markdown 보고서 출력 + 본 사고 후 정리
  - **분류 (bucket) 기반 영구 fix 우선순위** — "전체 N% 불일치" 보다 "패턴별 분류" 가 영구 fix 설계에 유용
- **참조횟수**: 0
- **참조 파일**:
  - `scripts/_temp/score-consistency-audit.ts` (이번 audit script)
  - `Dev/score-consistency-audit-2026-05-21.md` (실측 보고서)
  - errors.md [2026-05-21] 시스템 차원 결함
...
- **채택 패턴 (4 Phase 분해)**:
  1. **Phase 0 점검 (read-only)** — planner-architect 위임 / `Dev/admin-flow-audit-{날짜}.md` 별도 보고서 / 코드 수정 0 / 분량 ~200~400 LOC markdown
  2. **Phase 1 우선 PR 분해** — 점검 §4 (단절/중복/혼란/누락) + §6 (개선 후보) → 우선순위 (영향도 H 우선) → 작은 PR 여러 개 (PR당 ~50~500 LOC / 회귀 위험 분리 / rollback 용이)
  3. **Phase 2 신규 페이지 / 기존 페이지 흡수** — Phase 1 결과 검증 후 진입 (별도 결재 / 시안 의존 BLOCKED 가능)
  4. **단계별 사용자 결재** — 각 Phase 진입 전 옵션 결재 (1 옵션 / 2 옵션 / 3 옵션 / 보류)
- **본 세션 적용 결과**:
  - Phase 0: planner 1회 위임 (~30분) → 보고서 231줄 + 18건 인벤토리 + 영향도 H 8건
  - Phase 1: PR 6개 분해 (Admin-1~6) → planner 통합 설계 2회 + developer 위임 6회 + PM 직접 commit 6회 (~3시간 / +3,060 LOC)
  - 회귀 0 보장 / 강남구협회장배 시나리오 §3 단계 4·7·10·10.5 단절 모두 해소
- **표준 패턴 룰**:
  (a) **점검 보고서는 read-only 별도 파일** — scratchpad 박제 0 (오염 방지)
  (b) **단절 / 중복 / 혼란 / 누락 4 분류 + 영향도 H/M/L** 표준
  (c) **재사용 컴포넌트 우선** — 신규 hub 가 기존 컴포넌트 재사용 시 신규 비즈 로직 ↓ / 시그니처 변경 0
  (d) **planner 통합 설계** — 3 PR 묶음 시 planner 1회 호출로 권장 순서 + 충돌 회피 + LOC 추정
  (e) **PM 직접 검증 모드 (옵션 B)** — 단순 PR + 회귀 위험 0 케이스 = tester+reviewer 생략 / 30~40분 → 15~20분 단축
- **재사용 가능 케이스**: 다른 도메인 대규모 UX 개선 시 동일 4 Phase 패턴 적용 (예: 라이브 페이지 / 사용자 프로필 / 시즌 hub 등)
- **참조횟수**: 0
...
- **상황**: 매치 218 OT 표시 사용자 보고. PM 이 LIVE UI 화면 (이미지 49 = q4=12, ot=0) 만 보고 "이미지 49 상태로 복원" 으로 T1 백필 결재 받아 UPDATE 진행. 사후 DB SELECT (`BEFORE`) 결과 **이미 DB 는 q4=9, ot=[3] 정확 박제** — 즉 사용자가 본 UI 가 진짜 데이터 ≠. T1 백필이 정확한 OT 데이터를 잘못 덮어쓴 destructive 작업 ← 사용자가 즉시 (a) 롤백 결정 → ROLLBACK UPDATE 로 회수. 데이터 손실 0 (1 turn 안에 round-trip 회수 성공).
- **교훈**:
  - **UI 화면 ≠ DB 실측 가능성 의심** — 특히 API 가 DB 를 무시하고 재계산하는 패턴 (route.ts L823 "PBP 가 진실 원천") 이 있는 도메인. 사용자가 보는 화면은 API 응답이 거치는 모든 변환 단계 끝
  - **백필 SQL 전 진단 3 면 의무**: (1) DB raw SELECT (해당 컬럼 본값) (2) API 응답 raw fetch (curl + 동일 데이터 path 검증) (3) 의존 도메인 raw SELECT (예: PBP / stats / settings 등 cross-table). 3 면이 일치하지 않으면 백필 ❌ 진단 우선
  - **단방향 백필 = 회수 비용 큰 안티패턴** — 가능하면 (a) DB 데이터 보존 + UI/API fix 로 우회 / (b) 백필이 불가피하면 BEFORE 풀 데이터 캡처 + 롤백 스크립트 사전 준비
  - **테스트 매치 한정 보호**: 매치 218 = 테스트 매치라 회수 자유 / 실제 운영 매치였으면 사용자 (운영자) 신뢰 손상 + 다른 매치 회귀 위험. **운영 매치 백필 결재 요청 시 위 3 면 사전 검증 결과 동봉 의무**
- **재발 방지**: `errors.md [2026-05-13]` (Phase 22) 참조. 차후 백필 SQL 결재 요청 시 = 본 lesson 의 3 면 검증 결과를 사용자 결재 요청 메시지 안에 미리 포함.
