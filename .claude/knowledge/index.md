# 프로젝트 지식 목차
> 최종 갱신: 2026-05-02 (듀얼토너먼트 풀 시스템 설계 — architecture +1 / decisions +1) | 이전: 2026-05-01 (profile PATCH 500 birth_date Invalid Date 가드 — errors +1 / lessons +1)
...
| errors.md | 23 | 2026-05-01 | 에러 패턴 (sticky, @page Hancom PDF, th/td 정렬, DB 사고, add 누락, next/image 외부 호스트, apiSuccess 미들웨어 7회 재발, 카페 상세 HTML 시간 소스 `.num_subject` 단일, 모바일 가로 overflow grid 안티패턴(04-29) / Avatar 영문 overflow(04-29) / schema 변경 후 dev 서버 미재시작 prisma Unknown argument(04-29) / organizations 목록 status 필터 'active' 값(05-01) / **profile PATCH 500 'Internal error' birth_date Invalid Date 미가드 — new Date(externalString) 패턴 isNaN 가드 필수 룰 신설(05-01)**) |
