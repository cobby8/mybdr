# 프로젝트 지식 목차
> 최종 갱신: 2026-05-03 (mergePlaceholderUser 표준 함수 — errors +1, conventions +1) | 이전: 2026-05-03 (PBP 미달 본질 원인 분석 — errors +1, lessons +1)

## 파일별 요약
| 파일 | 항목 수 | 최종 업데이트 | 설명 |
|------|--------|------------|------|
| architecture.md | 44 | 2026-05-03 | 페이지 구조, 대회/대진표, 팀명 2필드, Referee 시스템, Flutter API 호환, L3 다음 단위, 코트 대관 시스템 설계(2026-04-25), Phase 10-1 경기 평가/신고 시스템(2026-04-27), BDR v2 Hero 카로셀 + 글로벌 헤더 단일화 + 모바일 가드(2026-04-29), Dev/design/ 단일 폴더 룰 + BDR-current 동기화(2026-05-01), Phase 13 마이페이지 hub 박제(2026-05-01), D-6 EditProfile Hybrid 박제(2026-05-01), ProfileShell 폐기(2026-05-01), D-3 ProfileWeeklyReport Hybrid 박제(2026-05-01), Q1 Reviews + ContextReviews 박제(2026-05-01), 듀얼토너먼트 풀 시스템 설계(2026-05-02), **STL (Single Truth Layer) Phase 1 — 라이브 페이지 데이터 무결성 응답 가공 레이어(2026-05-02)** |
| conventions.md | 30 | 2026-05-03 | 디자인/색상/경기집계/sticky/프린트CSS/공식 기록 가드/에이전트 호출 기준/스크립트 템플릿 재사용/세션 분리 원칙/Tailwind v4 color-mix 언더스코어 문법(04-22)/any 예외 규칙 kakao·HOF·SW(04-22)/모바일 최적화 체크리스트 10항목(04-29) / 팀 로고 업로드 자동 정규화 pipeline(05-02) / **placeholder ↔ real user 통합 표준 함수 mergePlaceholderUser(05-03)** |
| decisions.md | 93 | 2026-05-02 | 기술 결정 (KBL 순위/대진표/userId 연결/Referee v2/헬스체크 cron/공식 기록 가드/카페 정규식 파서/운영 DB 직접 연결/카페 dataid tie-break / 공지 방어 가드 / 과거 글 시분 원천 미제공 확정 / Phase 3 #6 Pagination / L3 Organization 기존 라우트 활용 / EditionSwitcher 동작 규약 / 카페 3게시판 전면 board 강제 + parser 힌트 metadata화 / 세션 역할 재정의 / 코트 대관 court_managers N:M 보류 / 코트 대관 payments 다형성 / 모바일 720px 통일 / Hero 카로셀 외부 lib 0 / 카로셀 absolute opacity stacking / iOS 16px input 강제(2026-04-29) / ProfileShell 폐기 children passthrough(2026-05-01) / D-6 EditProfile Hybrid 옵션 B(2026-05-01) / D-3 ProfileWeeklyReport 옵션 B Hybrid(2026-05-01) / Q1 Reviews 옵션 A 비노출 / B 코트만 / C 동시 마이그(2026-05-01) / 듀얼토너먼트 schema 변경 0 + 5 Phase + 결정 6건(2026-05-02) / STL Phase 1 도입 — 옵션 1 응답 가공 채택 + R5 폐기(2026-05-02) / **Admin-Web 시각 통합 — alias 9개 보강 채택 (admin 코드 변경 0 / web 영향 0 / 다크모드 강제 ❌)(2026-05-02)**) |
| errors.md | 30 | 2026-05-03 | 에러 패턴 (sticky, @page Hancom PDF, th/td 정렬, DB 사고, add 누락, next/image 외부 호스트, apiSuccess 미들웨어 7회 재발, 카페 상세 HTML 시간 소스 `.num_subject` 단일, 모바일 가로 overflow grid 안티패턴(04-29) / Avatar 영문 overflow(04-29) / schema 변경 후 dev 서버 미재시작 prisma Unknown argument(04-29) / organizations 목록 status 필터 'active' 값(05-01) / profile PATCH 500 birth_date Invalid Date(05-01) / `/live/[id]` 셋업팀 명단 0명(05-02) / Flutter app 의 PBP 누락 패턴(05-02) / 폴드5 외부 분기 부재(05-02) / team_members.userId NOT NULL → placeholder User 강제(05-03) / PBP 미달 본질 Flutter 운영자 입력 누락(05-03) / **placeholder ↔ real user 통합 시 3종 UNIQUE 충돌 — DELETE→UPDATE + nickname + skipTm 분리(05-03)**) |
| lessons.md | 28 | 2026-05-03 | 교훈 (프린트 API, 모바일 zoom, 브랜치 drift, Flutter 테스트 오염, 팀 병합 logo, 동명이인, HTTP 5xx, API 미들웨어 재발 4회, 다음카페 정규식 파서 95%, 개발 DB라 믿은 .env가 운영 DB, parser 키워드보다 운영자 명시 신호(게시판)가 1순위, 점진 정비는 영역 단위로 묶어야 커밋 중복 비용 안 발생, 헤더 변경 라우트 그룹별 영향 범위(04-29) / Phase 9-Mobile 안티패턴 재발 → 컨벤션 문서화(04-29) / Hero 카로셀 1일→2시간 단축(04-29) / revert + 부분 hub 패치 = sub 페이지 옛 sidebar 잔재(05-01) / Vercel 로그 접근 불가 환경 prisma 500 진단(05-01) / PBP score_at_time 시계열 활용(05-02) / **출전시간 미달 분석 = 엔진 정확도 vs 데이터 부족 분리 + boundary/lineup 자동 측정 헬퍼 가치(05-03)**) |
...
- [05-03] architecture: **알기자 BDR NEWS Phase 2 통합 발행 시스템** — community_posts +4컬럼 (tournament_match_id/tournament_id/period_type/period_key, 무중단 ADD COLUMN) + 알기자 User uid=3350 + Phase 2 prompt(독립 기사 400~700자) + match-brief-generator mode 분기(phase1-section/phase2-match) + auto-publish-match-brief(매치 종료 fire-and-forget) + admin/news 검수 UI(Server Actions 4종) + LinkifyNewsBody(선수/팀 자동 링크). 8 결정 (community_posts 하이브리드 / 자동 트리거 / 검수 발행 / 다층 단위 / deep link / 통합 admin / placeholder User / 댓글+좋아요). 5/2 9 매치 backfill 9건 draft 생성. Phase E 노출 UI(`/news` 매거진 + deep link) 큐
- [05-03] errors: **NEXT_PUBLIC_APP_URL 을 server-side internal fetch 에 사용 시 dev → 운영 서버로 가는 사고** — auto-publish 가 `/api/live/[id]/brief` 내부 fetch 시 `process.env.NEXT_PUBLIC_APP_URL` 우선 → 운영 URL `https://www.mybdr.kr` 로 가서 missing_api_key 응답 (운영에 GEMINI_API_KEY 미설정). curl 직접 호출은 정상이라 미스테리. 회귀 방지 룰: NEXT_PUBLIC_* 는 client-side 용 / server internal fetch baseUrl = `process.env.VERCEL_URL ? "https://" + process.env.VERCEL_URL : "http://localhost:3001"` / 더 안전한 방법은 함수 직접 호출
- [05-03] architecture: **알기자 brief Phase 1 정책 재설계 + 데이터 풀 확장 + 다양한 관점 prompt** — `/live/[id]` 요약 탭 [Lead] 섹션 LLM. 점프볼 D리그 단신(독립 기사) 패턴 시도 → Phase 1 컨텍스트(매치 페이지 1 섹션) 재정의 (평균 350→188자). 데이터 풀 확장: 양 팀 통계 + 모든 선수 stat + 더블더블/트리플더블/리바·어시·스틸·블락·+/-·3점 1위 자동 검출. system prompt에 10개 관점 명시. 어조 ~다 통일. validate-brief 단순화 (점수 검증 제거, 길이 400→350). 8건 모두 다른 관점 작성 검증
- [05-03] lessons: **LLM prompt 는 노출 위치 컨텍스트 명시 필수 — 같은 데이터도 위치별 다른 prompt** — 점프볼 D리그 단신(독립 기사 600~1000자) 패턴을 매치 페이지 1 섹션에 적용 시 정보 중복 + 길이 폭증. 해결: prompt에 [배치 컨텍스트] 섹션 추가 ("이 글은 매치 페이지의 한 섹션이며, 다음 정보는 같은 페이지에 이미 표시된다: ..."). LLM이 중복 회피하며 흐름·영웅만 작성. 재사용 룰: 외부 매체 스타일 참고 시 매체 노출 형태와 우리 시스템 매칭 검증 / 같은 데이터 다중 매체 발행 시 매체별 prompt 분기. validate-brief 의 "점수/팀명 강제 등장" 룰이 LLM 에 "반복 압박" 부작용 — 정확성 검증과 표현 자유도 충돌 발견
- [05-03] architecture: **가입 신청 일괄 처리 함수 — `src/lib/teams/approve-join-requests.ts` 신규** — `team_join_requests` pending 일괄 처리 (박제 + 추후 admin UI 공용). 3 액션 (`approve_with_jersey` / `approve_no_jersey` / `reject`) + 트랜잭션 + 멱등성 + 중복 INSERT 차단 (team_members `(teamId,userId)` UNIQUE / tt_players `(tournamentTeamId,userId)` UNIQUE) + playerName fallback (명시→User.name→nickname→null). 5/3 슬로우 8건 첫 적용 (pending 8→0, tt_players 5→8). 다음 적용: SKD 7 / MI 8 / 블랙라벨 9 / 업템포·피벗·아울스·MZ 7. `merge-temp-member.ts` 와 동일 패턴 (prisma 임포트, $transaction). 호출측 영향 0
- [05-03] errors: **team_members.userId NOT NULL → 명단 INSERT 시 placeholder User CREATE 강제 패턴** — 미가입 선수를 team_members 등록하려면 userId FK NOT NULL 이라 placeholder User 신규 생성 강제. tt_players 만 INSERT 하면 출전 가능하나 팀 마스터 명단에서 안 보임. 표준 패턴 = 3단계 트랜잭션 (User CREATE email=`placeholder-{teamId}-{jersey}@bdr.placeholder` + passwordDigest 영구 차단 / TeamMember CREATE / tt_players CREATE auto_registered=true). 사전 검증 = email + (teamId,jerseyNumber) + (tournamentTeamId,jerseyNumber) 3 UNIQUE 충돌 SELECT. 사후 통합 = mergeTempMember (실제 가입 시 placeholder→real User ID 통합 7단계 트랜잭션, 4-30 김영훈 케이스). 운영 부채 누적: 5/2 78명 → 5/3 블랙라벨 +11 → 89명. scratchpad 큐 #3 카운트 outdated (5명 표기) 갱신
