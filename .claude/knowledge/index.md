# 프로젝트 지식 목차
> 최종 갱신: 2026-05-04 (알기자 기사 사진 첨부 시스템 Phase 1 MVP — news_photos 신규 테이블 + admin 모바일 카메라 + Hero/갤러리 노출 — architecture +1 / decisions +1 / conventions +1) | 이전: 2026-05-04 (알기자 후속 — community status 필터 사고 fix + LinkifyNewsBody 헬퍼 통합 — conventions +2 / errors +1)
...
| conventions.md | 35 | 2026-05-04 | 디자인/색상/경기집계/sticky/프린트CSS/공식 기록 가드/에이전트 호출 기준/스크립트 템플릿 재사용/세션 분리 원칙/Tailwind v4 color-mix 언더스코어 문법(04-22)/any 예외 규칙 kakao·HOF·SW(04-22)/모바일 최적화 체크리스트 10항목(04-29) / 팀 로고 업로드 자동 정규화 pipeline(05-02) / placeholder ↔ real user 통합 표준 함수 mergePlaceholderUser(05-03) / minutes-engine 가드 범위 — 양팀 union 기준 통일 5~12(05-03) / **community_posts 사용자 노출 query status 필터 의무 + 검수 카테고리 도입 시 4단계 점검 룰(05-04) / 알기자 본문 노출 위치 = LinkifyNewsBody 의무 사용 + buildLinkifyEntries 헬퍼(05-04)** |
| decisions.md | 98 | 2026-05-04 | 기술 결정 (KBL 순위/대진표/userId 연결/Referee v2/헬스체크 cron/공식 기록 가드/카페 정규식 파서/운영 DB 직접 연결/카페 dataid tie-break / 공지 방어 가드 / 과거 글 시분 원천 미제공 확정 / Phase 3 #6 Pagination / L3 Organization 기존 라우트 활용 / EditionSwitcher 동작 규약 / 카페 3게시판 전면 board 강제 + parser 힌트 metadata화 / 세션 역할 재정의 / 코트 대관 court_managers N:M 보류 / 코트 대관 payments 다형성 / 모바일 720px 통일 / Hero 카로셀 외부 lib 0 / 카로셀 absolute opacity stacking / iOS 16px input 강제(2026-04-29) / ProfileShell 폐기 children passthrough(2026-05-01) / D-6 EditProfile Hybrid 옵션 B(2026-05-01) / D-3 ProfileWeeklyReport 옵션 B Hybrid(2026-05-01) / Q1 Reviews 옵션 A 비노출 / B 코트만 / C 동시 마이그(2026-05-01) / 듀얼토너먼트 schema 변경 0 + 5 Phase + 결정 6건(2026-05-02) / STL Phase 1 도입(2026-05-02) / Admin-Web 시각 통합 — alias 9개 보강 채택(2026-05-02) / minutes-engine B 옵션 채택(2026-05-03) / 알기자 Phase 1 영구 저장 위치 = tournament_matches.summary_brief JSON 컬럼 옵션 B 채택(2026-05-04) / 알기자 트리거 통합 = Promise.allSettled 패턴 채택 (Phase 1+2 독립 병렬)(2026-05-04) / **도메인 sub-agent 시스템 옵션 A 채택 + 시범 live-expert 단독 시작 (8 도메인 / KPI 3종 / Phase 5단계 / 롤백 6종 / 리스크 8종)(2026-05-04)**) |
| errors.md | 31 | 2026-05-04 | 에러 패턴 (sticky, @page Hancom PDF, th/td 정렬, DB 사고, add 누락, next/image 외부 호스트, apiSuccess 미들웨어 7회 재발, 카페 상세 HTML 시간 소스 `.num_subject` 단일, 모바일 가로 overflow grid 안티패턴(04-29) / Avatar 영문 overflow(04-29) / schema 변경 후 dev 서버 미재시작 prisma Unknown argument(04-29) / organizations 목록 status 필터 'active' 값(05-01) / profile PATCH 500 birth_date Invalid Date(05-01) / `/live/[id]` 셋업팀 명단 0명(05-02) / Flutter app 의 PBP 누락 패턴(05-02) / 폴드5 외부 분기 부재(05-02) / team_members.userId NOT NULL → placeholder User 강제(05-03) / PBP 미달 본질 Flutter 운영자 입력 누락(05-03) / placeholder ↔ real user 통합 시 3종 UNIQUE 충돌(05-03) / minutes-engine endLineup chain 가드 범위 버그(05-03) / **🚨 community_posts.status 필터 부재 — 알기자 draft 7건 무단 노출 사고 (4파일)(05-04 첫 항목)**) |
| lessons.md | 30 | 2026-05-04 | 교훈 (프린트 API, 모바일 zoom, 브랜치 drift, Flutter 테스트 오염, 팀 병합 logo, 동명이인, HTTP 5xx, API 미들웨어 재발 4회, 다음카페 정규식 파서 95%, 개발 DB라 믿은 .env가 운영 DB, parser 키워드보다 운영자 명시 신호(게시판)가 1순위, 점진 정비는 영역 단위로 묶어야 커밋 중복 비용 안 발생, 헤더 변경 라우트 그룹별 영향 범위(04-29) / Phase 9-Mobile 안티패턴 재발 → 컨벤션 문서화(04-29) / Hero 카로셀 1일→2시간 단축(04-29) / revert + 부분 hub 패치 = sub 페이지 옛 sidebar 잔재(05-01) / Vercel 로그 접근 불가 환경 prisma 500 진단(05-01) / PBP score_at_time 시계열 활용(05-02) / 출전시간 미달 분석 = 엔진 정확도 vs 데이터 부족 분리(05-03) / raw 정확도 비교 시 알고리즘 버전 명시 필수(05-03) / **Supabase Storage bucket 생성 우회 — DATABASE_URL 권한으로 storage.buckets 직접 INSERT (Storage API key 부재 시)(05-04)**) |
...
## 최근 추가된 지식 (최근 10건)
- [05-04] architecture: **알기자 기사 사진 첨부 시스템 Phase 1 MVP — news_photos 테이블 + admin 모바일 카메라 + Hero/갤러리** — 사용자 의도 ("담당자 핸드폰 사진 업로드 → 인식 → 함께 게시"). 5 결정 모두 추천대로: Q1=A 수동 매핑(매치 dropdown) / Q2=A Supabase Storage news-photos bucket / Q3=A+B Hero+갤러리 / Q4=A admin/news 통합+모바일 카메라(`<input capture="environment">`) / Q5=B 발행 후도 추가 / Q6=B news_photos 별도 테이블. schema = news_photo 모델 신규 (match_id Cascade + url + storage_path + width/height + is_hero + display_order + uploaded_by + caption + exif_meta + 인덱스 3종). API = /api/web/upload/news-photo POST/DELETE (admin only + sharp 정규화 long-edge 1920+WebP 80%+EXIF 회전 자동 + isHero 트랜잭션). UI = NewsPhotoManager(admin client) + NewsPhotoHero/NewsPhotoGallery(server). 노출 3곳 (community/[id], news/match, admin 미리보기 큐). Phase 2 큐=EXIF 자동 매핑 / Phase 3 큐=AI Vision. 운영 액션=Supabase 콘솔 news-photos bucket 생성.
...
- [05-03] architecture: **minutes-engine v3 — 출전시간 계산 엔진 메인 path 4단계 구조 확정** — `src/lib/live/minutes-engine.ts` 5/3 D-day 7회 보강 후 안정. 메인 path: ① Q1 starter = `MatchPlayerStat.isStarter` (Flutter sync, DB 100% 입력) / ② Q2~ = endLineup chain / ③ Boundary 강제 (firstSegment=qLen, 다음 쿼터 PBP 시 endClock=0) / ④ LRM cap (종료 매치만, 양팀 합 = expected 정확). Fallback: `inferStartersFromPbp()` 헬퍼 (실측 발동 0회). 가드 union 5~12 일관 (L131+L196). 정확도 종료 100%/라이브 ~99%, test 21/21 PASS. calculateMinutes 본체 191→133줄 (-30%). commits: 7ea0174 / 678a875 / 133d0de / 72aa643 / d3984db
- [05-03] conventions: **minutes-engine 가드 범위 — 양팀 union 기준 통일 (5~12)** — DB starter union 가드 (L131) + endLineup chain 가드 (L196) 모두 양팀 union 기준 `>=5 && <=12`. 단일팀 기준 (3~7) 사용 금지. 알고리즘이 양팀 union 으로 시뮬 (Q1 endLineup 자연스럽게 size=10) → 단일팀 기준 사용 시 정상 매치도 가드 fail → fallback 강제 발동 → active size=3 segment 시뮬 → 양팀 합 -15~20% 손실. 회귀 방지 test 케이스 L 추가 (양팀 5+5 starter chain 통과). commit d3984db
- [05-03] decisions: **minutes-engine B 옵션 채택 — PBP 추정 fallback 격리 + 메인 path 명확화** — 5/3 D-day 동안 7회 보강 후 정리 옵션 3개 (A1 fallback 완전 제거 / A2 풀타임 안전 fallback / B 격리+명확화) 검토 후 B 채택. `inferStartersFromPbp()` 헬퍼 분리 (PBP-only fallback 전용) + 메인 path 4분기 if/else (Q1 DB / Q1 fallback / Q2+ chain / Q2+ fallback) + 알고리즘 설계 헤더 주석 +22줄. calculateMinutes 본체 191→133줄 (-30%). 사유: 시간 정확도 최우선, 안전망 유지 + 가독성 향상 균형. commit 72aa643
- [05-03] lessons: **raw 정확도 비교 시 알고리즘 버전 명시 필수 — 측정 함정** — minutes-engine 빠른 진화 (5/3 하루 5종 commit) 동안 같은 토너먼트도 측정 시점에 따라 raw 정확도가 12%p 까지 변동. PM 보고 t388=82.6% / 열혈=95.45% (+12.85%p) → 재측정 t388=94.52% / 열혈=95.36% (+0.84%p). 본질 차이는 qLen 효과 (~1.4%p) 만. 교훈: PM 보고 시 "as of [커밋 hash]" 명시 / 가설 검증 전 현재 코드 기준 재측정 / 토너먼트 비교 시 qLen 정규화 같이 보고. 재발 방지: 측정 스크립트에 git commit hash 자동 기록
- [05-03] errors: **minutes-engine endLineup chain 가드 범위 버그 — 양팀 합 -18% 손실** — `minutes-engine.ts` L196 endLineup chain 가드가 단일팀 기준 (3~7) 이었는데 알고리즘은 양팀 union (size=10) 으로 동작 → 정상 매치도 가드 fail → fallback `inferStartersFromPbp` 강제 발동 → Q2+ active set 3명만 잡힘 → 양팀 합 ~15~17% 손실. DB starter 가드 (L131) 는 이미 5~12 (양팀 union) 일관. 5/3 저녁 매치 #147 (SKD vs MI) Q2 home=82%/away=84% 발견. fix 1줄 변경 `>= 5 && <= 12`. 회귀 방지: test 케이스 L. commit d3984db
- [05-03] conventions: **Placeholder ↔ Real User 통합 표준 함수** — `src/lib/teams/merge-placeholder-user.ts` 의 `mergePlaceholderUser(phUid, realUid, opts?)` 신규 + `mergeTempMember` 내부 위임 (skipTmTransfer:true). 운영 backfill = `mergePlaceholderUser({realName})` 직접 호출 / 가입 hook = `mergeTempMember` 자동. 사전 검증 필수 (동명이인 시그널: real 이 같은 대회 다른 팀 ttp 활발). stat/PBP 자동 보존 (ttp.id 변경 0). 헬퍼 `isPlaceholderEmail` / `getMergedNicknamePattern`. vitest 10건 PASS. commit e029fac
- [05-03] errors: **placeholder ↔ real user 통합 시 3종 UNIQUE 충돌 — DELETE→UPDATE 순서 + nickname 변경 + skipTm 모드 분리 필수** — 누적 발견 (셋업 → 18건 → #4 강화). 3 UNIQUE = teamMember(team_id, jersey_number) / User(nickname) / teamMember(team_id, user_id). 우회: ph tm DELETE 먼저, ph nickname `_merged_{uid}`, 가입 hook 모드 (skipTmTransfer:true 시 ph DELETE + members_count.decrement → 호출자 increment 와 net 0). 표준 함수 `src/lib/teams/merge-placeholder-user.ts`. 회귀 위험 0 (트랜잭션 ROLLBACK)
- [05-03] errors: **PBP 미달 본질 = Flutter 운영자 입력 누락** — 5/2+5/3 종료 11매치 22팀 전수 분석 결과 lastClock 절단 21팀(95%) + 쿼터 전환 lineup 불일치 22팀 모두. PBP-only 엔진 적용 후 95~98% 미달의 본질 원인 = ① **starter lineup PBP 미입력** (1차, 22팀 모두 — 쿼터 시작 시 코트 5명 명시 INSERT 안 함 → everSeen ∩ ¬sub_in 추정만으로 1~4명만 잡힘) ② **lastClock 절단** (set 종료 지연, 5~113초 PBP 무이벤트, 21팀) ③ **firstClock 절단** (set 시작 지연, 10~125초, 1팀+OT 케이스 #140 240초). 코드 cap (옵션 C) 은 합 정확화 OK / 개별 sec 신뢰도 ~5%. Flutter app fix 권장 = quarter_start/quarter_end boundary PBP 자동 INSERT + starter lineup 자동 INSERT (원영 검토)
- [05-03] lessons: **출전시간 미달 분석 = "엔진 정확도" vs "데이터 부족" 분리** — 95~99% 미달 시 "엔진 fix" 보다 "데이터 부족" 가정 우선 (대량 샘플 분포로 명확). PBP 시계열 분석 헬퍼 (firstClock / lastClock / 쿼터 전환 lineup 3종 자동 측정) 재사용성 높음. cap (옵션 C) 같은 사후 보정은 합은 맞추되 개별 sec 분배 신뢰도 ↓ — UI/코드 양쪽 "추정값 (~5%)" 명시 검토 가치. 부수 발견: errors.md 동일 패턴 (5/2 quarterStatsJson 미달 = lastClock 절단) 이미 기록됨 — 한 번 발견된 운영 패턴 재발 → knowledge 누적 가치 확인
<!-- 05-03 (NEXT_PUBLIC_APP_URL / 알기자 brief Phase 1 / LLM prompt 위치 / 가입 신청 일괄 처리 / team_members.userId NOT NULL / 알기자 BDR NEWS Phase 2 통합 발행) 6건 절단 (10건 유지 — 5/4 도메인 sub-agent 옵션 A 채택 prepend) — 필요 시 git log로 복원 -->
