...
### [2026-06-14] Tournament Hard 삭제 cascade 의존성 그래프
- **분류**: architecture
- **발견자**: planner-architect
...
### [2026-06-21] 종별 마스터 관리 = `/admin/categories` 신규 라우트 (Track B Phase4)
- **분류**: architecture
- **발견자**: planner-architect + developer (Track B 첫 배치)
- **내용**: `admin_categories`(Prisma `AdminCategory`·BigInt PK·name @unique·divisions/ages Json·sortOrder @map sort_order) CRUD UI = **신규 라우트 `src/app/(admin)/admin/categories/`**(설정 하위탭 아님 — `/admin/settings/page.tsx`는 "use client"+server action 2카드뿐이라 server 데이터조회를 못 끼움). API = `src/app/api/web/admin/categories/route.ts`(GET 목록 sortOrder asc + POST) + `/[id]/route.ts`(PATCH 부분수정 + DELETE 하드삭제). 가드 getWebSession()+isSuperAdmin()→403. **삭제 참조무결성 가드 불필요** = AdminCategory를 FK로 참조하는 모델 0건(grep 0). **대회는 category를 문자열명으로만 보유**(FK 아님) → 종별 삭제해도 기존 대회 안 깨짐. UI=Toss(data-skin 루트div만·lucide·TagInput 칩·dirty판정 카드별 1 PATCH). 응답 snake(sort_order)/server prisma camel(sortOrder). 한글 IME 가드(isComposing) Enter commit 2핸들러 필수. 커밋 50053a2.
- **참조횟수**: 0
