...
### [2026-07-02] 단체 self-serve 운영 콘솔 — 진입경로·권한모델·백엔드경계 방침
- **분류**: decision (self-serve 콘솔 아키텍처 / 권한 스코프 / 신규 백엔드 처리 방침)
- **발견자**: planner-architect (단체 self-serve 콘솔 기획 + 시안 브리프)
- **계기**: 협회/연맹 운영자가 직접 로그인해 자기 단체(멤버·팀·회원·하위단체·대회·회비·공지)를 스스로 관리하는 self-serve 콘솔 요청. 요청 상당수가 현재 DB 부재 신규 개념.
- **결정①(진입경로)**: 권장 **`/org-console`**(파트너콘솔 `/partner-admin` 미러·글로벌 단일진입→내 org 자동선택·다중소속 스위처). 근거=self-serve "내 단체 콘솔" 정체성 명확·컷오버 진행중 `/v2/ta` 셸과 비얽힘. 대안=`/v2/org/[slug]`·기존 `/v2/ta/organizations` stub 확장(super/tournament_admin 스코프라 정체성 흐림). **최종=사용자 결정 대기**.
- **결정②(권한모델)**: partner-admin/layout.tsx 서버 게이트 미러 — `organization_members` role **owner/admin만 콘솔 운영**·member 진입불가·super_admin 전능 우회. 서버 컴포넌트 레이아웃 강제(클라 불신).
- **결정③(백엔드경계 방침)**: 신규 개념을 **3단계 분류**해 시안 범위와 후속 승인 경계 명확화. [기존 DB]=P1 실동작(단체정보·멤버·대회연결) / [무중단 ADD]=P2 컨셉(직책 position·하위단체 parent_org_id) / [신규 테이블·결제]=후속 컨셉(소속팀·가입신청·회비정산·공지·인증). **시안은 전부 제작하되 [무중단ADD]·[신규테이블]·[결제]는 박제·DB·PG 연동을 후속+별도 사용자 승인으로 분리**(06-29 브리프 §2-2 예외 처리 방식 계승). 회비·정산 결제(PG)=최대 승인 필요·큰 작업.
- **결정④(디자인 시스템)**: self-serve 콘솔=**admin-v2**(13룰/AppNav/Material Symbols 미적용). 파트너콘솔은 admin-toss지만 여기선 권한게이트 서버로직만 차용·디자인은 admin-v2로 통일(기존 `/v2` 계열 정합).
- **대안 기각**: (a) 신규 백엔드까지 이번에 설계·박제 — 결제/신규테이블 4종 대공사·운영 DB 단일환경 리스크로 기각(시안 선행·후속 분리). (b) member까지 콘솔 진입 허용 — self-serve 운영 콘솔 성격상 owner/admin 한정이 안전(열람은 공개사이트로).
- **후속**: 사용자 6개 결정(진입경로·P1 첫화면·하위단체 깊이·직책 방식·회비정산 범위·게이트 스코프) 확정 후 시안 의뢰 발주. 박제는 P1(기존 DB) 우선·나머지는 백엔드·컷오버 조율 후.
- **참조횟수**: 0
...
- **발견자**: pm (수빈 결재)
- **계기**: 슈퍼관리자(★) 8명 전원이 "역할" 칼럼에 구독등급(일반유저/대회관리자)으로 표시 → 권한과 구독 혼동. 근거=`Dev/permission-representation-audit-2026-06-13.md`(전수조사 A/B/C안).
- **결정**: **옵션 B(2축 분리)** — "역할" 칼럼→"구독 등급"(membershipType 로직 무변경, 헤더 라벨만) / "관리자" 칼럼→"운영 권한"(is_admin→"슈퍼관리자" err칩 / recorder_admin·association_admin info칩 / 없으면 —). **구독 tier3 라벨 "대회관리자"는 유지**(칼럼 분리로 혼동 자연 해소 — 라벨 변경 시 기존 운영 혼선). 대안 A(라벨 재명명)·C(통합 권한칼럼) 기각.
- **효과**: 슈퍼관리자가 "일반유저/대회관리자"로 안 보이고 "운영 권한" 칼럼에 "슈퍼관리자" 칩으로 정확 표시. +12/-6·DB0·표시로직만·tsc0. 부제 슈퍼관리자 상한 하드코딩 4→MAX_SUPER_ADMINS(10) 정정(§2-4).
- **역박제**: skip — 시안 AdminUsers.jsx는 권한을 상태컬럼 UserStatusBadge에 통합(구독/운영 별도 칼럼 부재) → 헤더 동기화 대상 없음(PR-RECORDER-AUDIT와 동일 사유).
- **참조횟수**: 0
