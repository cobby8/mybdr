...
### [2026-07-01] 공개웹 DS v4 구토큰 갭 교체 매핑 (PUB-2 시리즈 표준)
- **분류**: convention
- **발견자**: pm
- **내용**: `(web)` 공개 페이지(`[data-pub]` 스코프)의 구토큰→DS v4 정합 시 아래 매핑만 적용. **교체 대상(진짜 갭)**: `--color-text-disabled`→`--ink-dim`(미정의), `--color-navy`/`#1B3C87`(장식용만)→`--cafe-blue`, `--color-info`/`--info`/`#0079B9`→`--cafe-blue`(포인트색 realign), `--bg-elev-2`→`--bg-alt`(미정의), `--color-on-accent`/`--ink-on-accent`(미정의)→`--ink-on-brand`, **유색/브랜드/그라디언트/danger 배경 위** 흰 글자·아이콘 `#fff`/`#FFFFFF`→`--ink-on-brand`. **절대 유지(스왑 금지)**: `--color-text-primary`(=var(--ink))·`--color-text-secondary`(=var(--ink-soft)) — globals.css **L2784~2817 호환 별칭**으로 정상 resolve됨. **절대 제외(데이터/로직)**: 팀 브랜드 색상 저장/판정 hex(`!== "#fff"` 비교·폼 setter/placeholder `#FFFFFF`·`#1B3C87`·resolveAccent fallback), 흰 카드(surface) 배경용 `#fff`(→ `--bg-elev`), 장식용 흰 테두리, `#FF6B35`/순위뱃지 은동 hex/rgba 오버레이. **불변**: API/fetch/Prisma/로직 0변경·globals.css 미터치·미정의 토큰 도입 0(도입 토큰은 전부 globals L7181~7227 `[data-pub]` 정의 확인). 검증: `tsc --noEmit`+`next build`+잔존갭 grep 0. 적용 완료 라우트: courts/community/messages·noti·search/series·orgs/rankings/profile/guest-apps/teams (PR-PUB-2-3~2-10) + games (PR-PUB-3-1·c315208). calendar·saved·reviews·scrim + PUB-3 정적11p(about/help/privacy/terms/safety/referee-info/coaches/shop/gallery/awards/news)=이미 클린. **PUB-2+PUB-3 갭교체 트랙 종료(2026-07-01)**. 데이터 hex 제외 추가 사례: games report-form 팀A/B색(#DC2626·#0F5FCC)·step-settings 유니폼색(#FFFFFF select 옵션)·UniformChip background prop.
- **참조횟수**: 0
