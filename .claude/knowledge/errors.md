...
- **[2026-06-14 정정 + 근본원인 확정]**: 위 "자동 종료 cron 없음·수동 종료 추정"은 **오기**. 재검증 결과 **자동 종료 trigger 실재** = `auto-complete.ts` `checkAndAutoCompleteTournament`(2026-05-12 도입) — 모든 매치 종료 시 `finalizeMatchCompletion`(2026-05-16 통합헬퍼, admin PATCH/match-sync/match.ts/batch-sync 4 path 단일화)이 호출 → `finished(종료매치)===total(전체매치)`면 `tournament.status='completed'` 자동 UPDATE. cron은 없으나 "매치 종료 hook"형 자동 trigger. **status 오염은 수동이 아니라 이 자동 trigger 산물일 가능성 높음.**
- **[오분류 메커니즘]**: `total`은 format/결선 존재 무관 "현재 DB 박제 매치"만 셈. **full_league_knockout인데 결선 매치 0건이면 total=예선 → 예선 전부 완료 시 finished===total → 자동 종료**("예선=전체" 오판). `isLeagueComplete`(seeding.ts:547)도 `round_number IS NULL`(예선)만 보고 결선 0건이어도 true → **결선 미생성을 auto-complete에 알리는 신호가 시스템에 없음.** admin PATCH route 실행순서 = `finalizeMatchCompletion`(auto-complete 포함, route.ts:265) → **그 다음** auto-knockout-gen(:292, KO-3 silent catch). 자동 종료가 결선 생성보다 **먼저** 실행 → 결선 생성 성공해도 이미 종료 / silent catch 실패 시 결선0건+종료 확정. auto-knockout-gen은 **admin PATCH route에만 존재**(Flutter sync/score-sheet 경유 종료 시 결선 생성 시도조차 안 함). **분기점=결선 skeleton 유무**: skeleton 있으면(bracket/route.ts:197) 결선이 total에 잡혀 종료 안 됨 / 6/9 수동 INSERT 대진처럼 skeleton 우회·round_number NULL이면 예선만 total → 자동 종료(본 사고).
- **[방지 — KO-9 / Sprint 1]**: `checkAndAutoCompleteTournament`에 **결선 미생성 가드** — `format==='full_league_knockout'`인데 결선 매치(round_number IS NOT NULL) 0건이면 `updated:false reason:'knockout-not-generated'`로 자동 종료 차단(예선만으론 절대 종료 안 됨). ~12 LOC·회귀≈0(format 가드로 1라운드 elim·dual·일반리그 격리). 설계: `Dev/knockout-auto-schedule-plan-2026-06-14.md` §추가 이슈: 예선 종료 → 대회 종료 오분류.
