...
- **즉시 fix (5/10)**:
  ...
  2. **code**: `auto-register/[matchId]/route.ts` Step 9-1 추가 — cron `usedSet` 가드 백포트 (`tournamentMatch findMany WHERE youtube_video_id IS NOT NULL` → Set → `availableVideos` 필터링)
- **남은 위험 (후속 PR)**:
  - `score-match.ts` swap-aware 미적용 — cron v3 `extractTeamsFromTitle` 패턴 백포트 권장
  - 1팀 부분 매칭 + time 60 만으로 80 통과 케이스 — 다른 시간대 영상이 동시 publish 시 위험
- **재발 방지 룰**:
...
- **참조 파일**:
  - `src/app/api/web/match-stream/auto-register/[matchId]/route.ts` (Step 9-1 가드 추가)
  - `src/app/api/cron/youtube-auto-match/route.ts` (line 339~349 `usedSet` 원본 패턴)
  - `src/lib/youtube/score-match.ts` (swap-aware 미적용 — 후속 PR)
