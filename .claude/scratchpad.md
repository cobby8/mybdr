...
| 2026-04-30 | (미커밋, subin) | **폼 placeholder/helper 정리 8건** (developer): 대회 직전 사용성 §B + 사용자 직접 요청("불필요한 예시글과 설명 삭제"). 5 파일 5 영역 8건 일괄. (1) `/profile/edit/page.tsx` 기본정보 탭 5건: B-1 닉네임 sub "커뮤니티에 표시되는 이름" 제거 + placeholder "표시될 이름" → "2~20자" / B-2 "이름 (실명)" → "이름 *", sub "비공개 · 대회 등록 시 확인용" 제거 / B-3 휴대폰 sub "대회 연락용" 제거 / B-4 신장 placeholder "예: 180" → "180" / B-5 "체중 (kg)" → "체중", sub "비공개" 제거, "예: 75" → "75". (2) `teams/new/_v2/step-basic.tsx` B-6 placeholder "예: REDEEM, 3POINT, 몽키즈" → "REDEEM" + small "한글·영문 2~20자" → "2~20자". (3) `community/new/page.tsx` B-7 placeholder "제목을 입력하세요 (최대 80자)" → "제목" (maxLength={80} 보존). (4) `referee/signup/page.tsx` B-8 placeholder "닉네임 (2~20자)" → "닉네임" (minLength/maxLength HTML 속성 보존). 인계 문서가 명시한 `profile/complete/page.tsx:337` 은 v2.2 박제 후 닉네임 input 자체가 제거됨(포지션/키/지역/사진 4단계로 변경) — 닉네임 placeholder 매칭 위치는 referee/signup 뿐. **연락처 탭(L788/L808 휴대폰/이름) 인계 미명시로 보존** — 별도 보고. profile/edit 닉네임 중복확인 기능과 동시 작업 충돌 회피(텍스트만 변경). 검증 룰 (maxLength/minLength/pattern/min/max/type) 100% 보존, alert 신규 0건. 4 파일. tsc 0 에러. | ✅ |
