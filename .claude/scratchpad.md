...
| 날짜 | 커밋 | 작업 요약 | 결과 |
|------|------|---------|------|
| 2026-05-09 | subin (미커밋) | **알기자 fire-and-forget abort fix — waitUntil 적용** (services/match.ts 2곳: line 269 updateMatch / line 478 updateMatchStatus). Vercel serverless 응답 종료 후 background Promise abort → news_publish_attempts 0건 / summary_brief NULL 회귀. `@vercel/functions` 신규 설치 + `triggerMatchBriefPublishAsync` → `waitUntil(triggerMatchBriefPublish(matchId))` 치환. tsc 0 에러. dev/local 영향 0 (waitUntil = 단순 Promise) / API 응답 시간 변경 0 / DB schema 0 / Flutter v1 0 / admin-news.ts 직접 호출 영향 0. | ⏳ 커밋 대기 |
