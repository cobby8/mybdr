...
| 날짜 | 커밋 | 작업 요약 | 결과 |
|------|------|---------|------|
| 2026-05-06 | (PM 커밋 대기) | **apiError 인자 순서 일괄 fix + transfer-request-modal endpoint URL fix** — 10 파일 수정: 9 API 파일 (teams/[id]/requests, teams/[id]/requests/[requestId], transfer-requests, transfer-requests/[requestId], teams/[id]/transfer-requests, teams/[id]/officer-permissions, teams/[id]/ghost-candidates, members/[memberId]/force-action, members/[memberId]/permanent-delete) 의 apiError("CODE", status, msg) → apiError(msg, status, "CODE") 모든 호출 패턴 정상화 + 메시지 누락 케이스 한국어 추가 (INVALID_TEAM_ID/INVALID_JSON/FORBIDDEN/NOT_FOUND 등 모두). transfer-request-modal endpoint /api/web/teams/search → /api/web/teams (search 전용 endpoint 미존재 → 500 fix). 사용자가 보고한 휴면/번호변경/탈퇴 신청 시 "ALREADY_PENDING" 영문 노출 → 한국어 메시지 표시 정상화. tsc 0 errors / schema 변경 0 / 기능 변경 0 / 운영 영향 0 / commit X. PM 후보 12 파일 중 jerseys-in-use·jersey-override·admin-check 3 파일 미존재 (작업 X). | ✅ |
