...
| 2026-05-02 | (진단 SELECT only) | **5/2 매치 132~136 minutesPlayed 정합성 검증** — minutesPlayed=초 단위 + Flutter 999초 hard cap. qsJson 합 양팀 각 140m 정확(매치 132/133/135), 매치 134만 132.1m (-7.9m) 약 미달. 매치 136 진행중(Q1 종료 후). 양팀 합 기대=280 player-min(7m×4Q×5명×2팀). 보정 권장 0건 — 라이브 API가 qsJson 우선이라 화면 표시 정확. 운영 룰: minutesPlayed cap 폐지 검토(원영 전달 큐) | ✅ |
...
| 2026-05-02 | (정밀 진단 SELECT only) | **매치 #133/#134 출전시간 미달 원인 추적 — Q3/Q4 last_clock 조기 절단** — PBP 시계열 분석으로 미달 원인 100% 확정: qsJson 의 쿼터별 합 미달 = PBP last_clock 시점부터 0초까지 lineup 시간 누락. 매치 #134 Q4 last_clock=113s (-330s/Q ↔ qsJson Q4 1770s/팀 정확 일치) / 매치 #134 Q3 last_clock=29s (-145s × 2 = 290s 추가 미달, 근접) / 매치 #133 Q3 last_clock=31s (-40s 근접). 정상 매치(#132/#135)는 Q4 last_clock=11s/0s/64s 더 끝점 가까움. **원인 = Flutter app 의 quarterStatsJson 산출 = 마지막 PBP clock 까지의 lineup 시간만 누적**. 이후 시간(예: #134 Q4 113초/1:53)은 PBP 미생성 → qsJson 미반영. **버그 아님**: 운영자가 Flutter app 에서 쿼터 종료 0초 직전 사이드라인 액션 미입력 (점수 격차로 의미 없는 추가 시간 / 경기 조기 종료 / undo 후 재입력 누락 등 8 시나리오 중 3·5·7). matchPlayerStat(=Flutter sync) 에는 정확히 반영되지만 qsJson 합 = 8400 기대값 미달. 보정 옵션: (1) DB 그대로 — 표시값은 lineup 의 실제 코트 시간이라 의미 일관 (2) STL R8/R3 패턴으로 last_clock+0~last_clock 시간 균등 분배 (5명 lineup 마지막 알면) — 별도 작업 필요. **사용자 결정 필요** | ✅ |
| 2026-05-02 | (가설 검증 SELECT only) | **사용자 가설 100% 정합 — sub_in/out 기반 직접 계산이 정답** — Flutter app 이 substitution PBP 를 풍부하게 보냄 (#134 = 29건, Q1:4/Q2:7/Q3:7/Q4:11). **단 전용 컬럼 sub_in_player_id/sub_out_player_id 미사용**, 대신 `action_type='substitution'` + `action_subtype='in:<ttpId>,out:<ttpId>'` 문자열 인코딩. 파싱 후 starter (OUT 으로 빠진 + IN 으로 들어온 적 없는) 추정 + lineup 추적 + 쿼터 종료 시 코트 5명 0s 까지 누적 = **이론합 274.95m / 기대 280m (98.2%)**. DB.minutesPlayed 합 218.18m 보다 **+56m 추가 회수**. 999s cap 걸린 8명이 실제로는 1107~1680s (18.5~28m), Flutter 999 cap 폐지 + 우리가 직접 sub-based 계산 시 정확값 가능. **새 우선 권고**: API/대시보드에서 quarterStatsJson 무시 + PBP substitution 파싱 → 실시간 계산. 단기 (D-day 잔여): 표시 그대로 (오차 의미 작음). 장기: `src/app/api/live/[id]/route.ts` minutesPlayed 산출 로직을 sub-based 로 교체 + Flutter 쪽 999 cap 제거 요청 (원영 큐). | ✅ |
