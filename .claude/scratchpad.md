...
| 2026-04-30 | (미커밋, subin) | **프로필 사진 업로드 (Vercel Blob)** (developer): @vercel/blob 패키지 추가 + 신규 API 2 (POST upload-image: multipart/2MB/jpeg·png·webp + put/del + DB 갱신, DELETE delete-image: blob del + null) + /profile/edit 사진 탭 활성화 (96px 아바타 Image 분기 + 숨김 file input + ref 트리거 + 1.5초 reload + 인라인 에러). next.config remotePatterns `*.public.blob.vercel-storage.com` 추가. .env.example BLOB_READ_WRITE_TOKEN 가이드. 4 파일. tsc 0. **운영 액션 필요: Vercel 대시보드 Blob 생성 + BLOB_READ_WRITE_TOKEN 환경변수 등록** (운영/프리뷰/dev). | ✅ |
