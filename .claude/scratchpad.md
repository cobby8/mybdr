...
| 날짜 | 담당 | 작업 | 결과 |
|------|------|------|------|
| 04-27 | developer | **Phase 9 P1-5 묶음 6 — /search 시안 v2(1) 박제 (사이드 필터 5종 신규)** — 1파일 수정(`src/app/(web)/search/_components/search-client.tsx` 1025→1449줄, +424). 시안 SearchResults.jsx 117~169줄 사이드 필터 박제 + 본문은 Phase 8 구조 유지. **레이아웃**: maxWidth 900→1180 + `display:grid; gridTemplateColumns:"240px minmax(0,1fr)"; gap:20; alignItems:flex-start` (탭/헤더 영역은 기존 width 그대로, 결과 영역만 grid). **사이드 aside**(sticky top:120 .card padding:18) — 필터 헤더 + 초기화 버튼(setFilters(INITIAL_FILTERS)). **신규 state**: `filters: FilterState{area:string[], level:string[], dateRange:"any|today|week|month|weekend", freeOnly:boolean, openOnly:boolean}` + `INITIAL_FILTERS` 상수(openOnly=false 시작 — 시안은 true지만 DB 매핑상 결과 0건 방지). **filter→props 매핑 5종**: ①**area** (실 동작) — 시안 8개(강남/서초/송파/성동/용산/중구/마포/영등포) toggleArrayFilter("area",v) 다중 선택. matchesArea(candidates: (string|null)[], selected) — `candidates.filter(Boolean).join(" ").includes(a)` 로 부분 매칭. games:[venue_name,city] / tournaments:[city] / teams:[city] / courts:[city,district,address] / users:[city]. ②**level** (DB 미지원) — 4개(초보/중급/상급/선출급) UI만 + opacity:.5 + disabled + "준비 중" 라벨. ③**dateRange** (실 동작) — 5종 radio + matchesDateRange(iso, range): today=오늘 자정~+24h / week=오늘~+7일 / month=오늘~+30일 / weekend=getDay()===0||6. games.scheduled_at + tournaments.start_date 적용. teams/courts/users/posts는 영향 없음. ④**freeOnly** (DB 미지원) — checkbox disabled + opacity:.5 + "(준비 중)" — 직렬화된 props에 fee/price 필드 없음. ⑤**openOnly** (실 동작) — tournaments만 영향. OPEN_STATUS_KEYS={registration,active,open,upcoming} 만 통과. **본문 데이터**: `filteredGames/Tournaments/Teams/Courts/Users` useMemo + filteredPosts=posts(필터 영향 0). 모든 섹션 `teams.length`→`filteredTeams.length` 등 변수 교체. counts/activeTabCount도 filtered 기준 → 탭 뱃지 숫자가 필터 적용 결과 반영. **보존 100%**: page.tsx(server) 0 변경, props 시그니처(q + 6 배열) 그대로, form submit + router.push 그대로, searchParams 처리 그대로, 7탭 구조 그대로, 하드코딩 색상 0건(모든 var(--*)), Material Symbols 유지, lucide-react 미사용. **scratchpad 백로그**: level/freeOnly DB 미지원 — Phase 9 P1-5 묶음 6 백로그로 추후 schema 확장 시 활성화. tsc --noEmit EXIT=0 PASS | ✅ (커밋 금지 — PM 처리) |
| 04-27 | developer | **Phase 9 P1-5 묶음 3 — 프로필 4 페이지 시안 v2(1) 박제 (billing/edit/activity/notification-settings)** — 3파일 수정 + 1파일 skip. ①`profile/billing/page.tsx` (~440→~870줄). 시안 Billing.jsx 박제: page max-width 880 + 빵부스러기(프로필 › 결제·구독) + eyebrow "결제 · BILLING" + h1 "결제·구독 관리". 구독 섹션: 카드 그라데이션 헤더(linear-gradient(135deg, --cafe-blue, --cafe-blue-deep)) 추가 — "현재 구독" eyebrow + plan name(ff-display 30px 900) + 가격/주기 + 우측 ACTIVE/CANCELLED 상태 dot+라벨 + "다음 결제·날짜". 메타 그리드(140px 1fr) — 요금/시작일/만료일/혜택. 액션 행(border-top) — 구독 해지(margin-left:auto, danger). 업그레이드 프롬프트 카드(bg-alt + borderLeft 3px accent) — "TEAM PRO 플랜으로". 결제 내역: h2 "결제 내역" 추가. board 4열 그대로 유지(기존 P0-4-B). ②`profile/activity/page.tsx` (544→700줄). 시안 MyActivity.jsx 박제: page + 빵부스러기(프로필 › 내 활동) + eyebrow "내 활동 · MY ACTIVITY" + h1 "신청한 모든 것" + sub. **counters 4종 그리드**(repeat(4, 1fr)) — 검토중(warn) / 승인·확정(cafe-blue) / 거절(ink-mute) / 취소(ink-dim), 각 카드 borderTop 3px tone. 카운트는 cache 3 도메인을 합쳐 status별 환산(games:0=pending,1=approved,2=rejected / tournaments:string / teams:string). filter chips → 기존 3탭 유지하되 chip 톤(active=cafe-blue bg+#fff) + Material symbol 아이콘 + count mono. 본문: 카드 컨테이너(.card padding:0) 안에 borderBottom 행 + **borderLeft 3px tone 색상**(success=ok / error=danger / warn=warn / muted=ink-mute). GameCard/TournamentCard/TeamCard 4열 grid(40px 1fr auto) — 좌측 Material 아이콘(sports_basketball/emoji_events/groups) + StatusBadge + 시간 mono + 제목 + sub + 우측 액션(team pending시 "신청 취소"). 하단 빠른 이동 3버튼(경기/대회/팀 찾기). API 흐름 0 변경(/api/web/me/activity x3 병렬 + cancelTeamJoin DELETE). ③`profile/edit/page.tsx` (600→1000줄, 전면 재작성). 시안 EditProfile.jsx 5탭 → **6탭** 박제(시안 5개 + 우리 추가 "환불 계좌"): basic/skill/contact/refund/photo/privacy. 좌측 220px 사이드 네비(sticky top:120 + borderLeft 3px accent active) + 우측 카드 본문(28px 32px). 헤더: eyebrow "EDIT PROFILE · 프로필 편집" + h1 + 우측 액션(✓ 저장됨 2초 트랜지션 / 취소(Link) / 저장 btn--primary). **API/state 100% 보존**(GET /api/web/profile / PATCH save / POST generate-bio / DELETE withdraw / RegionPicker / BANKS / SOCIAL_PROVIDERS). photo 탭=시안 박제 + 모든 버튼 disabled + "곧 제공됩니다" 안내. privacy 탭=시안 헤더 + "세부 공개 범위 설정은 곧 제공됩니다" 안내 + 소셜 연동 카드(provider 있을 때만) + 회원 탈퇴(Danger Zone) 통합. 모바일 styled-jsx로 사이드 → 가로 스크롤 탭 변환. 회원 탈퇴 모달 100% 보존. ④`profile/notification-settings/page.tsx` — **skip**. 이 파일은 redirect("/profile/settings?tab=notifications") 1줄 페이지(M1 Day 8 통합). 실제 알림 설정은 `_components_v2/notify-section-v2.tsx`에서 이미 v2 박제 완료 → 변경 불필요. 시안 NotificationSettings.jsx는 settings 허브에서 추후 재검토 권장. 하드코딩 색상 0건 / 모든 토큰 var(--*) / Material Symbols 유지 / lucide-react 미사용. tsc --noEmit EXIT=0 PASS. | ✅ (커밋 금지 — PM 처리) |
...
| 04-27 | (older) | (이전 항목들 — 작업 로그 10건 유지를 위해 절단) | — |

<!-- 절단 시점 마커: 아래 항목들은 새 로그에 의해 교체되었습니다.
...
| 04-27 | (older) | (Phase 6 Pricing 1차 수정 등 이전 항목들 — 작업 로그 10건 유지를 위해 절단) | — |
