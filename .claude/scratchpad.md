...
## 현재 작업
- **요청**: 관리자 영역 Toss 시안 박제 (admin-toss v2.41 정본). 단계 PR(PR-0~PR-5). 기준 패키지=`Dev/design/BDR v2.41-admin-toss/` + 계약문서 `_PR0-CONTRACT-CONFIRMED.md`
- **상태**: **PR-0 완료(계약 확정·코드0)**. 다음 = PR-1 공유 기반(toss.css→components/admin-toss→admin-shell). 단독 운영(dev 직접·subin 폐지). dev→main 머지=수빈 단독.
- **PR-0 산출(2026-06-27)**: §0 정본 패키지 배치(구버전 핸드오프 2개 _archive 격리) / §1 치환표(START-HERE·screenshots 부재→README+HANDOFF+deadbutton-audit, 정합=preview HTML 8+진입 HTML 9) / §5 스키마 실측 9건(★전면신규=#9 tournament_expense 1건뿐, #5 teams=기존 바인딩만) / §6 결정(6-1 보존단일화·6-2 admin/tournaments 목록 완전제거).
- **⚠️ 직전 작업**: v2.40 Admin Console A0~A5 완료(아래 진행표 — 별건, 종료).
...
| 날짜 | 작업 | 결과 |
|------|------|------|
| 2026-06-27 | **2026 BDR 유스챌린지 2차 — 대회생성+순위전 자동배정+선수입력 (운영 DB·생성/UPDATE)** | ✅ tournamentId=`2b93e9bf-4b78-47a4-aed3-9bd1e2153344`. **①생성**: Tournament1(in_progress·divisions[U11,U12]·스포라운드·apiToken64) + DivisionRule2(U11 round_robin·U12 group_stage_with_ranking) + 신규Team6(captain2933) + TournamentTeam10(U11=4·U12 A3/B3) + Match15(U11 6/27 6경기·U12 6/28 예선6+순위전3). **②순위전 자동배정 보정**(강남구 표준 맞춤): TournamentTeam category SET(=division_code, standings 필터 필수)·groupName "A조"→"A"·예선 round_number→null(advancer 가드 진입)·순위전3 settings.home/awaySlotLabel("A조 N위")+notes+roundName "순위결정전 N". → 예선6 완료 시 `finalizeMatchCompletion`→`advanceDivisionPlaceholders`가 순위(승-득실차) 기준 home/away 자동박제(home/away NULL 유지=실팀 미박제 사고방지). getDivisionStandings("U12")=6팀·advancer 0/6 인식 검증. **③선수입력**: U11 4팀 33명(어핑10·김포훕스타6·동탄KCC7·스티즈남양주10) TournamentTeamPlayer(auto_registered·userId NULL·등번호 unique·스티즈남양주는 grade 학년 박제·조하준 동명이인 2명 학년/등번호 구분). **+팀명** Team283 업핑→어핑(강남구협회장배 양쪽 반영, 사용자 확인). **결정**: organizer=수빈2836·scheduledAt=UTC(KST-9h)·매치 division=settings.division_code·스티즈강남 신규·스티즈남양주 U11=257/U12=신규. 임시스크립트 정리완료. 코드변경0(커밋 대상 없음). |
