...
- **상태**: 🔨 Phase 1 (AdminShell Toss 사이드바 + 5화면 리스킨). Phase0✅dev머지#725(bf54851). subin=dev
- **현재 담당**: pm → developer(Phase 1)
- **🔑 opt-in 아키텍처**: data-skin="toss"를 셸 전체에 붙이면 미리스킨(Phase2) 화면도 깨짐 → **셸 크롬(사이드바/헤더)+리스킨된 페이지 루트에만** opt-in. 미리스킨 화면=기존 스타일 유지
- **🚨 디자인 분기(절대)**: 관리자(admin/*·tournament-admin/*·partner-admin/*·referee/*)=**Toss**(toss.css·라이트·lucide-react·#3182F6·24px). 사용자 프론트 `(web)/*`=**mybdr 기존 절대불변**(var토큰·다크·Material·AppNav). 참가신청서만 사용자용인데 Toss(예외). 네임스페이스 분리(data-skin="toss"/.admin-shell). CDN lucide injection 금지→lucide-react.
- **2트랙**: A 리스킨(대회관리 외 admin 전부 — 기능/컬럼/필터/문구/라우트 1:1 유지·비주얼만) / B 리빌딩(대회관리만 — admin_categories·참가신청 로스터조인·DivisionRule)
- **Phase**: 0 디자인시스템 / 1 AdminShell+5화면 리스킨 / 2 나머지 백오피스 / 3 partner+referee / 4 대회관리 리빌딩
- **🚦 게이트(트랙B/P0)**: admin_categories·조인테이블 db push 전 승인. 트랙A는 schema 0
- **§8 미결(Phase3/4 진입 전 확인)**: ①출전 최소인원/게스트(토글 off) ②4종 시드 정확성 ③~~partner/referee 범위~~ ✅확정(Phase3 큐, 아래) ④참가신청 라우트 위치
- **📌 Phase 3 큐(보류해제, 트랙A 리스킨)**: 지시서=`관리자Toss전환-Phase3추가지시서`. 시안=`_handoff-admin-toss-P0/design-files/`(PartnerReferee.html·pr-screens.jsx·pr-data.jsx). 대표 6화면(파트너 PartnerHome/Venue/Campaigns·심판 RefHome/Assignments/Settlements)=패턴 레퍼런스 → **partner-admin 전체4 + referee 전체26** 동일 키트 리스킨. 자체 셸(Partner/Referee/referee-admin) 골격·기능·컬럼·문구·라우트 1:1 유지·비주얼만. mock(pr-data) 참고·실제 우선. schema0(게이트 불필요). PR 분리(partner / referee회원 / referee-admin). 라우트: partner-admin(`/partner-admin`·`/venue`·`/campaigns`(+`/[id]`)) / referee 회원(`/referee`·`/assignments`·`/settlements`·`/applications`·`/certificates`(+`/[id]`)·`/documents`·`/notifications`·`/profile`(+`/edit`)) / referee-admin(`/referee/admin`+assignments·members·settlements·announcements·pools·bulk-register·bulk-verify·fee-settings·settings)
- **직전완료**: 매칭 M1~M6 운영(main) 반영 완료 #717~#724
...
### 관리자 Toss Phase 1 — AdminShell 셸 크롬 + 5화면 리스킨 (2026-06-21)

📊 종합 판정: **통과 (차단 0)** — 권장 0 · 후속 2

✅ 잘된 점:
- **토큰 리매핑 정확성 + 누수 0 (최우선 — 정밀 검증)**: toss-admin.css가 오버라이드하는 베이스 토큰명 전부 globals.css 실재(`--bg-card/--bg-elev/--bg-head/--ink-on-brand/--cafe-blue-hair/--border-hard/--sh-xs/--accent-soft/--ok-soft/--warn-soft` grep 일치). admin.css가 `--color-surface:var(--bg-elev)`·`--color-card:var(--bg-card)`·`--color-accent-light:var(--accent-soft)` 식 별칭 → 베이스만 `[data-skin]` 스코프서 덮으면 자동 전파. globals.css/admin.css git diff **0**. 전부 `[data-skin="toss"]` 스코프(bare :root 0).
- **opt-in 스코핑 정확(Phase2 누수 차단)**: data-skin = (a)셸 크롬(sidebar aside·mobile 토글/오버레이/드로어·`.admin-topbar`) (b)5페이지 루트 div뿐. admin-shell.tsx 공유 `.admin-shell`/`.admin-main`/`.admin-main__inner` **미부착(diff 확인)** → Phase2 미스킨 자식은 별도 라우트=별도 page라 부착 div 자식 진입 0. topbar는 슬롯만 받음. (web) 프론트 git diff **0**.
- **라이트 고정 + 라운딩 타이 해소(소스 순서)**: 색은 `[data-skin]`+`[data-theme="dark"] [data-skin]`(0,2,0 우선) 라이트 페어로 다크 누수 0. admin.css 다크 라운딩 중 override 페어 없는 2건(`.admin-table-wrap{radius:0}`·`.admin-stat-pill{radius:2px}`, 0,2,0)은 toss override와 **명세도 타이** → layout.tsx import `globals→admin→toss-admin`(toss 최후) → 소스 순서로 **toss 승리** → 라운딩 정상. ThemeSwitch 유지 적절.
- **트랙A 기능 보존 0 회귀**: 셸 3 + 5페이지 + 4 content/table diff에서 비아이콘 로직 변경 **0건**(grep 빈 출력). 쿼리/server action props/필터/정렬/모달/페이지네이션/isSuperAdmin/라우팅/ThemeSwitch 불변. sidebar `toLucide()`는 navStructure 무변경+렌더시점 변환만. mobile-nav ESC/외부클릭/스크롤잠금/props 불변.
- **디자인 13룰 준수**: 인라인 style 전부 `var(--color-*)`(organizations 버튼 `--color-error/--color-success`). 하드코딩 hex는 toss-admin.css **토큰 정의부 값**(소비처 0). 실사용 lucide(shield/bell-ring/triangle-alert/user-plus)+셸 36종 전부 실존(node 실측, volleyball 대체 적정). AppNav 무관. `tsc --noEmit` exit 0.
- Material 잔존 4건은 `tournaments/[id]/audit-log`·`transfer-organizer` = **Phase1 범위 밖**(트랙B/미스킨, data-skin 미부착이라 기존 Material 유지가 정상 — 누수 0).

🟢 후속:
1. **admin.css dead 규칙(무해)**: `.admin-aside__link .material-symbols-outlined{}` 폰트사이즈 규칙은 lucide(size prop) 전환으로 무효. 잔여 정리 여지.
2. **ThemeSwitch 잔존**: Toss 영역 라이트 고정이라 무효과. developer 기록대로 후속 정리 예정(트랙A 보존 원칙상 현 제거 0 적절).

⚠️ reviewer 참고:
- **라운딩 타이 = import 순서 의존**: table-wrap/stat-pill 다크 라운딩은 명세도 동일 → toss-admin.css를 admin.css보다 먼저 import하면 라운딩 회귀. layout.tsx import 순서가 계약(차단 아님).
- 범위 = tournaments 목록/상태 page+content만. `[id]` 하위 Material 4건 정상.

### Phase 3 PR-A — partner-admin 재스킨 (2026-06-21)

📊 종합 판정: **통과 (차단 0)** — 권장 0 · 후속 2

✅ 잘된 점:
- **트랙A 기능 보존 0 회귀(최우선 — 정밀 diff 검증)**: diff 삭제측에 fetch/method/`/api/web/partner`/BigInt/isSuperAdmin/partner_members/redirect/JSON.stringify/filter분기/isEditable 등 **server logic 삭제 0건**. 삭제된 건 인라인 색상 style + Material 아이콘명(dashboard/campaign/stadium)뿐. 추가측 fetch/api/method/body 변경 0(잡힌 2줄=기존 form 바인딩 그대로). navItems **href 3종 불변**(아이콘명만 lucide로 교체). layout 권한가드(super_admin 분기·partner_members findFirst·redirect) / SWR 키(partner/me·stats·campaigns) / POST·PATCH body(court_info_id·rental_available·placement) / filterTabs 5종 / statusConfig 6분기 / VP2_TABS 2탭 / isEditable 3상태 전부 보존.
- **디자인 13룰(Toss) 준수**: partner 5파일 Material Symbols 잔존 **0**(grep 0건), 하드코딩 hex **0**(grep 0건, 인라인 style 전부 `var(--*)` 토큰), lucide-react 직접 import **0**(전부 키트 `<Icon>` 경유). 사용 lucide 아이콘명 **17/17 실존**(store/layout-dashboard/megaphone/building/clock/plus/share-2/eye/mouse-pointer-click/trending-up/layout-grid/id-card/circle-dot/arrow-left/pencil/x/check — node lucide-react 런타임 조회 OK). #3182F6 primary는 토큰(`--primary`) 경유.
- **사용 .ts-* 클래스 전부 실재 + 신규 규칙 0**: ts-navlink/ts-segment(__btn)/ts-chip/ts-card(--flat,--tight)/ts-field(__label)/ts-input/ts-select/ts-badge(--warn,--grey)/ts-btn 전부 toss-admin.css([data-skin] 스코프)에 정의됨. partner가 쓰는 토큰(`--card`·`--primary`·`--grey-50/100`·`--ink-mute/dim`·`--ok`·`--warn`·`--bg`·`--border`) 전부 Phase0 정의분에 실재 — undefined 토큰 0.
- **opt-in 격리 정확**: data-skin="toss"가 셸 루트 + 각 페이지 루트 + [id] 로딩 early-return div에 부착 → bare `var(--ink)` 등 토큰 활성화 누락 0. 부착처가 partner 영역에만 한정 → `(web)/*` 일반 프론트 git diff 0(누수 0).
- **키트 미수정(admin-toss/ 무변경)**: H/StatCard는 로컬/키트재사용. StatCard 시그니처 `{icon,label,value,delta,trend}` — partner는 icon/label/value만 전달(옵셔널 미사용) 정합. kit.tsx·data.tsx·index.ts diff 0(git status에 admin-toss/ 미포함).
- **tsc --noEmit exit 0**.

🟢 후속 (차단/권장 아님 — 별도 결정):
1. **toss-admin.css 144줄 추가는 PR-A 산출물 아님(타 에이전트 Phase1 셸 작업 — 리뷰 범위 외)**: git status에 toss-admin.css M 있으나 diff 내용=「PHASE 1 토큰 리매핑」(globals 베이스 토큰 `--bg-card`/`--accent` 등을 [data-skin] 스코프서 Toss값 덮기 + 사이드바/admin-table radius). partner 5파일이 쓰는 `--card`/`--primary`는 Phase0 정의분(기존)이라 **PR-A는 toss-admin.css 미의존**. developer 구현기록 "toss-admin.css 미접촉/신규규칙0"과 working tree(144줄 추가) **불일치** → 커밋 분리 시 toss-admin.css는 Phase1 PR로 빼는 게 정합(PR-A 커밋에 섞이면 범위 오염). **PM 확인 권장**.
2. **navlink active(현재 탭) 미표시 — 기존 동작 보존(회귀 아님)**: layout이 서버 컴포넌트라 usePathname 없음 → 원본도 active 강조/aria-current 없었음(HEAD diff 확인). Toss 재스킨도 동일 유지 = 회귀 0. 다만 현재 위치 시각 피드백 부재는 UX 개선 여지(후속: 클라 navlink 분리 or aria-current). 이번 비주얼-only 범위에선 적절.

⚠️ reviewer 참고:
- **기능 회귀 0 핵심**: 권한가드·SWR키·POST/PATCH body·filterTabs·statusConfig·VP2_TABS·isEditable 한 줄도 안 바뀜. 시각 속성(아이콘 컴포넌트·className·data-skin·인라인 토큰 var())만 교체. statusConfig는 `{label,color,bg}`→`{label,tone}` 타입만 변경(색 의미 동일 매핑, 데이터 분기 불변).
- **PhoneInput**: className만 `.ts-input` 전달(룩 교체), value/onChange(updateField) 바인딩 불변.
- **유일 후속 확인사항 = toss-admin.css 범위**(후속1) — 기능/디자인엔 무영향, 커밋 위생 차원.

## 구현 기록 (developer)

(B2/B3 완료 — 커밋 73951f2 push. 상세는 작업로그.)

### 관리자 Toss Phase 1 — AdminShell 셸 크롬 + 5화면 리스킨 (2026-06-21, 비주얼만 / 기능 회귀 0 / schema 0 / 신규route 0 / 운영 write 0)

📝 구현: toss-admin.css **토큰 리매핑**(운영 globals/admin.css 실토큰 → Toss 값, `[data-skin="toss"]` 스코프) + 셸 크롬·5페이지 루트에 `data-skin="toss"` opt-in + Material→lucide(`<Icon>` 키트). 마크업 색/아이콘만 교체, 컬럼/필터/정렬/액션/모달/문구/라우트/server action **diff 0**. globals.css·admin.css·전역:root **무수정**(diff 0). partner-admin/referee/(web)일반 **미접촉**.

| 파일 | 변경 내용 | 신규/수정 |
|------|----------|----------|
| `src/styles/toss-admin.css` | 기존 `.ts-*` 아래 **Phase1 토큰 리매핑** 블록 추가. `[data-skin="toss"]` + `[data-theme="dark"] [data-skin="toss"]` 둘 다 **동일 라이트값**(후자 명세도 높아 다크 누수 0=라이트 고정). 베이스토큰 remap: `--bg/--bg-elev/--bg-card/--bg-alt/--bg-head`(Toss grey)·`--ink*`(191F28계열)·`--border*`·`--cafe-blue*→#3182F6`·`--accent/--accent-soft→Toss Blue`·`--ok/--warn/--danger`(Toss톤)·shadow(brutalist offset→soft). admin.css 미커버 `--color-*`(elevated/primary-soft/accent-hover/text-dim 등) 명시 보강. 클래스 오버라이드: `.aside__link/.admin-aside__link`(radius14·다크active 복원)·`.admin-stat/.pa1-hero-stat`(radius16·soft shadow)·`.admin-stat-pill/.admin-status-tab/.admin-detail-modal/.admin-mobile-drawer/.admin-table-wrap`. **전부 `[data-skin="toss"]` 스코프**(누수0) | 수정 |
| `src/components/admin/sidebar.tsx` | `<aside>`에 `data-skin="toss"`. `SIDEBAR_ICON`(Material명→lucide kebab) 매핑표 + `toLucide()`(export). renderItem·푸터(마이페이지/사이트로) Material span→`<Icon name size>`. **navStructure(메뉴 정의)·필터·라우팅·ThemeSwitch 불변**. sports_basketball→volleyball(lucide basketball/dribbble 부재) | 수정 |
| `src/components/admin/mobile-admin-nav.tsx` | 토글버튼·오버레이·드로어 `<aside>`에 `data-skin="toss"`. menu/close/메뉴아이템(toLucide)/푸터 Material→`<Icon>`. ESC/외부클릭/스크롤잠금/props(roles,user) 불변 | 수정 |
| `src/components/admin/admin-shell.tsx` | `.admin-topbar`에만 `data-skin="toss"`. **공유 `.admin-shell`/`.admin-main`/`.admin-main__inner` 금지**(Phase2 자식 누수 차단) | 수정 |
| `admin/users/page.tsx` | 루트 div `data-skin="toss"` + `report`→flag(`<Icon>`). 쿼리/Hero stat/server action props 불변 | 수정 |
| `admin/users/admin-users-table.tsx` | `shield_person`→shield(`<Icon>`). DataTableV2 컬럼/모달/액션/필터탭 불변 | 수정 |
| `admin/games/page.tsx` | 루트 div `data-skin="toss"`. 쿼리/직렬화/페이지네이션 불변 | 수정 |
| `admin/games/admin-games-content.tsx` | `notifications_active`→bell-ring·`person_add`→user-plus·`person`→user(`<Icon>`). 신청현황 모달/상태탭/액션 불변 | 수정 |
| `admin/community/page.tsx` | 루트 div `data-skin="toss"` + `newspaper`→newspaper(`<Icon>`). 쿼리 불변 | 수정 |
| `admin/community/admin-community-content.tsx` | `visibility`→eye·`visibility_off`→eye-off·`delete`→trash-2(`<Icon>`). 숨김/복원/삭제 server action·모달 불변 | 수정 |
| `admin/organizations/page.tsx` | 루트 div `data-skin="toss"` + 5아이콘(close→x·check→check·block→ban·archive→archive·restart_alt→rotate-ccw). fetch/approve/reject 핸들러·탭·모달 불변 | 수정 |
| `admin/tournaments/page.tsx` | 루트 div `data-skin="toss"`(목록·상태 화면만). 쿼리/isSuperAdmin 분기 불변 | 수정 |
| `admin/tournaments/admin-tournaments-content.tsx` | `warning`→triangle-alert(`<Icon>`). 상태/공개 뱃지·hardMode 삭제 모달 불변 | 수정 |

🔑 **토큰 리매핑 전략(마크업 0변경 흡수)**: 운영 admin.css/자식은 globals 베이스토큰(`--bg-card/--ink/--accent/--ok/--warn/--danger/--cafe-blue*`) 또는 admin.css `--color-*` 별칭(=`var(--bg-card)`식 베이스 참조)을 소비. **베이스토큰만** `[data-skin="toss"]` 스코프에서 덮으면 별칭이 자동 전파 → 마크업 한 글자도 안 바꾸고 Toss 색 흡수.

🔑 **라이트 고정(다크 누수 0)**: `[data-skin="toss"]`(명세도 0,1,0) + `[data-theme="dark"] [data-skin="toss"]`(0,2,0) 둘 다 동일 라이트값 선언. html이 다크여도 후자가 우선 → Toss 영역 항상 라이트. 다크 전용 클래스 규칙(`[data-theme="dark"] .admin-aside__link{radius:0}` 등)도 명세도 높은 셀렉터로 Toss 라운딩 복원.

🔑 **스코핑(누수 0)**: data-skin은 **(a)셸크롬**(`<aside>`사이드바·모바일 토글/오버레이/드로어·`.admin-topbar`) **(b)5페이지 루트 div**에만. 공유 `.admin-shell`/`.admin-main`/`.admin-main__inner` **금지**(검증: admin-shell.tsx grep 0건). 자식 content/table은 페이지 루트 div 안에 중첩 렌더 → DOM 상속으로 skin 적용(별도 data-skin 불필요).

🟢 **ThemeSwitch 유지**(트랙A 기능보존 — 제거 0. Toss 영역에선 라이트 고정이라 무효과·후속 정리 예정).

🎨 **lucide 키트 경유**: `@/components/admin-toss` 의 `Icon`(Phase0 박제, kebab name→PascalCase 자동변환→lucide-react). CDN injection 0. **실존명 검증**: 36개 중 Dribbble만 부재→volleyball 대체(node로 lucide-react export 실측). 나머지(flag/shield/bell-ring/user-plus/user/eye/eye-off/trash-2/ban/rotate-ccw/triangle-alert/layout-dashboard/trophy/user-cog/volleyball/users/map-pin/message-square/megaphone/handshake/bar-chart-3/list/store/circle-user/arrow-left/menu/x/check 등) 전부 실존 확인.

✅ **검증 전부 통과**: `npx tsc --noEmit` **exit 0**. material-symbols 잔존 **셸+5화면 트리 0건**(grep). globals.css·admin.css git diff **0**. partner-admin/referee/(web)일반 변경 **0**(partner-admin 5파일=partner PR-A 별도 작업, 내 미접촉). data-skin=셸크롬+5페이지 루트만(공유래퍼 grep 0). 운영 write 0·schema 0·신규 route 0·CDN injection 0.

💡 tester 참고:
- **로직 회귀 0이 정상** — diff=아이콘 교체 + data-skin 속성 외 0. 컬럼/필터/정렬/액션/모달/라우트 전부 동일 동작.
- 테스트법: dev 3001 admin 로그인 → 사이드바/모바일 드로어/상단바가 **Toss 라이트(흰 배경·#3182F6 파랑·둥근 라운딩)**. 5화면(users/games/community/organizations/tournaments) 진입 시 Toss 색/lucide 아이콘. 미리스킨 화면(analytics/payments 등 Phase2)은 **기존 스타일 유지**(누수 0).
- **라이트 고정**: 우상단 ThemeSwitch로 다크 켜도 admin 셸/5화면은 라이트 유지(Toss 영역). (web) 프론트는 다크 정상 적용.
- 아이콘: 농구공(경기관리 메뉴)=volleyball(배구공 모양 — lucide 농구 부재 대체).

⚠️ reviewer 참고:
- 모든 CSS 규칙 `[data-skin="toss"]` 시작(스코프밖 0). 라이트 고정 셀렉터 페어(`[data-skin]` + `[data-theme="dark"] [data-skin]`) 명세도 우선순위 확인.
- 토큰 리매핑은 **베이스토큰**(globals 정의)을 스코프 오버라이드 — admin.css `--color-*` 별칭은 var() 참조라 자동 전파. globals/admin.css 원본 무수정.
- data-skin 공유래퍼 금지 준수(admin-shell.tsx의 .admin-shell/.admin-main/__inner엔 미부착 — Phase2 자식 누수 방지).
- volleyball 대체 1건(sports_basketball). Dribbble 부재는 node 실측으로 사전 차단(빈 span 회귀 0).

### 관리자 Toss Phase 3 PR-A — partner-admin 재스킨 (2026-06-21, 비주얼만 / 기능 회귀 0 / schema 0 / 신규route 0)

📝 구현: `partner-admin/*` 5파일을 Toss 디자인시스템으로 재스킨. 변경은 ①아이콘 Material→lucide(`<Icon>` 키트) ②className `.ts-*` ③`data-skin="toss"` 루트 속성 — **3종만**. 데이터패칭·server action·라우트·필터·폼·핸들러 로직 **diff 0**. toss-admin.css·admin/*·referee/*·admin-toss/ **미접촉**(읽기전용 재사용).

| 파일 | 변경 내용 | 신규/수정 |
|------|----------|----------|
| `partner-admin/layout.tsx` | 셸 루트 `data-skin="toss"`. 헤더=라이트 카드(var(--card)/--ink), 가로탭=`.ts-navlink`(width auto), 승인대기=`.ts-badge--warn`. 아이콘 lucide: storefront→store/dashboard→layout-dashboard/campaign→megaphone/stadium→building/pending→clock. **navItems/href/권한검증(super_admin 분기·partner_members) 불변** | 수정 |
| `partner-admin/page.tsx` | 루트 `data-skin`. navy hero 폐기→로컬 `H`(페이지헤더, 키트수정0)+`StatCard` KPI grid(4). 상태현황=`.ts-card`+`.ts-badge--grey`. 빠른액션=`.ts-card--flat`. add_circle→plus/hub→share-2. **SWR 2개(partner/me·stats)·실데이터 불변** | 수정 |
| `partner-admin/venue/page.tsx` | 루트 `data-skin`. H 헤더, sub-tab=`.ts-segment`(data-active), 폼=`.ts-field/.ts-input/.ts-select`, 영업시간=`.ts-card--flat`, 저장=`Btn`. badge→id-card/schedule→clock/sports_basketball→circle-dot. **handleSave/PATCH·VP2_TABS·updateField·PhoneInput(className만 .ts-input) 불변** | 수정 |
| `partner-admin/campaigns/page.tsx` | 루트 `data-skin`. H+`Btn`(plus), 필터=`.ts-chip`(data-active), 카드=`.ts-card`+`Badge`(tone), 폼=`.ts-field`. statusConfig 색→Badge tone 매핑(draft/paused/ended→grey·pending_review→warn·approved→ok·rejected→danger). visibility→eye/ads_click→mouse-pointer-click/trending_up→trending-up/grid_view→layout-grid/campaign→megaphone. **POST/handleCreate/filterTabs/totals reduce 불변** | 수정 |
| `partner-admin/campaigns/[id]/page.tsx` | 루트 `data-skin`. `StatCard` KPI 3(노출/클릭/CTR), 카드/배치=`.ts-card`, 폼=`.ts-field`, 버튼=`Btn`, 상태=`Badge`. arrow_back→arrow-left/edit→pencil/grid_view→layout-grid/add→plus. **PATCH/handleSave·placements POST·isEditable·startEdit 불변** | 수정 |

🔑 **opt-in 격리 준수**: data-skin="toss"를 셸 루트 + 각 페이지 루트(로딩 early-return 포함)에 부착 → bare `var(--ink)/--bg/--card/--primary/--ok/--warn` 등 Toss 토큰 활성화. 미부착 시 `.ts-*` 무효라 각 루트 필수. toss-admin.css 신규 규칙 추가 0(사용 클래스 `.ts-card/--flat/--tight`·`.ts-field/__label`·`.ts-input`·`.ts-select`·`.ts-segment/__btn`·`.ts-chip`·`.ts-badge--*`·`.ts-navlink`·`.ts-btn--*` 전부 실재 확인). 인라인 style은 토큰 var()만(하드코딩 hex 0).

🎨 **lucide 매핑(전부 키트 `<Icon>` 경유, material 잔존 0 in partner)**: store/layout-dashboard/megaphone/building/clock/plus/share-2/eye/mouse-pointer-click/trending-up/layout-grid/id-card/circle-dot/arrow-left/pencil. 전부 lucide-react 실존명(별칭 불필요).

✅ **검증 전부 통과**: `npx tsc --noEmit` **exit 0**. `git status --porcelain src/` = partner-admin 5파일만 M(미접촉 toss-admin.css·admin/*·referee/*·admin-toss/ 변경 0 확인). 기능 로직 diff 0(아이콘+className+data-skin 외 변경 없음). schema 0·신규 라우트 0·`(web)/*` 일반 프론트 0.

💡 tester 참고:
- 테스트법: dev 3001 또는 프리뷰에서 `/partner-admin`(대시보드)·`/partner-admin/campaigns`(+상세 `/[id]`)·`/partner-admin/venue` 진입. **파트너 멤버십 또는 super_admin 계정 필요**(layout 권한 가드).
- 정상 동작: Toss 라이트 톤(흰 카드+#3182F6 primary)으로 렌더, navy hero 사라짐. KPI는 StatCard(아이콘+값), 캠페인 카드 상태=컬러 Badge, 필터칩 클릭 시 primary 활성. **기능(필터 전환·폼 입력·저장·배치 추가)은 기존과 100% 동일하게 동작**(로직 무변경).
- 주의 입력: campaigns 상태 6종(draft/pending_review/approved/rejected/paused/ended) Badge tone 정확 노출 / venue 2탭(기본/시간·가격) 전환 / [id] editable 상태(draft/pending_review/rejected)에서만 수정·배치추가 버튼 노출.
- 데이터 0건/로딩: 캠페인 빈상태(megaphone 아이콘), venue 코트 0(building 아이콘 안내), 로딩 스켈레톤(grey-100) 정상.

⚠️ reviewer 참고:
- **기능 회귀 0 핵심**: server logic(권한 가드·SWR 키·POST/PATCH body·filterTabs·statusConfig 분기·VP2_TABS·isEditable)은 한 줄도 안 바뀜. 시각 속성(아이콘 컴포넌트·className·data-skin·인라인 토큰)만 교체.
- statusConfig: 기존 `{label,color,bg}` → `{label,tone:BadgeTone}` 로 **타입만** 변경(색 의미 동일 매핑). 데이터 분기·키 불변.
- PhoneInput: `type PhoneInputProps = Omit<input props,...>` 라 className 안전 전달(.ts-input). style prop 제거(불필요).
- data-skin 부착처: 셸 루트 + 각 페이지 루트 + [id] 로딩 early-return div(토큰 활성화 누락 방지).

### 관리자 Toss 디자인시스템 이식 — Phase 0 (2026-06-21, 이식만 / 리스킨 0 / schema 0 / 프론트 변경 0)

📝 구현: 시안 toss.css·toss-kit.jsx·toss-admin.jsx 를 운영에 이식. **Toss 토큰을 `[data-skin="toss"]` opt-in 스코프로 격리**(전역 :root 오염 0). CDN lucide injection→lucide-react import. window 전역노출→named export+TS 타입. Phase 0 에서 data-skin 적용처 0 = **시각 변화 0**.

| 파일 | 변경 내용 | 신규/수정 |
|------|----------|----------|
| `src/styles/toss-admin.css` | 시안 toss.css 박제. **모든 Toss 토큰+`.ts-*` 규칙을 `[data-skin="toss"]` 스코프로 격리**(bare :root 정의 0). 시안 `.tweaks`(개발 데모 패널)는 운영 키트 불필요로 제외 | 신규 |
| `src/app/layout.tsx` | `import "../styles/toss-admin.css"` 1줄 추가(+주석 3줄). 기존 admin.css import 바로 아래 | 수정 |
| `src/components/admin-toss/kit.tsx` | toss-kit.jsx 이식. Icon(lucide-react)·Btn·Badge·Toggle·Check·StepDots·Empty·Modal. Icon은 시안 kebab-case name(예 "search")→PascalCase 변환 후 lucide-react 조회. `won-sign`만 부재→별칭 Banknote | 신규 |
| `src/components/admin-toss/data.tsx` | toss-admin.jsx 이식. SectionTitle·StatusTabs·FilterBar·SortHead·DataTable(제네릭 `<T>`)·DetailModal·StatCard·PanelStat·PanelRow·MockToggle | 신규 |
| `src/components/admin-toss/index.ts` | barrel export(컴포넌트+타입). `@/components/admin-toss` 단일 진입점 | 신규 |

🔑 **네임스페이스 격리(핵심 보정 — data-skin opt-in)**:
- 기존 admin.css bare `.admin-shell` 이 전역 `var(--bg)/--ink/--ok/--warn/--danger` 사용 중 → Toss 토큰을 같은 이름으로 bare 스코프하면 미리스킨 화면이 즉시 Toss색으로 뒤집힘(리스킨 0 위반).
- 해결: toss-admin.css 토큰·`.ts-*` 규칙 전부 **`[data-skin="toss"]` 하위로**. Phase 0 은 어디에도 data-skin 미부착 → 시각 변화 0. Phase 1+ 에서 리스킨 셸이 `<div data-skin="toss">` opt-in.
- 전역 :root·globals.css·기존 admin.css **무수정**(검증: 셋 다 Toss 토큰 침범 0).

🎨 **lucide 매핑(CDN→lucide-react)**: 시안 `window.lucide.createIcons()` 제거. Icon 은 kebab→Pascal 자동변환(search→Search, chevron-up→ChevronUp, rotate-ccw→RotateCcw, chevrons-up-down→ChevronsUpDown). 별칭 1건: `won-sign`→`Banknote`. 매핑 실패 시 빈 span(레이아웃 방어).

✅ **검증 전부 통과**: `npx tsc --noEmit` **exit 0**(초기 4파일 끝 잘못 삽입된 `</content>` 제거 후). 전역 :root Toss 토큰 0 / 토큰 전부 `[data-skin]` 스코프 내 / **data-skin JSX 실적용 0**(Phase0) / createIcons 실호출 0 / CDN injection 0 / lucide-react import만 / globals.css·admin.css·`(web)/*` git diff **0**(프론트 회귀 0).

💡 tester: **시각 변화 0이 정상**(이식만, data-skin 미부착). `npx tsc --noEmit` exit 0. 토큰 격리 확인=`<div data-skin="toss"><Btn>..</Btn></div>` 임시래핑하면 Toss적용, 래핑없으면 .ts-* 미적용. `(web)/*` 동일.
⚠️ reviewer: 모든 CSS 규칙이 `[data-skin="toss"]` 시작(스코프밖 영향0, @keyframes만 글로벌·ts프리픽스 충돌0). DataTable 제네릭`<T>`·onSortChange no-op 폴백. ICON_ALIAS won-sign→Banknote 1건. Badge는 barrel 노출 유지(data.tsx 미사용 import 제거).

#### 수정 이력
| 회차 | 날짜 | 수정 내용 | 수정 파일 | 사유 |
|------|------|----------|----------|------|
| 1차 | 2026-06-21 | **차단①(잔여 XML 태그)**: toss-admin.css L237 박제 잔여 `</invoke>` 1줄 삭제(`.ts-phone`이 진짜 마지막 규칙). grep `<(/?)(invoke\|content\|parameter\|function)>` src/ 전체 → 0건 확인. **권장1 a11y(기반 컴포넌트 지금 박제)**: ①kit.tsx **Check**=`role="checkbox"`+`aria-checked`+Space/Enter onKeyDown 토글(Space preventDefault 스크롤점프방지). ②kit.tsx **Modal** X버튼 `aria-label="닫기"`(role/aria-modal/ESC 이미 있음). ③data.tsx **DetailModal**=패널 div에 `role="dialog"`+`aria-modal="true"`+`tabIndex=-1`+열릴때 panelRef.focus()(첫 포커스, focus-trap 라이브러리 미도입), X버튼 `aria-label="닫기"`(ESC 이미 있음). ④data.tsx **DataTable** 인라인 체크박스 2곳(전체선택/행선택)=`role="checkbox"`+`aria-checked`+`aria-label`+Space/Enter 토글. **권장2 ICON_ALIAS**: `thunderdunk`(lucide 실부재)→`Zap` 별칭 등록(Phase1+ 호출부 빈 span 방지). 격리 유지: 전부 `[data-skin="toss"]` 스코프·(web)/globals/admin.css·schema 변경 0·CDN injection 0. tsc --noEmit exit 0. data-skin JSX 실적용 0(Phase0 시각변화 0). | `toss-admin.css` · `kit.tsx` · `data.tsx` | tester/reviewer 후속: 차단①(잔여태그) + 권장1(a11y 기반박제) + 권장2(ICON_ALIAS) |

### 매칭 고도화 M6(최종) — 호스트 콘솔 ①데드코드 ②E-1 3구획 ③E-2 waiting탭+호스트카드 (2026-06-19, 비파괴 / schema·신규route 0 / 운영 write 0)

📝 구현: ①my-games 미사용 status맵 삭제 ②GameDetail 호스트 신청관리 2구획→**3구획**(대기승인/확정/대기열+승격) ③MyGames **waiting 탭** + 하단 **호스트 운영 카드**(현황 칩+빠른액션). **schema 0 / 신규 API route 0**(기존 쿼리 필드추가 + 기존 PATCH/confirm/DELETE 재사용).

| 파일 | 변경 내용 | 신규/수정 |
|------|----------|----------|
| `my-games/page.tsx` | **①** 미사용 `GAME_STATUS_LABEL`(3진행중/4완료/5취소, 참조 0건) 삭제. **③** gameItems status 분기에 `3→waiting` 추가 + `waitlistPosition` 매핑. 탭 분류에 `waiting`(순번순 정렬) 추가→MyGamesClient 전달. **호스트 집계**: hostedGameIds로 `game_applications.groupBy({by:[game_id,status]})` **1회** 호출→countMap(approved=1/pending=0/waiting=3). hostCards(HostGameCardData)·hostSummary(모집중1/확정2/완료3) 변환. 하단 GameCard 그리드→**요약 stat + HostGameCard 그리드** 교체. GameCard import 제거 | 수정 |
| `my-games/_components/host-game-card.tsx` | **신규** 호스트 운영 카드(클라). 종류배지(game_type Int 0픽업/1게스트/2연습)+상태배지+지역, 현황 칩(승인 N/정원·승인대기 N·대기열 N), 빠른액션(신청관리→상세/E-1·마감확정·수정→/edit·취소). **취소=기존 DELETE 재사용**(confirm 후, full uuid). 마감확정=status전용 전환 API 부재(아래 ⚠️)→안내 alert. var(--*)·44px·radius-chip | 신규 |
| `my-games/_components/my-games-client.tsx` | Tab 타입에 `waiting` 추가, waiting prop 수용, 탭 바에 "대기" 추가, list/counts 분기 확장 | 수정 |
| `my-games/_components/status-badge.tsx` | RegStatus에 `waiting` 추가 + STATUS_MAP에 "대기중"(accent 톤) | 수정 |
| `my-games/_components/reg-row.tsx` | RegItem에 `waitlistPosition?` 추가. waiting 액션 분기(상세보기+대기취소). 메타 라인에 "대기 N번"(accent) 표시 | 수정 |
| `games/[id]/page.tsx` | **②** hostApplicants 매핑에 `waitlist_position`·`promotion_deadline`(ISO) 추가(listGameApplications include라 추가 조회 0) | 수정 |
| `games/[id]/_v2/host-panel.tsx` | HostApplicant 타입에 is_guest/experience_years/message/waitlist_position/promotion_deadline 추가(applicants 통째 전달이라 전달부 무변경) | 수정 |
| `games/[id]/_components/host-applications.tsx` | **2구획→3구획** 재구성: ①대기승인(status=0, 승인/거절=기존 PATCH) ②확정참가자(status=1) ③대기열(status=3, waitlist_position 순번 원형배지 + 승격). **승격=confirm POST 재사용**: `promotion_deadline` 활성(미래) 대기자만 "승격 확정" 버튼+카운트다운, 그 외는 "대기 중·빈자리 시 자동 알림" 안내. 200/410(만료·다음순번)/409(정원재마감)→router.refresh. 거절 시 빈자리 자동승격 반영 위해 refresh. var(--color-*)·44px·rounded-[50%] | 수정 |

🔑 **3구획 + 승격 confirm UI 분기(PM 승인 결정)**:
- 승격은 **confirm POST만 매핑**. 시안 "승격 알림" 버튼 **미박제**(운영=빈자리 발생 시 자동 알림+promotion_deadline 활성화).
- confirm은 deadline 살아있는 대기자만 성공(없으면 410). → UI도 동일 기준: `isPromotable = deadline!=null && deadline>now`일 때만 "승격 확정" 활성, 아니면 안내 상태.
- 410/409 모두 router.refresh로 서버 상태 재반영(만료자 강등·다음순번 승격은 서버 demoteExpiredAndPromoteNext가 처리).

⚠️ **마감·확정(status 1→2) API 부재 — 빠른액션 1건 안내 처리**: 기존 PATCH `/api/web/games/[id]`는 status 필드를 받지 않음(검증 후 확인). schema/route 0 제약상 status 전용 전환 API 신설 불가 → 호스트 카드 "마감·확정"은 "정원 차면 자동 확정(수동 마감 준비 중)" alert로 박제만. **운영은 정원 충족 시 승인(confirm)으로 자동 status=2 전환**(M1)되므로 기능 공백 아님. 나머지 빠른액션(신청관리/수정/취소)은 실동작.

🏗️ 집계 정확:
- 호스트 카드 승인/대기/대기열 = `game_applications.groupBy([game_id,status])` **1회**(경기별 N회 count 회피). approved=status1, pending=status0, waiting=status3. current_participants(좌석선점)가 아닌 실제 status1 count 사용(시안 "승인 N/정원"=확정 참가자).
- waiting 탭/대기열 순번 = `waitlist_position` 오름차순(없으면 9999 뒤로).

✅ **tsc --noEmit: 통과 (exit 0)**. 초기 에러 2건: game_type이 시안 string 가정→실제 **Int**(0픽업/1게스트/2연습) 정정, title `string|null` fallback. dev 3001 GET: /games/my-games 200·/games 200(서버 500 0). schema diff 0·신규 route 0·운영 write 0(승인/거절/승격/취소 실호출 안 함, 코드경로).

💡 tester 참고:
- **운영 write 금지** — 승인/거절/승격/취소 실호출 금지(코드경로로 검증). dev 3001 GET 렌더만.
- **① 데드코드 삭제 영향 0**: GameCard 자체 STATUS_LABEL(game-status.ts) 사용→하단 호스트 카드 상태 라벨 정상(모집중/확정/완료/취소).
- **② 3구획**: 호스트로 본인 경기 상세 진입 시 신청 관리가 대기승인/확정참가자/대기열 3섹션. 대기열은 순번 원형 배지(1,2,3…).
- **승격 410 분기**: promotion_deadline 활성(자동 알림 받은) 대기자만 "승격 확정" 버튼+카운트다운. deadline null/과거면 "대기 중·빈자리 시 자동 알림". confirm 누르면 200=확정/410=만료(다음순번)/409=정원재마감.
- **③ waiting 분류**: 만석 경기 대기 신청(status=3)이 my-games "대기" 탭에 분리. RegRow에 "대기 N번"(accent) + 대기 배지("대기중").
- **호스트 카드 집계**: 내가 만든 경기 카드에 "승인 N/정원·승인대기 N·대기열 N" 칩. 빠른액션: 신청관리(→상세)·마감확정(준비중 alert)·수정(→/edit)·취소(confirm→DELETE).
- **마감확정 alert**: "정원 차면 자동 확정" 안내만(API 부재). 취소만 실제 write(테스트 시 미실행).

#### 수정 이력
| 회차 | 날짜 | 수정 내용 | 수정 파일 | 사유 |
|------|------|----------|----------|------|
| 1차 | 2026-06-20 | **보완①(수동 마감확정 API 신설)**: 신규 POST `/api/web/games/[id]/close`(status 1→2 전용). 가드=호스트만(`organizer_id!==userId`→403)·**status===1(모집중)일 때만→2**(2확정/3완료/4취소/0초안=409 INVALID_STATUS, 정원충족 자동확정 M1과 멱등 충돌 방지). 핵심: `if (game.organizer_id !== userId) return apiError(...,403)` + `if (game.status !== 1) return apiError("모집중인 경기만...",409)` → `prisma.games.update({data:{status:2}})`. host-game-card 버튼 placeholder alert→`handleClose()`(POST /close, 성공시 router.refresh·에러 alert)로 교체, **`recruiting && !isFull`일 때만 노출**(isFull이면 M1 자동확정 경로가 처리→숨김). destructive 0·schema 0. tsc 0 | `games/[id]/close/route.ts`(신규) · `host-game-card.tsx` | 사용자 승인: 정원 미달 시 호스트 조기 마감 불가 갭 해소 |
| 1차 | 2026-06-20 | **보완②(취소 시 대기자 알림 누락)**: DELETE route 취소 알림 대상 `status:{in:[0,1]}`→**`{in:[0,1,3]}`** 확장. M2 대기자(status=3)도 GAME_CANCELLED 통보, 거절(2) 제외 유지. 핵심: `where: { game_id: game.id, status: { in: [0, 1, 3] } }`. 코드경로=DELETE soft cancel(status=4) 직후 findMany→createNotificationBulk fire-and-forget. tsc 0 | `games/[id]/route.ts` L296 | reviewer 후속②: 대기열 신청자 취소 알림 미수신 |

### 리뷰 결과 (reviewer) — M6 호스트 콘솔 (2026-06-19)

📊 종합 판정: **통과 (차단 0)** — 권장 0 · 후속 3

✅ 잘된 점:
- **E-1 권한/IDOR 견고**: confirm route(L47~52) 본인||호스트 가드 + status=3 검증(409) + promotion_deadline null/만료 410 + tx 내 `current<max` 조건부 UPDATE(409 롤백)로 정원초과·race 모두 차단. PATCH route는 호스트 단독 가드(L40)+status=0만 처리(L57)로 이중 승인/거절 차단. **클라(host-applications)는 표시분기만, 권한은 전부 서버 판정** — 올바른 신뢰경계.
- **승격 UI 분기 정합**: `isPromotable = deadline!=null && deadline>now`(L286)가 서버 confirm 성공조건(deadline 활성)과 정확히 일치. 410/409/200 모두 router.refresh로 서버상태 재반영. 시안 "승격 알림" 미박제 = PM 승인 결정대로(운영=빈자리 자동 알림).
- **status맵 데드코드 삭제 안전**: grep 결과 `GAME_STATUS_LABEL` 참조 **page.tsx 1건(주석/삭제설명)뿐**, 실사용 0건. GameCard는 game-status.ts 정본 STATUS_LABEL 사용 → 표시 회귀 0 확인.
- **groupBy 집계 N+1 회피**: hostedGameIds(take:10) `in` 1회 groupBy로 status별 count → countMap 변환. 경기별 count 반복 없음. approved=status1(좌석선점 current가 아닌 실 승인수) 사용 = 시안 "승인 N/정원" 의미 정합.
- **waiting 분류 정확**: gameApps status=3→waiting 매핑(page L227), waiting 탭 waitlist_position asc 정렬(L312), RegRow "대기 N번"(accent)+status-badge "대기중". MyGamesClient 4탭(예정/대기/지난/취소) counts 분기 정합.
- **game_type Int 매핑 정합**: HostGameCard KIND_LABEL(0픽업/1게스트/2연습경기) = GameCard 정본(game-card.tsx L72~74) 일치. title `?? "제목 없음"` null fallback OK.
- **디자인 13룰 준수**: var(--*) 토큰만(하드코딩 hex 0), 순번배지 rounded-[50%] 정사각(pill 9999px 0), Material Symbols 영역 없음(아이콘 미신설), AppNav 무관, 시안 jsx 미수정. badge--blue/--ok/--ghost 클래스 globals.css L338~342 실재 확인.

🟢 후속 (차단/권장 아님 — 별도 결정):
1. **마감·확정 API 갭(평가: 심각도 낮음 / 후속 선택)**: 기존 PATCH `/api/web/games/[id]`가 status 필드 미수용 → 호스트 카드 "마감·확정" 버튼은 안내 alert(실전환 X). **기능 공백 아님** — 정원 충족 시 confirm/apply 경로가 자동 status 1→2 전환(M1). 다만 "정원 미달인데 호스트가 조기 마감하고 싶은" 케이스는 불가. schema/route 0 제약상 이번 박제는 적절. status 전용 전환(POST `/close` 등) 신설은 후속 결정 사항.
2. **취소 시 대기열(status=3) 알림 누락(기존 M1 코드, M6 무관)**: DELETE route(route.ts L297)가 취소 알림 대상을 `status:{in:[0,1]}`로 한정 → **대기열(3) 신청자는 경기 취소 알림 미수신**. M2에서 status=3 신규 도입 시 이 in절 미갱신. M6 변경 아니나 호스트가 취소하면 대기자가 모르고 남음. 후속 `[0,1,3]` 확장 검토.
3. **취소 경기 대기자 confirm 엣지(이론적, 영향 경미)**: 경기 status=4(취소)여도 confirm route는 application.status=3 + deadline 활성만 검증(게임 status 미검사) → 이론상 deadline 활성 대기자가 취소된 경기에 좌석 확보 가능. 단 취소 시 빈자리 트리거가 없어 deadline 활성화가 거의 안 일어나므로 실현 가능성 낮음. 방어로 confirm에 `game.status in (1,2)` 가드 추가 권장(후속).

⚠️ reviewer 참고:
- **마감·확정 API 갭**: 기존 PATCH가 status 미수용 → 호스트 카드 마감확정 버튼은 안내 alert(실전환 X). status 전환은 정원충족 자동(M1) 또는 confirm 경로에만 존재. status 전용 PATCH 필드 추가 여부는 후속 결정.
- **취소 식별자**: HostGameCard 취소는 **full uuid**(DELETE route가 `where:{uuid:id}` full uuid 사용)로 호출. host-applications의 confirm/PATCH는 [id]=short8/full 모두 처리 라우트라 gameId(URL [id]) 그대로 사용 — 두 경로 식별자 차이 의도적.
- **groupBy 1회**: 경기 10건 이하(take:10)라 in 절 가벼움. status 2(거절)는 카드 칩에 미표시(승인/대기/대기열만).
- **거절 후 자동승격 반영**: host-applications 거절(PATCH)은 서버가 빈자리 승격 트리거→클라 router.refresh로 대기열 변동 반영. 승인은 좌석 불변이라 refresh 없이 낙관적 업데이트.
- **디자인 회귀**: var(--*)만(하드코딩 hex 0, 기존 text-white→--color-on-accent 정정). Material Symbols 영역 없음(아이콘 미신설, 텍스트/배지만). pill 9999px 0(순번 배지=rounded-[50%] 정사각). AppNav 무관. 시안 jsx 미수정.

### 매칭 고도화 M5 — 찾기/검색 UX (정렬·빠른필터·빈상태, 시안 A, 2026-06-19, 비파괴 / 운영 write 0)

📝 구현: 경기 목록에 **정렬(드롭다운/시트) + 빠른필터 칩(AND) + 빈상태 CTA** 메모리 레이어 추가. listGames 서버함수/route/API 패칭 **무변경**(회귀 0). schema 변경 0. game-card.tsx 무변경.

**🧭 좌표 가용성 판단(확정)**: `games`에 lat/lng 컬럼 없음 → 좌표 0. "가까운순" 정렬·좌표 기반 near **원천 제외**. "내 동네"는 좌표 대신 **로그인 + `users.preferred_regions`(Json 배열) ∩ city·district 부분매칭**으로만 동작, 그것도 메타로 노출 제어(클라 하드코딩 0).

| 파일 | 변경 내용 | 신규/수정 |
|------|----------|----------|
| `(web)/games/page.tsx` | getWebSession→sub로 `prisma.user.findUnique(preferred_regions)` 조회(string만 추림). `sortOptions`(soon/filling/latest — **가까운순 제외**)·`chipOptions`(today/weekend/filling/free + near는 `preferredRegions.length>0`일 때만 push) 메타 계산. GameForClient 매핑에 `createdAt`(ISO, latest 정렬키) 추가. GamesClient에 sortOptions/chipOptions/preferredRegions prop 주입 | 수정 |
| `(web)/games/_components/games-client.tsx` | SortKey/ChipKey·SortOptionMeta/ChipOptionMeta export. 정렬 메모리(`sortGames`)·칩 매칭(`matchesChip`)·fillPct 유틸. SortMenu 컴포넌트(데스크톱 드롭다운/모바일 하단시트, Material Symbols swap_vert/expand_more/check). 빠른필터 칩 바(chipOptions 메타로 렌더, 다중 AND). 빈상태 시안 카드+CTA. **기존 KindTab/FilterChipBar/clientFilters(weekend/free/beginner) 동작 보존**(별개 레이어로 AND 누적) | 수정 |
| `globals.css` | 시안 `.games-chips`·`.games-chip[--clear]`·`.games-sort-*`(드롭다운+모바일 하단시트) 토큰화 박제(var(--*)만). 모바일 정렬 버튼/시트항목 44px | 수정 |

**🔑 정렬/칩 메모리 레이어**:
- 정렬: `soon`=scheduled_at asc(**미래 우선**: 미래경기를 과거/무효보다 앞, 같은 군 내 시간 오름차순) / `filling`=fill_pct(current/max*100) desc / `latest`=created_at desc.
- 빠른필터 칩(AND): `today`(scheduled_at=오늘 로컬) / `weekend`(요일 0·6) / `filling`(fill_pct≥70) / `free`(fee 0‖null) / `near`(preferred_regions ∩ `${city} ${district}` includes). 활성 칩 `every()` 만족해야 통과.
- `clientFilters`(기존 FilterChipBar) → `quickChips`(M5) → `sortGames` 순으로 AND 적용 후 정렬.

**📌 sort_options/chip_options 메타가 좌표0 반영**: sortOptions 배열에 "가까운순" 미포함(주석 명시). chipOptions의 near는 `hasPreferredRegions` 조건부 push라 비로그인/지역미설정 시 칩 자체 비노출. 클라(GamesClient)는 메타가 가진 옵션만 렌더 → UI에 정렬/칩 종류 하드코딩 0.

**⚠️ 페이지네이션 상호작용(확인 완료)**: `listGames`는 **`take:60` 단일 호출, skip/cursor·무한스크롤·더보기 없음 = 전체 로드(상위 60건)**. 따라서 클라 메모리 정렬/필터가 **현재 목록 전체에 정확히 적용**됨(페이지 단위 정렬 한계 없음). 무한스크롤/더보기와 충돌 가능성 0. (단 take=60 상한 자체는 기존 동작 그대로 — M5에서 변경 안 함.)

**빈상태**: filtered 0건 → ① games 자체 0건이면 "등록된 경기가 없습니다." 단순 안내(CTA 없음) ② 필터로 0건이면 시안 카드("조건에 맞는 모집이 없어요" / "필터를 줄이거나 인접 지역을 둘러보세요") + CTA [맞춤필터 끄기](빠른칩 리셋)·[인접 지역 보기](near만 해제, near 켜져있을 때만 노출). 버튼 44px.

**🎨 디자인룰**: var(--*) 토큰만(--cafe-blue/--accent/--ink-on-brand/--border/--bg-card/--bg-alt/--sh-lg/--radius-chip/--radius-card)·하드코딩 hex 0·**Material Symbols**(swap_vert/expand_more/check/search_off, lucide 0)·정렬 시트/모바일 44px·720px 분기(데스크톱 드롭다운↔모바일 하단시트)·AppNav 무관·시안 jsx 미수정. pill 9999px 미사용(칩=radius-chip 6px).

✅ **tsc --noEmit: 통과 (exit 0)**. 초기 `prisma.users`→정정 `prisma.user`(모델명 User PascalCase, 클라 user). schema diff 0. 운영 write 0(SELECT preferred_regions 1회만, 로그인 시).

💡 tester 참고:
- **정렬 정확**: soon=임박순(미래 먼저·시간 오름차순) / filling=많이 찬 순 / latest=최근 등록순. 데스크톱 드롭다운, 모바일(≤720px) 하단시트.
- **가까운순/좌표near 숨김**: 정렬 메뉴에 "가까운순" 옵션 없음. 빠른필터에 좌표기반 near 없음. "내 동네" 칩은 로그인+preferred_regions 설정 시에만 노출(비로그인/미설정=칩 자체 없음).
- **칩 AND**: 여러 칩 동시 활성 시 모두 만족하는 경기만. 기존 FilterChipBar(weekend/free/beginner) 칩과도 AND 누적.
- **빈상태**: 칩으로 0건 → 카드+[맞춤필터 끄기]([인접 지역 보기]는 near 켜졌을 때만). games 0건 → 단순 안내.
- **페이지네이션 한계**: 없음 — 전체 로드(take 60)라 정렬/필터가 목록 전체 정확 적용.
- **기존 필터 회귀**: KindTab(전체/픽업/게스트/연습)·FilterChipBar 토글·dot 카운트·URL 칩 그대로 동작해야 함.
- 테스트법: dev 3001 또는 정적. 로그인(preferred_regions 설정 계정)으로 "내 동네" 칩 노출, 비로그인은 미노출 확인.

⚠️ reviewer 참고:
- **좌표 메타 제어**: 가까운순/near 제외를 클라 하드코딩이 아닌 서버 메타(sortOptions/chipOptions)로 처리 — 향후 좌표 컬럼 도입 시 page.tsx 메타에만 추가하면 됨.
- **"내 동네" 매칭 정밀도**: preferred_regions가 "서울"이면 모든 서울 경기 매칭, "강남구"면 강남구만. 부분 includes라 광역/세부 혼재 가능(데이터 의존). 거리 정렬 아닌 지역 라벨 매칭임을 명시.
- **기존 vs 신규 칩 레이어 공존**: FilterChipBar의 weekend/free와 M5 weekend/free가 의미 중복(둘 다 켜면 동일 조건 AND=무해). 의도적 분리(기존 동작 보존).
- **정렬 안정성**: Array.prototype.sort 비안정 환경 대비 안 함(tie-break 미세 순서 변동 가능). soon/latest는 키 명확, filling은 동률 fill_pct 다수 시 순서 비결정 — 표시 영향 경미.

#### 수정 이력
| 회차 | 날짜 | 수정 내용 | 수정 파일 | 사유 |
|------|------|----------|----------|------|
| 1차 | 2026-06-19 | 빈상태 CTA "맞춤필터 끄기" 0건 탈출 보완: `handleResetQuickChips`가 quickChips만(`setQuickChips([])`) 리셋→ **quickChips + clientFilters(FilterChipBar weekend/free/beginner) 동시 리셋**으로 확장. filtered는 두 클라레이어 AND라 clientFilters로 0건 시 기존엔 눌러도 0건(사용자 갇힘)이었음. KindTab(kind)·URL칩(date/city)은 리셋 제외(탭=컨텍스트 유지/URL=서버listGames 별개레이어). "인접 지역 보기"(near만 해제) 유지. tsc 0 | `games-client.tsx` L405~414 | M5 보완 1건(0건 탈출 갇힘 버그) |
...
### 관리자 Toss Phase 1 — AdminShell 셸크롬 + 5화면 리스킨 검증 (2026-06-21, tsc+정적grep+읽기 / 운영write 0)

**환경**: 운영 write 0. `npx tsc --noEmit`(exit 0) + git diff/status(읽기) + grep 정적 + lucide-react 실존 node 실측. partner-admin/* 미열람(별도 작업). git 무수정.

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| 1. `npx tsc --noEmit` | ✅ 통과 | exit 0, 에러 0 |
| 1. 변경 파일 범위 | ✅ 통과 | `git status src/`=명세 13파일만 M(toss-admin.css+셸3+5페이지+4자식). partner-admin/referee/(web)/(site) 0 |
| 2. 기능 회귀 0(diff=아이콘+data-skin 외 0) | ✅ 통과 | 9페이지 diff 삭제측 로직키워드(fetch/prisma/where/orderBy/status/filter/onClick/router/POST·PATCH/server action) grep **0건**. 유일 삭제 1줄=community 아이콘 삼항(`status==="hidden"?"visibility":"visibility_off"` → `?"eye":"eye-off"`) = **status 분기 보존, Material→lucide 등가 스왑** |
| 3. 토큰 리매핑 스코핑 | ✅ 통과 | toss-admin.css Phase1 추가블록 전 규칙 `[data-skin="toss"]`/`[data-theme="dark"] [data-skin="toss"]` 스코프 시작. bare/전역:root 침범 0(누수후보 grep=주석 텍스트 1건뿐) |
| 3. globals.css·admin.css 무수정 | ✅ 통과 | `git status` 둘 다 미변경(diff 0) |
| 4. data-skin 부착처(셸크롬+5페이지 루트) | ✅ 통과 | topbar(L88)·aside(L263)·모바일 토글/오버레이/드로어(L138/149/160)·5페이지 루트 `<div data-skin="toss">` 부착 |
| 4. 공유 래퍼 누수 0 | ✅ 통과 | admin-shell.tsx `.admin-shell`(L77)·`.admin-main`(L83)·`.admin-main__inner`(L96)에 data-skin **미부착**(grep) → Phase2 자식 상속 0 |
| 5. 라이트 고정(다크 누수 0) | ✅ 통과 | 토큰블록 `[data-skin]`+`[data-theme="dark"] [data-skin]` 둘 다 동일 라이트값. 후자 명세도(0,2,0) 우선 → 다크 토글 시 Toss 영역 라이트 유지. 다크 전용 회귀셀렉터(.admin-aside__link radius 등)도 복원 |
| 5. ThemeSwitch 유지(제거 0) | ✅ 통과 | sidebar L294·mobile-admin-nav L225 잔존 |
| 6. 셸+5화면 material-symbols 잔존 0 | ✅ 통과 | 대상 9페이지+셸3 className `material-symbols` grep **0건**(매칭=주석/Phase2 미대상 컴포넌트뿐). `tournaments/[id]/*`는 Phase1 범위 외 |
| 6. lucide 실존(빈 아이콘 0) | ✅ 통과 | 사용 32종(SIDEBAR_ICON 매핑타겟 19 + Icon name= 직접 13) **node lucide-react export 실측 전부 실존**. Dribbble/basketball 부재→volleyball 대체 정당 |
| 7. mybdr var(--color-*) 직접/다크/AppNav 0 | ✅ 통과 | sidebar `var(--color-border/surface)`=기존코드(diff=data-skin 속성 1개만 추가), toss-admin.css가 `--color-*` 별칭을 Toss 스코프 리매핑 → Toss색 해석(직접 mybdr색 0) |
| 7. partner-admin/referee/(web) diff 0 | ✅ 통과 | git status 빈 출력(미접촉) |

📊 종합: **14개 중 14개 통과 / 0개 실패**

💬 판정: **통과 (수정 요청 0)**. tsc 통과·기능 회귀 0(diff=아이콘 스왑+data-skin 속성 외 0)·토큰 리매핑 `[data-skin]` 스코프 격리·공유 래퍼 누수 0·라이트 고정·lucide 32종 실존·디자인 분기 정상. 구현 기록과 working tree 일치. **커밋 권장**.

⚠️ tester 참고:
- 정적+타입 검증으로 모든 항목 충족. 런타임 시각 확인(dev3001 admin 로그인 → 셸/5화면 Toss 라이트 렌더)은 미실행(운영 write 금지 범위 내 GET이나, 본 검증은 정적으로 회귀 0 입증). 시각 최종 확인은 사용자가 프리뷰에서 1회 권장.
- Phase2 미리스킨 화면(analytics/payments 등)의 material-symbols 잔존은 **정상**(리스킨 대상 아님, 누수 0 = data-skin 미부착이라 기존 스타일 유지).

### 관리자 Toss Phase 3 PR-A — partner-admin 재스킨 검증 (2026-06-21, tsc+정적grep+읽기GET / 운영write 0)

**환경**: 운영 write 0(POST/PATCH 실호출 안 함). `npx tsc --noEmit`(exit 0) + git diff(읽기) + grep 정적 + lucide-react 실존 대조 + dev3001 GET(상태코드만). partner-admin/* 5파일만 검증(admin/*·toss-admin.css는 타 에이전트 동시작업이라 정의 실재 여부만 read). git 무수정.

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| 1. `npx tsc --noEmit` | ✅ 통과 | exit 0, 에러 0 |
| 2. 기능 회귀 0(데이터패칭) | ✅ 통과 | SWR 키(partner/me·stats·campaigns)·fetcher·useSWR 호출 git diff 0. page.tsx statCards 데이터 동일(icon 키만 추가) |
| 2. 기능 회귀 0(server action) | ✅ 통과 | POST/handleCreate·PATCH/handleSave·placements POST·handleAddPlacement diff 0(버튼 래퍼만 Btn 교체) |
| 2. 기능 회귀 0(필터/폼/핸들러) | ✅ 통과 | filterTabs·VP2_TABS·isEditable·startEdit·updateField·totals reduce 불변. statusConfig는 **타입만** `{label,color,bg}`→`{label,tone}`(키 6종·라벨·색의미 동일). editFields 인라인→const 추출(데이터 동일) |
| 3. 미접촉(toss-admin.css·admin/*·referee/*·admin-toss/) | ✅ 통과 | `git status --porcelain src/`=partner-admin 5파일만 M. 그 외 src/ 변경 0 |
| 3. 미접촉(일반 (web)/* partner 제외) | ✅ 통과 | git diff에 partner 외 (web)/* 0 |
| 4. data-skin="toss" 부착 | ✅ 통과 | 셸 루트+page/venue/campaigns/[id] 각 루트+[id] 로딩 early-return(L135) 전부 부착 |
| 4. 사용 .ts-* 클래스 실재(신규규칙 0) | ✅ 통과 | ts-card/--flat·ts-field/__label·ts-input·ts-select·ts-segment/__btn·ts-chip·ts-badge--warn/--grey·ts-navlink 전부 toss-admin.css 실재(신규 추가 0, read만) |
| 4. 사용 bare 토큰 실재 | ✅ 통과 | --ink/--ink-mute/--ink-dim·--bg·--card·--border·--primary·--ok·--warn·--grey-50/-100 전부 `[data-skin="toss"]` 스코프 정의 실재 |
| 4. var(--color-*)/다크 잔존 0 | ✅ 통과 | partner-admin 5파일 `var(--color-` grep **0건**(mybdr 다크 토큰 전부 Toss bare로 교체) |
| 4. Material 잔존 0 / lucide만 | ✅ 통과 | `material-symbols` grep **0건**. 사용 16종(store·layout-dashboard·megaphone·building·clock·plus·share-2·eye·mouse-pointer-click·trending-up·layout-grid·id-card·circle-dot·arrow-left·pencil) 전부 lucide-react 실존(share-2→Share2 포함, 빈 span 폴백 0) |
| 4. AppNav admin 적용 0 | ✅ 통과 | AppNav/app-nav/AppHeader grep 0건 |
| 4. 하드코딩 hex 0(토큰 var만) | ✅ 통과 | `#hex` grep **0건**(navy hero #16213E·#1B3C87·badge #C0CCDB 등 전부 제거) |
| 5. 라이트·#3182F6 Toss 룩 | ✅ 통과 | `--primary:#3182F6`(Toss Blue)·`--bg:#F2F4F6`·`--card:#FFFFFF`·`--ink:#191F28`=라이트 톤, 전부 data-skin 스코프 |
| 보조: 키트 시그니처 정합 | ✅ 통과 | Btn(variant/size/icon)·Badge(tone:primary\|ok\|warn\|danger\|grey)·StatCard(icon/label/value) prop 전부 존재. campaigns tone 매핑(grey/warn/ok/danger) 유효 |
| 보조: dev3001 읽기 GET 렌더 | ✅ 통과 | /partner-admin·/campaigns·/venue **HTTP 307**(비로그인→/login 리다이렉트=layout 권한가드 정상). **500 서버에러 0**=컴파일·렌더 정상 |

📊 종합: **16개 중 16개 통과 / 0개 실패**. 차단 0. 수정 요청 0.

🔑 핵심 확인:
- **기능 회귀 0**: 5파일 git diff가 ①아이콘 Material→lucide ②className `.ts-*` ③`data-skin` 속성 ④인라인 hex/토큰→Toss bare 토큰 — 시각 속성만. 데이터패칭·server action·필터·폼·핸들러 로직 한 줄도 안 바뀜(statusConfig는 타입만, editFields는 const 추출).
- **미접촉 격리**: partner-admin/* 5파일만 M. toss-admin.css/admin/*/referee/*/admin-toss/는 read만(타 에이전트 동시작업 충돌 0).
- **디자인 분기 정합**: var(--color-*)/Material/하드코딩 hex/AppNav 전부 0. data-skin opt-in 셸+페이지 루트 부착, .ts-*·bare 토큰 전부 실재, 라이트 #3182F6 Toss 룩.

### 관리자 Toss 디자인시스템 이식 — Phase 0 검증 (2026-06-21, tsc+정적grep+읽기전용)

**환경**: 운영 write 0. `npx tsc --noEmit` + grep 정적 + git diff(읽기전용) + lucide-react 실export 대조(node require). dev 미기동. git 무수정.

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| 1. `npx tsc --noEmit` | ✅ 통과 | exit 0, 에러 0 |
| 1. 잔여 닫는태그 0 | ❌ **실패** | **toss-admin.css 237줄 `</invoke>` 잔여**(파일 마지막 줄). tsc는 CSS 미검사라 통과했으나 명시 요구사항 위반. 유효치 않은 CSS 토큰 |
| 2. 전역 :root Toss 토큰 0 | ✅ 통과 | bare `:root` 정의 0(매칭은 전부 주석). 토큰·`.ts-*` 전부 `[data-skin="toss"]` 스코프 |
| 2. globals/admin Toss색 침범 0 | ✅ 통과 | `#3182F6` grep → globals.css/admin.css 매칭 0 |
| 3. 프론트 회귀 0 | ✅ 통과 | globals.css·admin.css·`(web)/*` git diff 0. 변경=layout.tsx 1줄 + 신규 2(toss-admin.css·admin-toss/) |
| 4. opt-in (data-skin JSX 적용 0) | ✅ 통과 | `data-skin="toss"` 실적용처 0(매칭 전부 주석·CSS 셀렉터). Phase 0 시각변화 0 정상 |
| 5. CDN lucide / createIcons injection 0 | ✅ 통과 | createIcons·window.lucide·unpkg 실호출 0(1건 주석). lucide-react import만 |
| 5. Icon 매핑 정상 | ✅ 통과 | 코드 사용 아이콘(Search/ChevronUp·Down/ChevronsUpDown/Chevron L·R/RotateCcw/Check/Inbox/X/Banknote) 전부 lucide-react 실존. `WonSign` 실부재 확인→`won-sign→Banknote` 별칭 정당 |
| 6. barrel export 18종 | ✅ 통과 | Icon·Btn·Badge·Toggle·Check·StepDots·Empty·Modal·StatusTabs·FilterBar·SortHead·DataTable·DetailModal·StatCard·PanelStat·PanelRow·SectionTitle·MockToggle 전부 index.ts export |
| 보조: @keyframes 글로벌 충돌 0 | ✅ 통과 | tsfade·tsrise = ts프리픽스 고유, globals/admin 동명 0 |

📊 종합: **11개 중 10개 통과 / 1개 실패**. 차단성 낮음(마지막 줄·실렌더 영향 경미) 그러나 명시 요구(검증1 "잔여 0") 위반 → 수정 요청 1건.

💡 디자인 분기 격리(핵심)는 전부 통과: 전역 토큰 미오염·프론트 회귀 0·opt-in 미적용으로 Phase 0 시각변화 0 확인. lucide CDN→react 전환·별칭·barrel 모두 정상.

⚠️ 단 1건 실패 = CSS 끝 `</invoke>` 잔여물. developer가 한 줄 삭제하면 해결.

### 매칭 고도화 M6(최종) 검증 — 호스트 콘솔 ①데드코드 ②E-1 3구획 ③waiting탭+호스트카드 (2026-06-19, tsc+정적+읽기전용)

**환경**: 운영 단일 DB write 0(승인/거절/승격/취소 실호출 안 함). `npx tsc --noEmit` + 정적 코드경로 + 디자인 grep + git diff(읽기전용)만. dev 3001 미기동(OOM 회피)→정적 갈음. node 전체 kill 안 함. git 무수정.

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| `npx tsc --noEmit` | ✅ 통과 | exit 0, 에러 0 |
| schema 변경 0 | ✅ 통과 | `git diff prisma/schema.prisma` 비어있음 |
| 신규 API route 0 | ✅ 통과 | 변경 8파일 모두 page/컴포넌트/시안. 신규 route.ts 0 |
| ① my-games GAME_STATUS_LABEL 삭제 영향 0 | ✅ 통과 | 로컬맵 삭제(L46-49 주석), 참조 0건. GameCard 자체 STATUS_LABEL(game-status.ts) 사용→표시 영향 0 |
| ① 취소=4 "취소" 표시 | ✅ 통과 | host-game-card STATUS_META 4="취소", game-status.ts 정본 정합 |
| ② E-1 3구획 분류(대기0/확정1/대기열3) | ✅ 통과 | host-applications L151-156 status별 filter 정확. 대기열 waitlist_position asc 정렬 |
| ② 승격 confirm UI 분기(isPromotable) | ✅ 통과 | deadline!=null && >now만 "승격 확정"+카운트다운, 그 외 "빈자리 시 자동 알림"(L286,318-339) |
| ② 승격 410/409 처리 | ✅ 통과 | handlePromote 200/410/409 분기→router.refresh. confirm route 410(만료·demoteExpiredAndPromoteNext)/409(정원재마감) 정합 |
| ② 호스트(organizer)만 노출(IDOR) | ✅ 통과 | isHost일 때만 HostPanel. PATCH/confirm/DELETE 모두 organizer_id 가드. confirm은 본인‖호스트 |
| ③ waiting 탭/status=3 분류 | ✅ 통과 | page.tsx L223-233 status=3→waiting, 탭 순번순 정렬(L312-314). MyGamesClient waiting 탭 |
| ③ 호스트 카드 집계(groupBy 1회) | ✅ 통과 | groupBy([game_id,status]) 1회(L191-198), approved=1/pending=0/waiting=3 countMap |
| ③ waiting 배지/순번 | ✅ 통과 | status-badge "대기중"(accent), reg-row "대기 N번"(accent, L182-186), E-1 순번 원형배지 |
| ③ "마감확정" placeholder alert(비동작) | ✅ 통과 | recruiting 시 alert 안내만. status 전환 write 0. 취소만 실 write(미실행) |
| game_type Int 매핑(0픽업/1게스트/2연습) | ✅ 통과 | KIND_LABEL 0/1/2 정합. tsc 초기 string가정 에러 정정 확인 |
| 디자인 grep(hex/lucide/9999/핑크) | ✅ 통과 | 신규/수정 라인 위반 0. `#fff` 2건은 기존 라인(M6 diff 밖, git diff 확인) |
| 720px·44px·Material Symbols·정사각원형 | ✅ 통과 | host-game-card 44px, E-1 순번 rounded-[50%] 정사각(9999px 0), 720px 부모 grid |
| AppNav 무변경 | ✅ 통과 | nav 관련 파일 변경 0 |
| 시안 jsx 미수정 | ⚠️ 주의 | GameDetail.jsx 168줄 추가(역박제) — 실패 아님, 아래 PM 판단 |

📊 종합: **18개 중 17개 ✅ 통과 / 1개 ⚠️ 주의(실패 아님) / 0개 실패**. 차단 0.

#### ⚠️ 주의 1건 — 시안 GameDetail.jsx 변경(PM 판단, 수정 요청 아님)
- `Dev/design/BDR-current/.../screens/GameDetail.jsx` 168줄 추가 = 운영 3구획(HostApplicationsPanel)을 시안에 **역박제**(운영→시안 동기화).
- 프롬프트 ⚠️ "시안 jsx 미수정"과 표면상 충돌하나, **CLAUDE.md "운영→시안 동기화 룰"**(운영 UI 변경 시 BDR-current/ 같이 갱신)에 부합. 추가분 디자인 위반 토큰 0(var(--*)만·720px·44px). 운영 src/ 영향 0.
- → "시안 jsx 미수정 제약 vs 동기화 룰" 우선순위를 **PM 확정 권장**. 기능/디자인 회귀는 없음.

#### 참고 — 검증 한계(비차단)
- dev 미기동 정적 검증: 호스트 콘솔 클릭(승인/거절/승격/취소 시각·카운트다운)은 dev 3001 정상화 후 수동 확인 권장. 코드경로/IDOR/상태분기 정확성은 입증 완료. 운영 write 0.

### 매칭 고도화 M5 검증 — 찾기 UX(정렬·빠른필터·빈상태, 시안A) (2026-06-19, tsc+정적+읽기전용)

**환경**: 운영 단일 DB write 0. `npx tsc --noEmit` + 정적 코드경로 + 디자인 grep + git status(읽기전용)만. dev 3001 미기동(구현기록 OOM 명시)→정적 갈음. node 전체 kill 안 함. git 무수정. 대상 3파일: `(web)/games/page.tsx` / `_components/games-client.tsx` / `globals.css`(.games-chip*/.games-sort-*).

| 테스트 항목 | 결과 | 비고 |
|-----------|------|------|
| `npx tsc --noEmit` 전체 | ✅ 통과 | exit 0, 에러 0 |
| schema 변경 0 | ✅ 통과 | git status에 schema.prisma 없음. M5 SELECT preferred_regions 1회만(로그인 시) |
| 정렬 soon(임박순) 정확 | ✅ 통과 | sortGames: 미래(scheduled_at>=now) 우선 → 같은 군 내 시간 asc, 무효(null/NaN) Infinity로 맨 뒤 |
| 정렬 filling(모집임박) 정확 | ✅ 통과 | fillPct(b)-fillPct(a) desc. fillPct=round(cur/max*100), max≤0→0, 100 상한 |
| 정렬 latest(최신) 정확 | ✅ 통과 | createdAt desc, null→-Infinity 맨 뒤. page.tsx createdAt 매핑·listGames select created_at 확인 |
| take:60 전체로드(페이지 한계 0) | ✅ 통과 | listGames `take=60` 단일호출, skip/cursor 없음 → 메모리 정렬/필터가 목록 전체 정확 적용. listGames 무변경 |
| sort_options 좌표 "가까운순" 제외 | ✅ 통과 | sortOptions=soon/filling/latest 3개만(주석 명시). near 정렬 원천 미포함. 클라는 메타 옵션만 렌더 |
| chip "내 동네" 노출 제어(서버 메타) | ✅ 통과 | hasPreferredRegions>0 조건부 push. 비로그인/세션 sub 비정상(catch)/지역 미설정 → near 칩 미포함=비노출. 클라 하드코딩 0 |
| 칩 다중 AND | ✅ 통과 | `[...quickChips].every(matchesChip)` + clientFilters .filter 별도 레이어 AND 누적 후 sortGames |
| 칩 today/weekend 파생 정확 | ✅ 통과 | today=로컬 Y/M/D 일치, weekend=getDay 0\|6. scheduledAt null/NaN→false |
| 칩 filling(≥70)/free(0\|null) | ✅ 통과 | filling=fillPct>=70. free=`!feePerPerson \|\| Number(fee)===0`(null=무료 포함) |
| 칩 near(preferred 부분매칭) | ✅ 통과 | `${city} ${district}`.includes(r) some. preferredRegions 빈배열이면 false(이중 가드) |
| 빈상태: games 0건 단순안내 | ✅ 통과 | filtered 0 & games.length===0 → "등록된 경기가 없습니다."(CTA 없음, 5단어 이내) |
| 빈상태: 필터 0건 카드+CTA | ✅ 통과 | "조건에 맞는 모집이 없어요"+"필터를 줄이거나 인접 지역을 둘러보세요". [맞춤필터 끄기] 상시 / [인접 지역 보기]는 `quickChips.has("near")`일 때만. 버튼 minHeight 44px |
| 기존 회귀 KindTab/FilterChipBar/clientFilters | ✅ 통과 | KindTabBar counts·FilterChipBar collapsible(filterOpen)·clientFilters(weekend/free/beginner) .filter 보존. 신규는 별개 레이어 추가만. dot count(urlActiveCount+clientFilters.size) 보존 |
| listGames 서버함수/route 무변경 | ✅ 통과 | git status 수정파일=games-client.tsx/page.tsx/globals.css 딱 3개. game.ts·route.ts·game-card.tsx 무변경 |
| 디자인 grep(M5 라인) | ✅ 통과 | globals.css M5 블록(3006~) 하드코딩hex/lucide/9999/핑크살몬코랄 0(var(--*)만). games-client.tsx의 "lucide" 매치는 주석 텍스트뿐(실 import 0) |
| Material Symbols / 720px / 44px | ✅ 통과 | material-symbols 6회(swap_vert/expand_more/check/tune/add/search_off, lucide 0). @media 720px 분기(드롭다운↔하단시트). 정렬버튼/시트항목 min-height 44px |
| AppNav 무변경 / game-card 무변경 | ✅ 통과 | games-client.tsx에 AppNav/app-nav 참조 0(콘텐츠만). game-card.tsx git 무수정(M2 d2d3768 이후 변경 없음) |

📊 종합: **20개 중 20개 통과 / 0개 실패**. 차단 0. 수정 요청 0.

#### 참고 — 동작 검증 한계(비차단)
- **dev 미기동 정적 검증**: 브라우저 클릭(드롭다운/하단시트/칩 토글 시각)·로그인 preferred_regions 노출은 dev 3001 정상화 후 사용자 수동 확인 권장. 코드경로 정확성은 입증 완료. 운영 write 0.
- **정렬 안정성(구현기록 명시)**: filling 동률 다수 시 tie-break 비결정(미세 변동). 표시 영향 경미 — 차단 아님.
...
### 관리자 Toss Phase 0 (디자인시스템 이식) 리뷰 — 2026-06-21

📊 종합 판정: **통과** — 차단 0 / 권장 3 / 후속 2
대상 5파일(toss-admin.css·layout.tsx·admin-toss/kit.tsx·data.tsx·index.ts). `npx tsc --noEmit` exit 0(재검증). 네임스페이스 격리 완벽, 컴포넌트 이식 충실도 높음(누락 0·기능 동일), lucide CDN 완전 제거, Phase 0 범위(리스킨 0·schema 0·프론트 0) 정확. **머지 차단 사유 없음.**

✅ 잘된 점:
- **네임스페이스 격리 완벽(최우선 관점 — 통과).** toss-admin.css 전 규칙이 `[data-skin="toss"]` 시작(토큰 정의도 bare :root 0, 스코프 루트에만). 시안 원본은 `:root`+`body`에 bare 정의였는데(원본 toss.css 확인) 운영 admin.css `.admin-shell`이 같은 이름 전역토큰(`--bg/--ink/--ok/--warn/--danger`)을 쓰는 게 실재 확인 → bare로 풀었으면 미리스킨 화면 즉시 색 뒤집힘. opt-in 스코프로 충돌 원천 차단. globals.css·admin.css에 toss 토큰/`data-skin` 침범 0(grep 0건), `(web)/*`·globals·admin git diff 0(회귀 0). Phase 0 JSX에 data-skin 적용처 0 = 시각 변화 0. **opt-in 전엔 토큰명 충돌(--bg 등) 완전 무영향.**
- **컴포넌트 이식 충실도 높음(누락 0).** kit 8개(Icon·Btn·Badge·Toggle·Check·StepDots·Empty·Modal)+data 10개(SectionTitle·StatusTabs·FilterBar·SortHead·DataTable·DetailModal·StatCard·PanelStat·PanelRow·MockToggle) 시안과 1:1. DataTable 핵심기능(정렬위임·전체/행선택+stopPropagation·페이지네이션 disabled·empty/loading·onRowClick·gridCols 44px) 전부 보존. 제네릭 `<T>`+`String(rowKey)` key 캐스팅·`onSelectChange?.` optional 체이닝은 타입안전/크래시방어 **개선**. 시안 주석의 "Toast"는 시안 본문에도 컴포넌트 정의 없음(주석 오기)→미이식 정상(.ts-toast CSS는 존재, 인라인 사용 가능).
- **lucide CDN 완전 제거.** 시안 `window.lucide.createIcons()`/CDN injection 0 → `import * as Lucide from "lucide-react"`만. kebab→Pascal 변환 정확(search→Search, chevrons-up-down→ChevronsUpDown, rotate-ccw→RotateCcw 등). 이식 두 파일이 실사용하는 10종(search/inbox/check/chevron-*/chevrons-up-down/x/rotate-ccw) 전부 lucide-react 1.7.0 실재 확인. 매핑 실패 시 빈 span 폴백(런타임 에러·레이아웃깨짐 0). lucide-react ^1.7.0 API(named PascalCase export) 호환 확인.
- **Phase 0 범위 정확.** layout.tsx는 import 1줄(+주석)만, 기존 admin.css import 바로 아래 순서(globals→admin→toss). 실제 admin 화면 리스킨 0, schema 변경 0(prisma 무관), 프론트 무영향. 하드코딩 hex: 컴포넌트 0(인라인 #fff 흰색만, 시안 동일), CSS는 `.ts-chip--age`(#F3F0FE/#6D5AE6) 1쌍이나 **시안 toss.css L124 verbatim**(연령칩 도메인색, 토큰 팔레트 외)이라 박제 충실로 정상. `.ts-modal__x{position:absolute}`도 시안 L210 verbatim.
- **CSS 구조 정합.** `.ts-*` 클래스가 시안 toss.css와 1:1(라이트·#3182F6·radius-card 24px/btn 16px/input 14px/chip 12px·sh-xs~lg shadow·Pretendard). @keyframes(tsfade/tsrise)만 글로벌이나 `ts` 프리픽스로 globals.css 충돌 0. barrel(index.ts) 컴포넌트+타입 전부 노출, 단일 진입점.

🟡 권장 수정 (머지 차단 아님 — 검토 권장):
- **[kit.tsx·data.tsx — a11y 보강(Phase 1 리스킨 전 권장)]** ①`DetailModal`(data.tsx L495~)에 `role="dialog"`+`aria-modal="true"` 없음(`Modal`엔 있음 — 비대칭). ②`Check`(kit L149)·DataTable 체크박스(data L366·L399)에 `role="checkbox"`+`aria-checked` 없어 스크린리더가 체크 상태 못 읽음. ③모달 닫기 X 버튼(아이콘 전용)에 `aria-label="닫기"` 없음. ④모달 focus trap·열림 시 포커스 이동 없음(ESC 닫기는 있음). **심각도: 저~중** — Phase 0은 렌더 화면 0이라 즉시 영향 0이나, Phase 1+ 리스킨 시 키트가 그대로 노출되므로 **리스킨 착수 전 보강 권장.**
- **[kit.tsx ICON_ALIAS — Phase 1+ 아이콘 별칭 보강 필요]** 시안 화면/데이터 파일(Phase 1+ 이식 호출부)이 쓰는 `thunderdunk`가 lucide-react 1.7.0에 **부재**(kebab→Pascal=Thunderdunk 없음, 실측 MISSING) + ICON_ALIAS 미등록 → 빈 span. 브랜드 로고용 추정. (`check-circle-2`·`volleyball`은 실측 EXISTS — 안전.) **심각도: 저** — Phase 0 호출부 미이식이라 무영향. Phase 1+ 이식 시 `thunderdunk` 별칭 추가 또는 호출부 치환 필요. 빈 span 폴백이라 크래시 0.
- **[data.tsx FilterBar select 라벨 패턴 확인]** L176~178 `<option>{f.label}: {o.label}` — 모든 옵션 텍스트에 "라벨: " 접두(예 "상태: 전체"/"상태: 승인"). 시안 동일 거동이나, 실제 admin 필터 이식 시 옵션 가독성 검토 권장. **심각도: 저** — 동작 무관, 표시 취향.

🔵 후속 (동작영향 0):
- **MockToggle은 시안 데모 전용(filled/empty/loading 강제 전환).** 실 admin 화면 이식 시 제거 또는 dev-only 가드 대상. barrel 노출돼 있으나 Phase 1+ 미사용 시 무해. 인지만.
- **DataTable pagination은 클라 page만 전달(total/perPage는 호출부 책임).** 서버 페이지네이션 연동은 Phase 1+ 호출부 결정. 키트 자체는 표시·이벤트만 — 올바른 분리. 인지만.

### 매칭 M5 찾기 UX(정렬·빠른필터·빈상태, 시안A) 리뷰 — 2026-06-19

📊 종합 판정: **통과** — 차단 0 / 권장 2 / 후속 2
대상 3파일(games/page.tsx·games-client.tsx·globals.css .games-chip*/.games-sort-*). tsc --noEmit exit 0. 정렬/칩/파생 로직 정확(NaN·0division 방어 견고), 메타 서버제어로 좌표 0 반영 깔끔, 기존 KindTab/FilterChipBar/clientFilters/listGames 무변경 회귀 0, take=60 전체로드라 메모리 정렬 정확, 디자인 13룰 준수. **머지 차단 사유 없음.**

✅ 잘된 점:
- **정렬/필터 로직 정확 + 방어 견고.** fillPct: `max<=0 → return 0`으로 0division 차단, `Math.min(100, round)`로 초과 정원도 상한. soon: scheduledAt null/무효는 `NaN→Infinity`로 맨 뒤, 미래(`>=now`)를 과거보다 앞에 둔 2단 비교(aFuture!==bFuture 먼저) 후 시간 오름차순 — 임박순 의미 정확. latest: createdAt null은 `-Infinity`로 맨 뒤, desc. filling: fillPct desc. today/weekend/filling/free/near 칩 파생 전부 null·NaN 가드(scheduledAt 없으면 false, `Number.isNaN(d.getTime())` 체크). free는 `!feePerPerson || fee===0`(null=무료) 일관. 칩 다중 AND = `[...quickChips].every()` 정확.
- **메타 서버제어 — 클라 하드코딩 0.** page.tsx가 sortOptions(soon/filling/latest, "가까운순" 미포함)·chipOptions(near는 `hasPreferredRegions`일 때만 push) 계산 → GamesClient는 메타가 가진 옵션만 `.map()` 렌더. 좌표 0 판정·세션/preferred_regions 판정 전부 서버. 향후 좌표 컬럼 도입 시 page.tsx 메타에만 추가하면 됨(클라 무변경). SortMenu도 `options.length>1`일 때만 노출.
- **기존 레이어 회귀 0.** FilterChipBar 컴포넌트 무변경(URL date/city + 클라 weekend/free/beginner 그대로). clientFilters 필터 블록 보존, M5 quickChips는 **별개 Set·별개 useMemo 단계**로 추가(① clientFilters → ② quickChips → ③ sortGames AND 누적). listGames/route/groupBy/typeCounts/KindTabBar 무변경. game-card.tsx 무변경(M2 대기뱃지와 중복 0 — M5는 카드 자체 안 건드림).
- **preferred_regions 조회 안전.** getWebSession→sub→`prisma.user.findUnique(select preferred_regions만)` 1회(로그인 시만, 비로그인 0회). `Array.isArray` + 문자열·trim 길이 필터로 Json 오염 방어, `BigInt(session.sub)` 비정상 시 try/catch로 `preferredRegions=[]` 폴백(near 칩만 숨고 나머지 동작 유지). 기존 api/web/games/route.ts와 동일하게 preferred_regions를 지역 매칭에 사용 — 패턴 일관.
- **성능 합리적.** take=60 전체 로드 위 메모리 정렬/필터라 페이지네이션 충돌 0(무한스크롤/더보기 없음). useMemo 의존성(games·clientFilters·quickChips·sort·preferredRegions) 정확. preferred SELECT는 로그인 시 1회·단일 컬럼 — 영향 무시 가능.
- **빈상태 UX 정확.** games 0건=CTA 없는 단순 안내("등록된 경기가 없습니다."), 필터로 0건=시안 카드+CTA. [맞춤필터 끄기]=quickChips 전체 리셋, [인접 지역 보기]=`quickChips.has("near")`일 때만 노출+near만 delete. 문구 5단어 이내("조건에 맞는 모집이 없어요"/"필터를 줄이거나 인접 지역을 둘러보세요"). 버튼 minHeight 44px.
- **디자인 13룰 준수.** var(--*) 토큰만(cafe-blue/accent/ink-on-brand/border/bg-card/bg-alt/sh-lg/radius-chip/radius-card/ink-dim/ink-mute)·하드코딩 hex 0·Material Symbols(swap_vert/expand_more/check/search_off/tune/add, lucide 0)·720px 분기(데스크톱 드롭다운↔모바일 하단시트, 시트항목/버튼 44px·safe-area-inset)·pill 9999px 미사용(radius-chip)·AppNav 무관·시안 jsx 미수정. SortMenu 바깥클릭 오버레이(fixed inset-0)·aria(haspopup/expanded/listbox/option/selected/pressed) 양호.

🟡 권장 수정 (머지 전 검토 권장):
- **[games-client.tsx L314~319 — near 부분 includes false positive]** `area="${city} ${district}"` + `preferredRegions.some(r => area.includes(r))` 단순 문자열 포함이라, preferred="서울"이고 어떤 경기 district가 "서울대입구"(city=관악구)면 매칭됨, "성동" 선호 시 "성동구"·"성북…성동" 등 부분일치 가능. **심각도: 저** — preferred_regions는 보통 시도/구 단위 선택값이라 실무 데이터에선 위험 낮고, dev가 명시한 "거리 정렬 아닌 지역 라벨 매칭" 트레이드오프. 정밀화 원하면 토큰 분리(`area.split(/\s+/).includes(r)`) 또는 city/district 각각 정확매칭. **정책 확인(차단 아님).**
- **[games-client.tsx L529·L625 — 칩 리셋 2버튼 동작 동일 혼선]** 칩 바 "초기화 ✕"와 빈상태 CTA "맞춤필터 끄기" 둘 다 `handleResetQuickChips`(quickChips만 리셋). FilterChipBar(clientFilters weekend/free/beginner)·URL 칩(date/city)은 안 비움 → 그 칩들로만 0건이면 "맞춤필터 끄기" 눌러도 여전히 0건일 수 있음(quickChips 비어있어 버튼은 늘 노출). **심각도: 저** — quickChips 0건일 때 CTA가 헛도는 케이스. 빈상태 카드의 "맞춤필터 끄기"를 quickChips.size>0일 때만 노출하거나, clientFilters까지 함께 리셋 고려. **정책 확인.**

🔵 후속 (동작영향 0 또는 경미):
- **[games-client.tsx L482 sortGames — tie-break 비결정]** Array.prototype.sort는 안정정렬 보장(ES2019+, 모던 브라우저 OK)이라 동률(filling 동일 fill_pct) 시 원본 순서 유지 → 실무 영향 0. dev 언급 "비안정 환경 대비 안 함"은 구형 환경 가정인데 타깃 브라우저에선 안정정렬이라 무해. 인지만.
- **[기존 vs M5 칩 의미 중복]** FilterChipBar의 weekend/free와 M5 quickChips의 weekend/free 의미 중복(둘 다 켜면 동일 조건 AND=무해). 의도적 분리(기존 동작 보존)라 동작영향 0. UX상 같은 라벨 2곳 존재는 향후 통합 검토 여지. 인지만.
...
| 대상 | 문제 | 상태 |
|------|------|------|
| src/styles/toss-admin.css L237 (Phase 0, tester) | 박제 시 잘못 삽입된 `</invoke>` 잔여 태그가 파일 마지막 줄에 남음(`.ts-phone` 규칙 다음). 유효치 않은 CSS 토큰 — 검증1 "잔여 닫는태그 0" 명시 요구 위반. tsc는 CSS 미검사라 통과했으나 박제 무결성 깨짐. **해결: L237 한 줄 삭제**(`.ts-phone`이 진짜 마지막 규칙). 실렌더 영향 경미하나 회신 검증 항목이라 처리 필요 | ✅ 완료(Phase0 보완1차) — L237 `</invoke>` 삭제. grep src/ 전체 잔여 0 확인. tsc exit 0 |
...
| **toss-admin.css L237 `</invoke>` 잔여 태그 (Phase 0 차단, reviewer+tester 독립확인)** | 파일 끝(`.ts-phone` 다음) 줄에 박제 산출물 잔여 XML 태그 `</invoke>`가 그대로 남음. 유효치 않은 CSS(tsc는 CSS 미검사라 통과했으나 실제 잔존). 대부분 파서가 무시하나 malformed source·"잔여0" 위반·파싱경고 가능. **그 한 줄 삭제만** 필요(다른 변경 없음) | ✅ 완료(Phase0 보완1차) — 삭제 완료. grep src/ 잔여 0 |
...
| 날짜 | 작업 | 결과 |
|------|------|------|
| 2026-06-21 | 관리자 Toss **Phase 1** AdminShell 셸크롬+5화면 리스킨 검증 (tester) | ✅ **14/14 통과·실패0**. tsc0·변경 13파일만(partner/referee/(web) 0). **기능 회귀0**: 9페이지 diff 삭제측 로직키워드(fetch/prisma/where/orderBy/status/filter/onClick/router/POST·PATCH) grep0, 유일 삭제1줄=community 아이콘 삼항 status분기 보존한 Material→lucide 등가스왑. 토큰 리매핑 전 규칙 `[data-skin="toss"]` 스코프(bare:root 0)·globals/admin.css diff0. data-skin=셸크롬(topbar/aside/모바일토글·오버레이·드로어)+5페이지 루트div만, **공유 .admin-shell/.admin-main/__inner 미부착**(grep). 라이트 고정(`[data-skin]`+`[data-theme=dark] [data-skin]` 둘다 라이트, 후자 명세도 우선)·ThemeSwitch 유지. material잔존 셸+5화면0(매칭=주석/Phase2 미대상). **lucide 32종 node 실측 전부 실존**(volleyball 대체 정당, 빈아이콘0). var(--color-*) 직접/AppNav 0. **커밋 권장** |
| 2026-06-21 | 관리자 Toss **Phase 1** AdminShell 셸크롬+5화면 리스킨 (developer) | ✅ 13파일(toss-admin.css+셸3+5페이지+자식4). **토큰 리매핑**: globals 베이스토큰(`--bg-card/--ink/--accent/--ok/--warn/--danger/--cafe-blue*`등)을 `[data-skin="toss"]`+`[data-theme="dark"] [data-skin="toss"]` 둘다 라이트Toss값으로(admin.css `--color-*` 별칭 자동전파→마크업0변경 흡수·다크누수0). data-skin opt-in=셸크롬(`<aside>`/모바일토글·오버레이·드로어/`.admin-topbar`)+5페이지 루트div만(**공유 .admin-shell/.admin-main/__inner 금지**, grep0). Material→lucide(`<Icon>` 키트 경유): flag/shield/bell-ring/user-plus/user/eye/eye-off/trash-2/ban/rotate-ccw/triangle-alert/사이드바14종. sports_basketball→volleyball(Dribbble 부재 node실측 대체). **로직 회귀0**(컬럼/필터/정렬/액션/모달/문구/라우트/server action diff0)·ThemeSwitch유지. tsc0·material잔존 셸+5화면0·globals/admin.css diff0·partner-admin/referee/(web)일반 변경0·schema0·신규route0·CDN injection0·운영write0 |
| 2026-06-21 | 관리자 Toss Phase 3 PR-A partner-admin 재스킨 (developer) | ✅ partner-admin/* 5파일(layout·page·venue·campaigns·campaigns/[id]) Toss 재스킨. 변경 3종만=①Material→lucide(`<Icon>` 키트)②className `.ts-*`③`data-skin="toss"` 루트(셸+각 페이지+[id]로딩 early-return). **기능 회귀0**: 데이터패칭·server logic(super_admin 가드·SWR키·POST/PATCH body·filterTabs·statusConfig 분기·VP2_TABS·isEditable·handleSave/Create/AddPlacement) diff0. navy hero→H+StatCard KPI grid. 필터=.ts-chip·카드=.ts-card·폼=.ts-field/.ts-input/.ts-select·sub-tab=.ts-segment·상태=Badge(tone)·버튼=Btn. PhoneInput=className만 .ts-input. lucide 15종 전부 실존(별칭불필요). **toss-admin.css 신규규칙0**(사용 .ts-* 전부 실재 확인)·인라인 토큰var()만(hex0). tsc0·git porcelain=partner-admin 5파일만 M(toss-admin.css·admin/*·referee/*·admin-toss/ diff0)·schema0·신규route0 |
| 2026-06-21 | 관리자 Toss 디자인시스템 이식 Phase 0 리뷰 (reviewer) | 📊 통과 차단0/권장3/후속2. 단 tester 발견 **toss-admin.css L237 `</invoke>` 잔여태그 독립확인=실잔존**(수정요청1 등록, 머지 전 한 줄 삭제). ✅ 네임스페이스 격리 완벽(전 규칙 `[data-skin="toss"]` 시작·bare:root 0·시안 원본은 :root/body bare였음·admin.css `.admin-shell`이 같은이름 전역토큰 사용 실재→opt-in이 충돌 원천차단·globals/admin/`(web)/*` diff0·data-skin JSX0=시각변화0)·컴포넌트 이식충실(kit8+data10 시안1:1·DataTable 전기능보존·제네릭<T>/optional체이닝 개선·Toast는 시안도 정의無 주석오기)·lucide CDN완전제거(import만·이식2파일 10아이콘 전부 1.7.0 실존·빈span폴백)·범위정확(리스킨0·schema0·프론트0·하드코딩hex 컴포넌트0/.ts-chip--age는 시안verbatim). 권장①a11y(DetailModal role/aria-modal누락·Check/체크박스 role없음·X버튼 aria-label·focus trap, Phase1 전 보강)②ICON_ALIAS thunderdunk 부재(실측MISSING)별칭 Phase1+ 필요(check-circle-2/volleyball은 EXISTS)③FilterBar 옵션 "라벨:" 접두. 후속 MockToggle 데모전용·pagination 분리 |
| 2026-06-21 | 관리자 Toss 디자인시스템 이식 Phase 0 검증 (tester) | 📊 11개 중 10 통과/1 실패. ✅ tsc0·전역:root Toss토큰0(bare:root 정의0·전부`[data-skin="toss"]`스코프)·globals/admin #3182F6 침범0·프론트회귀0(globals/admin/`(web)/*` diff0, 변경=layout 1줄+신규2)·**data-skin JSX 실적용0**(Phase0 시각변화0 정상)·CDN createIcons/window.lucide injection0(lucide-react import만)·Icon매핑 전부 lucide실존(WonSign 실부재→won-sign→Banknote 별칭 정당)·barrel 18종 전부 export·@keyframes tsfade/tsrise 고유(충돌0). ❌ **toss-admin.css L237 `</invoke>` 잔여 태그**(파일 끝, `.ts-phone` 다음, 유효치 않은 CSS — 검증1 "잔여0" 위반, tsc는 CSS 미검사라 미감지)→수정요청1(한 줄 삭제). 운영write0·git무수정 |
| 2026-06-21 | 관리자 Toss 디자인시스템 이식 Phase 0 (developer) | ✅ 이식만(리스킨0·schema0·프론트변경0). 산출5: toss-admin.css(신규·**Toss 토큰+`.ts-*` 전부 `[data-skin="toss"]` opt-in 스코프 격리**, bare:root 0)·layout.tsx(import1줄)·admin-toss/kit.tsx(Icon[lucide-react]·Btn·Badge·Toggle·Check·StepDots·Empty·Modal, kebab→Pascal 변환·won-sign→Banknote 별칭)·data.tsx(StatusTabs·FilterBar·SortHead·DataTable제네릭`<T>`·DetailModal·StatCard·PanelStat·PanelRow·SectionTitle·MockToggle)·index.ts barrel. **CDN createIcons 제거→lucide-react import**·window 전역노출→named export+TS타입. tsc0(초기 `</content>` 4파일 잔존 제거 후). 검증: 전역:root Toss토큰0·**data-skin JSX 실적용0**(Phase0)·createIcons실호출0·globals/admin.css/`(web)/*` diff0 |
| 2026-06-19 | 매칭 M6(최종) 호스트콘솔 리뷰 (reviewer) | 📊 통과 차단0/권장0/후속3. E-1 권한/IDOR 견고(confirm 본인‖호스트+status3+deadline410+tx current<max 409, PATCH 호스트단독+status0만)·**클라는 표시분기만 권한 전부 서버판정**·승격 isPromotable=deadline활성=서버 confirm 성공조건 정합(410/409/200 refresh)·데드코드 GAME_STATUS_LABEL 실참조0(GameCard 정본 영향0)·groupBy 1회 N+1회피(approved=status1 실승인수)·waiting 분류/순번정렬/배지 정합·game_type Int(0픽업1게스트2연습) GameCard 정합·title null fallback·디자인13룰(var(--*)·rounded-[50%]·Material Symbols 미신설·AppNav무관·시안미수정·badge--blue/ok/ghost 실재). 후속①마감확정 status전용 API부재(공백아님-정원충족 자동전환 M1, 조기마감만 불가)·②DELETE 취소알림 status[0,1]만→대기열3 미수신(기존 M1, in절 미갱신)·③취소경기 confirm 게임status 미검사 이론적 엣지(트리거 거의無, game.status가드 권장) |
| 2026-06-19 | 매칭 M6(최종) 호스트콘솔 ①데드코드②E-1 3구획③E-2 (developer) | ✅ ①my-games 미사용 GAME_STATUS_LABEL(참조0) 삭제(GameCard 자체 라벨 영향0). ②GameDetail host-applications 2→**3구획**(대기승인0/확정1/대기열3 순번배지+승격), page hostApplicants에 waitlist_position·promotion_deadline 추가(추가조회0), host-panel 타입 확장. **승격=confirm POST 재사용**(deadline 활성 대기자만 "승격 확정"+카운트다운/그외 "대기중·자동알림", 410만료·409정원재마감→refresh, "승격알림" 미박제). ③MyGames **waiting 탭**(status3 분류·순번정렬)·status-badge/reg-row waiting 라벨("대기 N번")·하단 **HostGameCard 신규**(현황칩 승인/정원·대기·대기열 + 빠른액션, **취소=DELETE 재사용**)·요약 stat·groupBy 1회 집계(과다조회 회피). **schema0·신규route0**. ⚠️마감확정=PATCH가 status 미수용→안내alert(정원충족 자동확정 M1 존재). tsc0·dev GET my-games200/games200·운영write0 |
| 2026-06-19 | 매칭 M5 찾기UX(정렬·빠른필터·빈상태) 리뷰 (reviewer) | 📊 통과 차단0/권장2/후속2. tsc0. 정렬/필터 방어 견고(fillPct max<=0→0·Math.min상한·soon NaN→Infinity 미래우선·latest null→-Infinity·칩 null/NaN가드·AND every)·메타 서버제어(sortOptions 가까운순제외·near는 hasPreferredRegions時만 push, 클라 하드코딩0)·기존 KindTab/FilterChipBar/clientFilters/listGames/game-card 무변경 회귀0(별개 Set·별개 useMemo 단계 AND)·preferred_regions SELECT 1회 try/catch폴백·take60 전체로드 정렬정확·빈상태 CTA(games0=안내/필터0=카드+맞춤필터끄기·near時 인접지역)·디자인13룰(토큰·Material Symbols·720px 드롭다운↔시트 44px·pill0). 권장①=near area.includes 부분매칭 false positive(서울↔서울대입구, dev명시 트레이드오프·심각도저)·권장②=맞춤필터끄기 CTA가 quickChips만 리셋(clientFilters/URL칩 0건 시 헛돔). 후속=tie-break(ES2019 안정정렬이라 무해)·기존vsM5 weekend/free 의미중복(무해) |
| 2026-06-19 | 매칭 M5 찾기UX(정렬·빠른필터·빈상태, 시안A) (developer) | ✅ page.tsx: 좌표0 확정(games lat/lng없음)→sortOptions(soon/filling/latest, **가까운순 제외**)·chipOptions(today/weekend/filling/free + near는 로그인&preferred_regions時만) 메타 계산·GameForClient createdAt추가·preferred_regions SELECT 1회(로그인時). games-client.tsx: 정렬 메모리(soon 미래우선/filling fill_pct desc/latest created_at desc)·SortMenu(데스크톱 드롭다운/모바일 하단시트, Material Symbols)·빠른필터칩 AND·빈상태 카드+CTA(맞춤필터끄기/인접지역보기). **기존 KindTab/FilterChipBar/clientFilters 보존**(별개 레이어 AND). globals.css .games-chip/.games-sort-* 토큰 박제. **listGames=전체로드(take60·skip/cursor·무한스크롤 없음)→메모리정렬 전체 정확·페이지네이션 한계없음**. game-card.tsx 무변경. tsc0·schema0·운영write0(prisma.users→user 정정 1건) |
