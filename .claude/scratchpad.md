...
| 날짜 | 커밋 | 작업 | 결과 |
|------|------|------|------|
| 2026-04-29 | (미커밋, subin) | **/profile/edit 5탭 모바일 깨짐 긴급 픽스** (developer): 사용자 캡처 45 — 366px viewport 5탭 라벨 세로 한 글자씩 줄바꿈("기/본/정/보"). 원인: L1122-1133 styled-jsx `.profile-edit-grid > aside { flex-direction: row; overflow-x: auto }`가 5탭을 가로로 강제 배치 → 366÷5=73px 폭에서 한국어 4글자 못 들어감. PM 옵션 1(가로 스크롤) 채택. **수정**: aside 셀렉터에 `-webkit-overflow-scrolling: touch` 추가 + `aside > button` 신규 룰 5종(`min-width:100px / white-space:nowrap / flex-shrink:0 / border-left:0 / border-bottom:3px solid transparent / justify-content:center`) — 좌측 borderLeft 활성 표시를 모바일에서 하단 borderBottom으로 전환(가로 배치 자연스러움). button에 `data-active={active}` prop 추가하여 `[data-active="true"]` 셀렉터로 활성 탭 하단 accent 표시. **부제 줄바꿈 픽스**: 본문 영역 p/label/span에 `word-break: keep-all` — "이/름" 글자 단위 끊김 → 어절 단위. 박제 룰 준수: var(--*) 토큰만, alert/Material Symbols/radius 4px 신규 0건. tsc 0 에러. 변경 +21/-1 = 1 파일. | ✅ |
