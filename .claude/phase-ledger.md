...
| **⑪ 운영 박제 (Phase 1C)** | 1C | 🔵 진행 중 (batch) | CLI | 2026-05-28 | PR #650 누적. **PR-1C-1~7 ✅** (`40d19db` UA1 / `9734de4` UA2+UC2 / `19dfa03` UB1 / `e4e629b` UA3 / `7e57690` UC1 / `08524bd` PA5 Matches 옵션A / `<PR-1C-7 commit>` PA1 AdminList = 4 옵션 인라인 panel admin-entry-cta + admin-tournament-list 신규 / 4 옵션 운영 실재 라우트 / 시안 미fetch 컬럼 제외). 모두 API/AppNav/가짜링크 변경 0 / tsc 0. 잔여 9 PR (PR-1C-8~16). **admin PR 구조 점검: matches만 server+client 괴리 / 잔여 PA8·PA4·PA3 일부 client / PA9·PA6·PA2 page 자체 UI**. PR별 상세 = scratchpad. |
